function [A,dimension,numVars] =xloadHAD(dName,timestep,stage,precision)
%dName dir name
%numsteps 
% precision
...
%fnames=["alpha","chi","trK","Gamt1","Gamt2","Gamt3","shift1", "shift2","shift3", "gb1","gb2","gb3", "gt11","gt12","gt13", "gt22", "gt23","gt33","A11","A12", "A13", "A22","A23","A33"];
%file_index=10000*(stage+1)+timestep
file_index=3*timestep+ (stage+1);
numVars=size(fnames,2);

filename=dName+"/"+fnames(1)+"_"+string(file_index)+".dat";
%filename=dName+"/"+"dx_alpha_"+string(file_index)+".dat";
...
for v=1:size(fnames,2)
    filename=dName+"/"+fnames(v)+"_"+string(file_index)+".dat";
    %filename=dName+"/"+"dx_alpha_"+string(file_index)+".dat";
    fid = fopen(filename, 'rb');
    x = fread(fid,1,'uint');
    y = fread(fid,1,'uint');
