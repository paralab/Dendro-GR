...
int main(int argc, char** argv) {
    ...
    //     EM3_NUM_VARS: "<<em3::EM3_NUM_VARS<<" is not divisable by
    //     EM3_ASYNC_COMM_K: "<<em3::EM3_ASYNC_COMM_K<<std::endl; exit(0);
    // }
    ...
    // t,double*var){em3::analyticalSol(x,y,z,t,var);};
    ...
    if (em3::EM3_ENABLE_BLOCK_ADAPTIVITY) {
        ...
        const Point pt_max(em3::EM3_BLK_MAX_X, em3::EM3_BLK_MAX_Y,
                           em3::EM3_BLK_MAX_Z);
        em3::blockAdaptiveOctree(
            tmpNodes, pt_min, pt_max,
            m_uiMaxDepth - (binOp::fastLog2(em3::EM3_ELE_ORDER)), m_uiMaxDepth,
            comm);
        ...
        function2Octree(f_init, em3::EM3_NUM_VARS,
                        ...
                        em3::EM3_NUM_REFINE_VARS, tmpNodes, m_uiMaxDepth,
                        em3::EM3_WAVELET_TOL, em3::EM3_ELE_ORDER, comm);
    }

    // this is to tackle for higher dx sometime function to octree does not have
    // enought resolution to perform wavelet computation.
    tmpNodes.clear();
    ...
    if (!em3::EM3_ENABLE_BLOCK_ADAPTIVITY)
        ot::meshWAMRConvergence(mesh, f_init, em3::EM3_WAVELET_TOL,
                                em3::EM3_NUM_VARS, em3::EM3_ELE_ORDER,
                                em3::EM3_REFINE_VARIABLE_INDICES,
                                em3::EM3_NUM_REFINE_VARS, 5);
    ...
    if (TS_MODE == 0) {
        ...
        ts::ExplicitNUTS<DendroScalar, em3::EM3Ctx>* enuts =
            new ts::ExplicitNUTS<DendroScalar, em3::EM3Ctx>(appCtx);
        ...
        for (enuts->init(); enuts->curr_time() < em3::EM3_RK45_TIME_END;
            ...
            if (isRemesh) {
                ...
                appCtx->remesh_and_gridtransfer(
                    em3::EM3_DENDRO_GRAIN_SZ, em3::EM3_LOAD_IMB_TOL,
                    em3::EM3_SPLIT_FIX, true, false, false);
                // appCtx->terminal_output();
            }

            enuts->sync_with_mesh();

            if ((step % em3::EM3_IO_OUTPUT_FREQ) == 0) appCtx->write_vtu();

            if ((step % em3::EM3_CHECKPT_FREQ) == 0) appCtx->write_checkpt();
            ...
            sprintf(fName, "%s_enuts.prof",
                    em3::EM3_PROFILE_FILE_PREFIX.c_str());
            if (!rank) {
        ...
        ts::ETS<DendroScalar, em3::EM3Ctx>* ets =
            new ts::ETS<DendroScalar, em3::EM3Ctx>(appCtx);
        ets->set_evolve_vars(appCtx->get_evolution_vars());
        ets->set_ets_coefficients(tsType);

        for (ets->init(); ets->curr_time() < em3::EM3_RK45_TIME_END;
             ets->evolve()) {
