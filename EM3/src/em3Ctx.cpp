...
EM3Ctx::EM3Ctx(ot::Mesh* pMesh) : Ctx() {
    ...
    m_uiCVar = this->create_vec(ts::CTXVType::CONSTRAINT, true, false, false,
                                EM3_CONSTRAINT_NUM_VARS);
    m_uiPVar =
        this->create_vec(ts::CTXVType::PRIMITIVE, true, false, false,
                         EM3_NUM_VARS);  // used as to compare the analytical
...
int EM3Ctx::rhs(DVec* in, DVec* out, unsigned int sz, DendroScalar time) {
    ...
    for (unsigned int blk = 0; blk < numBlocks; blk++) {
        ...
        em3rhs(unzipOut, (const DendroScalar**)unzipIn, offset, ptmin, ptmax,
               lsz, bflag);
...
bool EM3Ctx::is_remesh() {
    ...
    if (EM3_ENABLE_BLOCK_ADAPTIVITY) return isRefine;
    ...
    isRefine = m_uiMesh->isReMeshUnzip((const double**)unzipVar, refineVarIds,
                                       EM3_NUM_REFINE_VARS, waveletTolFunc,
                                       EM3_DENDRO_AMR_FAC);
...
DVec EM3Ctx::get_evolution_vars() { return m_uiEVar; }

DVec EM3Ctx::get_constraint_vars() { return m_uiCVar; }

DVec EM3Ctx::get_primitive_vars() { return m_uiPVar; }
...
int EM3Ctx::compute_constraints() {
    ...
    if (!m_uiMesh->getMPIRank()) {
        ...
        if (m_uiTinfo._m_uiStep == 0) {
            ...
            for (unsigned int v = 0; v < EM3_CONSTRAINT_NUM_VARS; v++)
                fileGW << EM3_CONSTRAINT_VAR_NAMES[v] << "_l2\t"
                       << EM3_CONSTRAINT_VAR_NAMES[v] << "_l2rs\t";
