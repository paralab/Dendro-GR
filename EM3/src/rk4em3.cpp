...
namespace ode
{
    namespace solver
    {
        ...
        RK4_EM3::RK4_EM3(ot::Mesh *pMesh, double pTBegin, double pTEnd,double pTh): RK(pMesh,pTBegin,pTEnd,pTh)
        {
        ...
        void RK4_EM3::initialGridConverge()
        {
            ...
            do
            {
                ...
                {
                    isRefine=em3::isRemeshForce(m_uiMesh,(const double **)m_uiUnzipVar,em3::VAR::U_B0,em3::EM3_CHI_REFINE_VAL,em3::EM3_CHI_COARSEN_VAL,true);
                    //isRefine=m_uiMesh->isReMeshUnzip((const double **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,em3::EM3_DENDRO_AMR_FAC);
                }
                    

                if(isRefine)
                {
                    ot::Mesh* newMesh=m_uiMesh->ReMesh(em3::EM3_DENDRO_GRAIN_SZ,em3::EM3_LOAD_IMB_TOL,em3::EM3_SPLIT_FIX);
                ...
                for(unsigned int index=0; index<em3::EM3_CONSTRAINT_NUM_VARS; index++)
                {
                    ...
                    if(m_uiMesh->isActive())
                    {
                        double l_min=vecMin(m_uiPrevVar[em3::VAR::U_B0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        double l_max=vecMax(m_uiPrevVar[em3::VAR::U_B0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
        ...
        void RK4_EM3::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn, unsigned int numEvolVars,unsigned int numConstVars,const unsigned int * evolVarIndices, const unsigned int * constVarIndices, bool zslice)
        {
            if(!m_uiMesh->isActive())
                return;
                    ...
                    for(unsigned int j=0;j<(eleOrder+1);j++ )
                        for(unsigned int i=0;i<(eleOrder+1);i++)
                        {
                            ...
                            if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                            {
                                ...
                                em3::analyticalSol((double)x,(double)y,(double)z,m_uiCurrentTime,var);
                                for(unsigned int v=0; v < em3::EM3_NUM_VARS; v++)
            ...
            for(unsigned int v=0; v < em3::EM3_NUM_VARS; v++)
            {
                ...
                if(!m_uiMesh->getMPIRank()){
                    std::cout<<" var : "<<em3::EM3_VAR_NAMES[v]<<" (min, max, l2): \t"<<l_min<<"\t"<<l_max<<"\t"<<l2_norm<<std::endl;
                }
            ...
            {
                pDataNames.push_back(std::string(em3::EM3_VAR_NAMES[evolVarIndices[i]]));
                pDataNames.push_back("diff" + std::string(em3::EM3_VAR_NAMES[evolVarIndices[i]]));
            ...
            {
                pDataNames.push_back(std::string(em3::EM3_CONSTRAINT_VAR_NAMES[constVarIndices[i]]));
            ...
            char fPrefix[256];
            sprintf(fPrefix,"%s_%d",em3::EM3_VTU_FILE_PREFIX.c_str(),m_uiCurrentStep);
        ...
        void RK4_EM3::unzipVars_async(double ** zipIn , double **uzipOut)
        {
            ...
            for(unsigned int var=0;var<em3::EM3_NUM_VARS;var+=em3::EM3_ASYNC_COMM_K){
        ...
        void RK4_EM3::performSingleIteration()
        {
            ...
            if(m_uiMesh->isActive())
            {
                ...
                for(unsigned int stage=0;stage<(em3::EM3_RK4_STAGES-1);stage++)
                {
                    ...
                    for(unsigned int blk=0;blk<blkList.size();blk++)
                    {
                        ...
                        //std::cout<<" dx: "<<dx<<" ele x : "<<(1u<<(m_uiMaxDepth-m_uiMesh->getAllElements()[blkList[blk].getLocalElementBegin()].getLevel()))/(double)em3::EM3_ELE_ORDER<<" computed blk: "<<blkList[blk].computeGridDx()<<std::endl;
                ...
                #ifdef DEBUG_RK_SOLVER
                    if(!rank)std::cout<<" stage: "<<(em3::EM3_RK4_STAGES-1)<<" begin: "<<std::endl;
                    for(unsigned int index=0;index<em3::EM3_NUM_VARS;index++)
                        ot::test::isUnzipNaN(m_uiMesh,m_uiUnzipVar[index]);
                #endif
                ...
                #ifdef DEBUG_RK_SOLVER
                    if(!rank)std::cout<<" stage: "<<(em3::EM3_RK4_STAGES-1)<<" af rhs UNZIP RHS TEST:"<<std::endl;
                    for(unsigned int index=0;index<em3::EM3_NUM_VARS;index++)
                        ot::test::isUnzipInternalNaN(m_uiMesh,m_uiUnzipVarRHS[index]);
                #endif
        ...
        void RK4_EM3::rkSolve()
        {
            ...
            for(double t=m_uiCurrentTime;t<m_uiTimeEnd;t=t+m_uiT_h)
            {
                ...
                if((m_uiMesh->isActive()) && (m_uiCurrentStep%em3::EM3_CHECKPT_FREQ)==0)
                    storeCheckPoint(em3::EM3_CHKPT_FILE_PREFIX.c_str());
                em3::timer::t_ioCheckPoint.stop();
                // write sol to vtu.
                if((m_uiMesh->isActive()) && (m_uiCurrentStep%em3::EM3_TIME_STEP_OUTPUT_FREQ)==0)
                {

                    l_min=vecMin(m_uiPrevVar[em3::VAR::U_B0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em3::VAR::U_B0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    ...
                    l_min=vecMin(m_uiPrevVar[em3::VAR::U_B1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em3::VAR::U_B1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_B1|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em3::VAR::U_B2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em3::VAR::U_B2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_B2|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em3::VAR::U_E0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em3::VAR::U_E0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_E0|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em3::VAR::U_E1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em3::VAR::U_E1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_E1|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em3::VAR::U_E2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em3::VAR::U_E2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_E2|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    em3::timer::profileInfoIntermediate(em3::EM3_PROFILE_FILE_PREFIX.c_str(),m_uiMesh,m_uiCurrentStep);
                ...
                if((m_uiCurrentStep%em3::EM3_REMESH_TEST_FREQ)==0)
                {
                    ...
                    else 
                    {
                        ...
                        {
                            const bool isRefine1 = m_uiMesh->isReMeshUnzip((const double **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,em3::EM3_DENDRO_AMR_FAC);
                            const bool isRefine2 = isRefine=em3::isRemeshForce(m_uiMesh,(const double **)m_uiUnzipVar,em3::VAR::U_B0,em3::EM3_CHI_REFINE_VAL,em3::EM3_CHI_COARSEN_VAL,false);
                ...
                if((m_uiCurrentStep%em3::EM3_IO_OUTPUT_FREQ)==0)
                {
                    ...
                    if(m_uiMesh->isActive())
                    {
                        ...
                        l_min=vecMin(m_uiConstraintVars[em3::VAR_CONSTRAINT::C_DIVE]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        l_max=vecMax(m_uiConstraintVars[em3::VAR_CONSTRAINT::C_DIVE]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        //ewh added l2_norm calc 2020.5.26 
                        double l2_norm=normL2(m_uiConstraintVars[em3::VAR_CONSTRAINT::C_DIVE]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        l2_norm = l2_norm/sqrt(total_dof);
                        ...
                        l_min=vecMin(m_uiConstraintVars[em3::VAR_CONSTRAINT::C_DIVB]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        l_max=vecMax(m_uiConstraintVars[em3::VAR_CONSTRAINT::C_DIVB]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        l2_norm=normL2(m_uiConstraintVars[em3::VAR_CONSTRAINT::C_DIVB]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        l2_norm = l2_norm/sqrt(total_dof);
                    ...
                    writeToVTU(m_uiPrevVar,m_uiConstraintVars,em3::EM3_NUM_EVOL_VARS_VTU_OUTPUT,em3::EM3_NUM_CONST_VARS_VTU_OUTPUT,em3::EM3_VTU_OUTPUT_EVOL_INDICES,em3::EM3_VTU_OUTPUT_CONST_INDICES,em3::EM3_VTU_Z_SLICE_ONLY);
        ...
        void RK4_EM3::storeCheckPoint(const char * fNamePrefix)
        {
            ...
            if(m_uiMesh->isActive())
            {
                ...
                (m_uiCurrentStep%(2*em3::EM3_CHECKPT_FREQ)==0) ? cpIndex=0 : cpIndex=1; // to support alternate file writing.
                ...
                io::checkpoint::writeVecToFile(fName,m_uiMesh,(const double **)m_uiPrevVar,em3::EM3_NUM_VARS);
                ...
                if(!rank)
                {
                    ...
                    checkPoint["DENDRO_RK45_LOAD_IMB_TOLERANCE"]=em3::EM3_LOAD_IMB_TOL;
            ...
            for(unsigned int cpIndex=0;cpIndex<2;cpIndex++)
            {
                ...
                if(!rank)
                {
                    ...
                    if(restoreStatus==0)
                    {
                        ...
                        em3::EM3_WAVELET_TOL=checkPoint["DENDRO_RK45_WAVELET_TOLERANCE"];
                        em3::EM3_LOAD_IMB_TOL=checkPoint["DENDRO_RK45_LOAD_IMB_TOLERANCE"];
                ...
                if(isActive) {
                    ...
                    restoreStatus=io::checkpoint::readVecFromFile(fName,newMesh,m_uiPrevVar,em3::EM3_NUM_VARS);
