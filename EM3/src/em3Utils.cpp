...
void readParamFile(const char* fName, MPI_Comm comm) {
    ...
    if (!rank) {
        ...
        em3::EM3_VTU_FILE_PREFIX =
            parFile["EM3_VTU_FILE_PREFIX"].get<std::string>();
        em3::EM3_CHKPT_FILE_PREFIX =
            parFile["EM3_CHKPT_FILE_PREFIX"].get<std::string>();
        em3::EM3_PROFILE_FILE_PREFIX =
            parFile["EM3_PROFILE_FILE_PREFIX"].get<std::string>();
        em3::EM3_RESTORE_SOLVER = parFile["EM3_RESTORE_SOLVER"];
        em3::EM3_ENABLE_BLOCK_ADAPTIVITY =
            parFile["EM3_ENABLE_BLOCK_ADAPTIVITY"];
        ...
        if (parFile.find("EM3_REFINEMENT_MODE") != parFile.end())
            em3::EM3_REFINEMENT_MODE = static_cast<em3::RefinementMode>(
                parFile["EM3_REFINEMENT_MODE"]);

        em3::EM3_NUM_REFINE_VARS = parFile["EM3_NUM_REFINE_VARS"];

        for (unsigned int i = 0; i < em3::EM3_NUM_REFINE_VARS; i++)
            em3::EM3_REFINE_VARIABLE_INDICES[i] =
                parFile["EM3_REFINE_VARIABLE_INDICES"][i];

        em3::EM3_NUM_EVOL_VARS_VTU_OUTPUT =
            parFile["EM3_NUM_EVOL_VARS_VTU_OUTPUT"];
        em3::EM3_NUM_CONST_VARS_VTU_OUTPUT =
            parFile["EM3_NUM_CONST_VARS_VTU_OUTPUT"];

        for (unsigned int i = 0; i < em3::EM3_NUM_EVOL_VARS_VTU_OUTPUT; i++)
            em3::EM3_VTU_OUTPUT_EVOL_INDICES[i] =
                parFile["EM3_VTU_OUTPUT_EVOL_INDICES"][i];

        for (unsigned int i = 0; i < em3::EM3_NUM_CONST_VARS_VTU_OUTPUT; i++)
            em3::EM3_VTU_OUTPUT_CONST_INDICES[i] =
                parFile["EM3_VTU_OUTPUT_CONST_INDICES"][i];
    ...
    if (EM3_NUM_REFINE_VARS > EM3_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of refine variables should "
                     "be less than number of EM3_NUM_VARS"
                  << std::endl;
        exit(0);
    }
    if (EM3_NUM_EVOL_VARS_VTU_OUTPUT > EM3_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of evolution VTU variables "
                     "should be less than number of EM3_NUM_VARS"
                  << std::endl;
        exit(0);
    }
    ...
    par::Mpi_Bcast(EM3_VTU_OUTPUT_CONST_INDICES, EM3_CONSTRAINT_NUM_VARS, 0,
                   comm);
}
...
void dumpParamFile(std::ostream& sout, int root, MPI_Comm comm) {
    ...
    if (rank == root) {
        ...
        sout << YLW << "\tEM3_ELE_ORDER :" << em3::EM3_ELE_ORDER << NRM
             << std::endl;
        sout << YLW << "\tEM3_PADDING_WIDTH :" << em3::EM3_PADDING_WIDTH << NRM
             << std::endl;
        sout << YLW << "\tEM3_DIM :" << em3::EM3_DIM << NRM << std::endl;
        sout << YLW << "\tEM3_IO_OUTPUT_FREQ :" << em3::EM3_IO_OUTPUT_FREQ
             << NRM << std::endl;
        sout << YLW << "\tEM3_REMESH_TEST_FREQ :" << em3::EM3_REMESH_TEST_FREQ
             << NRM << std::endl;
        sout << YLW << "\tEM3_CHECKPT_FREQ :" << em3::EM3_CHECKPT_FREQ << NRM
             << std::endl;
        sout << YLW << "\tEM3_RESTORE_SOLVER :" << em3::EM3_RESTORE_SOLVER
             << NRM << std::endl;
        sout << YLW << "\tEM3_ENABLE_BLOCK_ADAPTIVITY :"
             << em3::EM3_ENABLE_BLOCK_ADAPTIVITY << NRM << std::endl;
        sout << YLW << "\tEM3_VTU_FILE_PREFIX :" << em3::EM3_VTU_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW << "\tEM3_CHKPT_FILE_PREFIX :" << em3::EM3_CHKPT_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW
             << "\tEM3_PROFILE_FILE_PREFIX :" << em3::EM3_PROFILE_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW << "\tEM3_VTU_Z_SLICE_ONLY :" << em3::EM3_VTU_Z_SLICE_ONLY
             << NRM << std::endl;
        sout << YLW << "\tEM3_IO_OUTPUT_GAP :" << em3::EM3_IO_OUTPUT_GAP << NRM
             << std::endl;
        sout << YLW << "\tEM3_DENDRO_GRAIN_SZ :" << em3::EM3_DENDRO_GRAIN_SZ
             << NRM << std::endl;
        sout << YLW << "\tEM3_ASYNC_COMM_K :" << em3::EM3_ASYNC_COMM_K << NRM
             << std::endl;
        sout << YLW << "\tEM3_DENDRO_AMR_FAC :" << em3::EM3_DENDRO_AMR_FAC
             << NRM << std::endl;
        sout << YLW << "\tEM3_CFL_FACTOR:" << em3::EM3_CFL_FACTOR << NRM
             << std::endl;
        sout << YLW << "\tEM3_WAVELET_TOL :" << em3::EM3_WAVELET_TOL << NRM
             << std::endl;
        sout << YLW << "\tEM3_LOAD_IMB_TOL :" << em3::EM3_LOAD_IMB_TOL << NRM
             << std::endl;
        sout << YLW << "\tEM3_RK45_TIME_BEGIN :" << em3::EM3_RK45_TIME_BEGIN
             << NRM << std::endl;
        sout << YLW << "\tEM3_RK45_TIME_END :" << em3::EM3_RK45_TIME_END << NRM
             << std::endl;
        sout << YLW
             << "\tEM3_RK45_TIME_STEP_SIZE :" << em3::EM3_RK45_TIME_STEP_SIZE
             << NRM << std::endl;
        sout << YLW << "\tEM3_RK45_DESIRED_TOL :" << em3::EM3_RK45_DESIRED_TOL
             << NRM << std::endl;
        sout << YLW << "\tEM3_COMPD_MIN : ( :" << em3::EM3_COMPD_MIN[0] << " ,"
             << em3::EM3_COMPD_MIN[1] << "," << em3::EM3_COMPD_MIN[2] << " )"
             << NRM << std::endl;
        sout << YLW << "\tEM3_COMPD_MAX : ( :" << em3::EM3_COMPD_MAX[0] << " ,"
             << em3::EM3_COMPD_MAX[1] << "," << em3::EM3_COMPD_MAX[2] << " )"
             << NRM << std::endl;
        sout << YLW << "\tEM3_BLK_MIN : ( :" << em3::EM3_BLK_MIN_X << " ,"
             << em3::EM3_BLK_MIN_Y << "," << em3::EM3_BLK_MIN_Z << " )" << NRM
             << std::endl;
        sout << YLW << "\tEM3_BLK_MAX : ( :" << em3::EM3_BLK_MAX_X << " ,"
             << em3::EM3_BLK_MAX_Y << "," << em3::EM3_BLK_MAX_Z << " )" << NRM
             << std::endl;
        sout << YLW << "\tEM3_OCTREE_MIN : ( :" << em3::EM3_OCTREE_MIN[0]
             << " ," << em3::EM3_OCTREE_MIN[1] << "," << em3::EM3_OCTREE_MIN[2]
             << " )" << NRM << std::endl;
        sout << YLW << "\tEM3_OCTREE_MAX : ( :" << em3::EM3_OCTREE_MAX[0]
             << " ," << em3::EM3_OCTREE_MAX[1] << "," << em3::EM3_OCTREE_MAX[2]
             << " )" << NRM << std::endl;
        sout << YLW << "\tKO_DISS_SIGMA :" << em3::KO_DISS_SIGMA << NRM
             << std::endl;
        sout << YLW << "\tEM3_ID_TYPE:" << em3::EM3_ID_TYPE << NRM << std::endl;
        sout << YLW << "\tEM3_ID_AMP1:" << em3::EM3_ID_AMP1 << NRM << std::endl;
        sout << YLW << "\tEM3_ID_LAMBDA1:" << em3::EM3_ID_LAMBDA1 << NRM
             << std::endl;
        ...
        sout << YLW << "\tEM3_MAXDEPTH :" << em3::EM3_MAXDEPTH << NRM
             << std::endl;

        sout << YLW << "\tEM3_NUM_REFINE_VARS :" << em3::EM3_NUM_REFINE_VARS
             << NRM << std::endl;
        ...
        sout << em3::EM3_REFINE_VARIABLE_INDICES[em3::EM3_NUM_REFINE_VARS - 1]
             << "]" << NRM << std::endl;

        sout << YLW << "\tEM3_REFINEMENT_MODE :" << em3::EM3_REFINEMENT_MODE
             << NRM << std::endl;

        sout << YLW << "\tEM3_NUM_EVOL_VARS_VTU_OUTPUT :"
