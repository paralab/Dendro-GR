...
int main (int argc, char** argv)
{
    ...
    ot::Mesh * mesh = ot::createMesh(tmpNodes.data(),tmpNodes.size(),em1::EM1_ELE_ORDER,comm,1,ot::SM_TYPE::FDM,em1::EM1_DENDRO_GRAIN_SZ,em1::EM1_LOAD_IMB_TOL,em1::EM1_SPLIT_FIX);

    if(!em1::EM1_ENABLE_BLOCK_ADAPTIVITY)
        ot::meshWAMRConvergence(mesh,f_init,em1::EM1_WAVELET_TOL,em1::EM1_NUM_VARS,em1::EM1_ELE_ORDER,em1::EM1_REFINE_VARIABLE_INDICES,em1::EM1_NUM_REFINE_VARS,5);
    ...
    { 
        ...
        ts::ExplicitNUTS<DendroScalar,em1::EM1Ctx>*  enuts = new ts::ExplicitNUTS<DendroScalar,em1::EM1Ctx>(appCtx);
        ...
        for(enuts->init(); enuts->curr_time() < em1::EM1_RK45_TIME_END ; enuts->evolve_with_remesh(pt_remesh_freq))
        {
            ...
            if(isRemesh)
            {
                ...
                appCtx->remesh_and_gridtransfer(em1::EM1_DENDRO_GRAIN_SZ, em1::EM1_LOAD_IMB_TOL,em1::EM1_SPLIT_FIX,true,false,false);
                //appCtx->terminal_output();
            }
                    
            enuts->sync_with_mesh();
            
            if((step % em1::EM1_IO_OUTPUT_FREQ) == 0 )
             appCtx -> write_vtu();   

            if( (step % em1::EM1_CHECKPT_FREQ) == 0 )
            ...
            //enuts->dump_pt(std::cout);
              ...
              sprintf(fName,"%s_enuts.prof",em1::EM1_PROFILE_FILE_PREFIX.c_str());
              if(!rank) 
              {
    ...
    }else if(TS_MODE==1)
    {
        // GTS code here. 
        em1::EM1Ctx *  appCtx = new em1::EM1Ctx(mesh);
        ts::ETS<DendroScalar,em1::EM1Ctx>* ets = new ts::ETS<DendroScalar,em1::EM1Ctx>(appCtx);
        ets->set_evolve_vars(appCtx->get_evolution_vars());
        ets->set_ets_coefficients(tsType);
        
        
        for(ets->init(); ets->curr_time() < em1::EM1_RK45_TIME_END ; ets->evolve())
        {
