...
EM1Ctx::EM1Ctx(ot::Mesh* pMesh) : Ctx() {
    ...
    m_uiCVar = this->create_vec(ts::CTXVType::CONSTRAINT, true, false, false,
                                EM1_CONSTRAINT_NUM_VARS);
    m_uiPVar =
        this->create_vec(ts::CTXVType::PRIMITIVE, true, false, false,
                         EM1_NUM_VARS);  // used as to compare the analytical
...
int EM1Ctx::rhs(DVec* in, DVec* out, unsigned int sz, DendroScalar time) {
    ...
    for (unsigned int blk = 0; blk < numBlocks; blk++) {
        ...
        em1rhs(unzipOut, (const DendroScalar**)unzipIn, offset, ptmin, ptmax,
               lsz, bflag);
...
int EM1Ctx::rhs_blk(const DendroScalar* in, DendroScalar* out, unsigned int dof,
    ...
    em1rhs(unzipOut, (const DendroScalar**)unzipIn, 0, ptmin, ptmax, lsz,
           bflag);
...
int EM1Ctx::write_vtu() {
    ...
    for (unsigned int i = 0; i < numConstVars; i++) {
        pDataNames.push_back(std::string(
            EM1_CONSTRAINT_VAR_NAMES[EM1_VTU_OUTPUT_CONST_INDICES[i]]));
        pData[i] = consVar[EM1_VTU_OUTPUT_CONST_INDICES[i]];
    }

    for (unsigned int i = 0; i < numEvolVars; i++) {
        pDataNames.push_back(
            std::string(EM1_VAR_NAMES[EM1_VTU_OUTPUT_EVOL_INDICES[i]]));
    ...
    for (unsigned int i = 0; i < numPrimVars; i++) {
        ...
        pData[numConstVars + numEvolVars + i] =
            primVar[EM1_VTU_OUTPUT_EVOL_INDICES[i]];
    }
...
int EM1Ctx::write_checkpt() {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiTinfo._m_uiStep % (2 * EM1_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
        ...
        sprintf(fName, "%s_octree_%d_%d.oct", EM1_CHKPT_FILE_PREFIX.c_str(),
                cpIndex, rank);
        ...
        sprintf(fName, "%s_%d_%d.var", EM1_CHKPT_FILE_PREFIX.c_str(), cpIndex,
                rank);
        io::checkpoint::writeVecToFile(fName, m_uiMesh, (const double**)eVar,
                                       em1::EM1_NUM_VARS);

        if (!rank) {
            sprintf(fName, "%s_step_%d.cp", EM1_CHKPT_FILE_PREFIX.c_str(),
                    cpIndex);
...
int EM1Ctx::restore_checkpt() {
    ...
    if (isActive) {
        ...
        sprintf(fName, "%s_%d_%d.var", EM1_CHKPT_FILE_PREFIX.c_str(),
                restoreFileIndex, activeRank);
...
int EM1Ctx::pre_timestep(DVec sIn) { return 0; }

int EM1Ctx::pre_stage(DVec sIn) { return 0; }

int EM1Ctx::post_stage(DVec sIn) { return 0; }

int EM1Ctx::post_timestep(DVec sIn) { return 0; }
...
bool EM1Ctx::is_remesh() {
    ...
    if (EM1_ENABLE_BLOCK_ADAPTIVITY) return isRefine;
    ...
    isRefine = m_uiMesh->isReMeshUnzip((const double**)unzipVar, refineVarIds,
                                       EM1_NUM_REFINE_VARS, waveletTolFunc,
                                       EM1_DENDRO_AMR_FAC);
...
DVec EM1Ctx::get_evolution_vars() { return m_uiEVar; }

DVec EM1Ctx::get_constraint_vars() { return m_uiCVar; }

DVec EM1Ctx::get_primitive_vars() { return m_uiPVar; }
...
int EM1Ctx::compute_constraints() {
    ...
    if (!m_uiMesh->getMPIRank()) {
        ...
        for (unsigned int v = 0; v < EM1_CONSTRAINT_NUM_VARS; v++)
            std::cout << "\t ||" << EM1_CONSTRAINT_VAR_NAMES[v]
    ...
    if (!m_uiMesh->getMPIRank()) {
        ...
        if (m_uiTinfo._m_uiStep == 0) {
            ...
            for (unsigned int v = 0; v < EM1_CONSTRAINT_NUM_VARS; v++)
                fileGW << EM1_CONSTRAINT_VAR_NAMES[v] << "_l2\t"
                       << EM1_CONSTRAINT_VAR_NAMES[v] << "_l2rs\t";
    ...
    if (m_uiMesh->isActive()) {
        ...
        if ((m_uiMesh->isActive()) &&
            ...
            if (!m_uiMesh->getMPIRank()) {
                ...
                for (unsigned int v = 0; v < EM1_NUM_VARS; v++)
                    std::cout << "\t ||diff_[" << EM1_VAR_NAMES[v]
                              << "]|| (min, max, l2,l2rs) : (" << vmin[v]
                              << ", " << vmax[v] << ", " << l2_norm[v] << ", "
                              << l2_rs[v] << " ) " << std::endl;
            }
...
unsigned int EM1Ctx::getBlkTimestepFac(unsigned int blev, unsigned int lmin,
                                       unsigned int lmax) {
