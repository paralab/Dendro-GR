...
void readParamFile(const char* fName, MPI_Comm comm) {
    ...
    if (!rank) {
        ...
        em1::EM1_VTU_FILE_PREFIX =
            parFile["EM1_VTU_FILE_PREFIX"].get<std::string>();
        em1::EM1_CHKPT_FILE_PREFIX =
            parFile["EM1_CHKPT_FILE_PREFIX"].get<std::string>();
        em1::EM1_PROFILE_FILE_PREFIX =
            parFile["EM1_PROFILE_FILE_PREFIX"].get<std::string>();
        em1::EM1_RESTORE_SOLVER = parFile["EM1_RESTORE_SOLVER"];
        em1::EM1_ENABLE_BLOCK_ADAPTIVITY =
            parFile["EM1_ENABLE_BLOCK_ADAPTIVITY"];
        ...
        if (parFile.find("EM1_REFINEMENT_MODE") != parFile.end())
            em1::EM1_REFINEMENT_MODE = static_cast<em1::RefinementMode>(
                parFile["EM1_REFINEMENT_MODE"]);

        em1::EM1_NUM_REFINE_VARS = parFile["EM1_NUM_REFINE_VARS"];

        for (unsigned int i = 0; i < em1::EM1_NUM_REFINE_VARS; i++)
            em1::EM1_REFINE_VARIABLE_INDICES[i] =
                parFile["EM1_REFINE_VARIABLE_INDICES"][i];

        em1::EM1_NUM_EVOL_VARS_VTU_OUTPUT =
            parFile["EM1_NUM_EVOL_VARS_VTU_OUTPUT"];
        em1::EM1_NUM_CONST_VARS_VTU_OUTPUT =
            parFile["EM1_NUM_CONST_VARS_VTU_OUTPUT"];

        for (unsigned int i = 0; i < em1::EM1_NUM_EVOL_VARS_VTU_OUTPUT; i++)
            em1::EM1_VTU_OUTPUT_EVOL_INDICES[i] =
                parFile["EM1_VTU_OUTPUT_EVOL_INDICES"][i];

        for (unsigned int i = 0; i < em1::EM1_NUM_CONST_VARS_VTU_OUTPUT; i++)
            em1::EM1_VTU_OUTPUT_CONST_INDICES[i] =
                parFile["EM1_VTU_OUTPUT_CONST_INDICES"][i];
    ...
    if (EM1_NUM_REFINE_VARS > EM1_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of refine variables should "
                     "be less than number of EM1_NUM_VARS"
                  << std::endl;
        exit(0);
    }
    if (EM1_NUM_EVOL_VARS_VTU_OUTPUT > EM1_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of evolution VTU variables "
                     "should be less than number of EM1_NUM_VARS"
                  << std::endl;
        exit(0);
    }
    ...
    par::Mpi_Bcast(EM1_VTU_OUTPUT_CONST_INDICES, EM1_CONSTRAINT_NUM_VARS, 0,
                   comm);
}
...
void dumpParamFile(std::ostream& sout, int root, MPI_Comm comm) {
    ...
    if (rank == root) {
        ...
        sout << YLW << "\tEM1_ELE_ORDER :" << em1::EM1_ELE_ORDER << NRM
             << std::endl;
        sout << YLW << "\tEM1_PADDING_WIDTH :" << em1::EM1_PADDING_WIDTH << NRM
             << std::endl;
        sout << YLW << "\tEM1_DIM :" << em1::EM1_DIM << NRM << std::endl;
        sout << YLW << "\tEM1_IO_OUTPUT_FREQ :" << em1::EM1_IO_OUTPUT_FREQ
             << NRM << std::endl;
        sout << YLW << "\tEM1_REMESH_TEST_FREQ :" << em1::EM1_REMESH_TEST_FREQ
             << NRM << std::endl;
        sout << YLW << "\tEM1_CHECKPT_FREQ :" << em1::EM1_CHECKPT_FREQ << NRM
             << std::endl;
        sout << YLW << "\tEM1_RESTORE_SOLVER :" << em1::EM1_RESTORE_SOLVER
             << NRM << std::endl;
        sout << YLW << "\tEM1_ENABLE_BLOCK_ADAPTIVITY :"
             << em1::EM1_ENABLE_BLOCK_ADAPTIVITY << NRM << std::endl;
        sout << YLW << "\tEM1_VTU_FILE_PREFIX :" << em1::EM1_VTU_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW << "\tEM1_CHKPT_FILE_PREFIX :" << em1::EM1_CHKPT_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW
             << "\tEM1_PROFILE_FILE_PREFIX :" << em1::EM1_PROFILE_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW << "\tEM1_VTU_Z_SLICE_ONLY :" << em1::EM1_VTU_Z_SLICE_ONLY
             << NRM << std::endl;
        sout << YLW << "\tEM1_IO_OUTPUT_GAP :" << em1::EM1_IO_OUTPUT_GAP << NRM
             << std::endl;
        sout << YLW << "\tEM1_DENDRO_GRAIN_SZ :" << em1::EM1_DENDRO_GRAIN_SZ
             << NRM << std::endl;
        sout << YLW << "\tEM1_ASYNC_COMM_K :" << em1::EM1_ASYNC_COMM_K << NRM
             << std::endl;
        sout << YLW << "\tEM1_DENDRO_AMR_FAC :" << em1::EM1_DENDRO_AMR_FAC
             << NRM << std::endl;
        sout << YLW << "\tEM1_CFL_FACTOR:" << em1::EM1_CFL_FACTOR << NRM
             << std::endl;
        sout << YLW << "\tEM1_WAVELET_TOL :" << em1::EM1_WAVELET_TOL << NRM
             << std::endl;
        sout << YLW << "\tEM1_LOAD_IMB_TOL :" << em1::EM1_LOAD_IMB_TOL << NRM
             << std::endl;
        sout << YLW << "\tEM1_RK45_TIME_BEGIN :" << em1::EM1_RK45_TIME_BEGIN
             << NRM << std::endl;
        sout << YLW << "\tEM1_RK45_TIME_END :" << em1::EM1_RK45_TIME_END << NRM
             << std::endl;
        sout << YLW
             << "\tEM1_RK45_TIME_STEP_SIZE :" << em1::EM1_RK45_TIME_STEP_SIZE
             << NRM << std::endl;
        sout << YLW << "\tEM1_RK45_DESIRED_TOL :" << em1::EM1_RK45_DESIRED_TOL
             << NRM << std::endl;
        sout << YLW << "\tEM1_COMPD_MIN : ( :" << em1::EM1_COMPD_MIN[0] << " ,"
             << em1::EM1_COMPD_MIN[1] << "," << em1::EM1_COMPD_MIN[2] << " )"
             << NRM << std::endl;
        sout << YLW << "\tEM1_COMPD_MAX : ( :" << em1::EM1_COMPD_MAX[0] << " ,"
             << em1::EM1_COMPD_MAX[1] << "," << em1::EM1_COMPD_MAX[2] << " )"
             << NRM << std::endl;
        sout << YLW << "\tEM1_BLK_MIN : ( :" << em1::EM1_BLK_MIN_X << " ,"
             << em1::EM1_BLK_MIN_Y << "," << em1::EM1_BLK_MIN_Z << " )" << NRM
             << std::endl;
        sout << YLW << "\tEM1_BLK_MAX : ( :" << em1::EM1_BLK_MAX_X << " ,"
             << em1::EM1_BLK_MAX_Y << "," << em1::EM1_BLK_MAX_Z << " )" << NRM
             << std::endl;
        sout << YLW << "\tEM1_OCTREE_MIN : ( :" << em1::EM1_OCTREE_MIN[0]
             << " ," << em1::EM1_OCTREE_MIN[1] << "," << em1::EM1_OCTREE_MIN[2]
             << " )" << NRM << std::endl;
        sout << YLW << "\tEM1_OCTREE_MAX : ( :" << em1::EM1_OCTREE_MAX[0]
             << " ," << em1::EM1_OCTREE_MAX[1] << "," << em1::EM1_OCTREE_MAX[2]
             << " )" << NRM << std::endl;
        sout << YLW << "\tKO_DISS_SIGMA :" << em1::KO_DISS_SIGMA << NRM
             << std::endl;
        sout << YLW << "\tEM1_ID_TYPE:" << em1::EM1_ID_TYPE << NRM << std::endl;
        sout << YLW << "\tEM1_ID_AMP1:" << em1::EM1_ID_AMP1 << NRM << std::endl;
        sout << YLW << "\tEM1_ID_LAMBDA1:" << em1::EM1_ID_LAMBDA1 << NRM
             << std::endl;
        sout << YLW << "\tEM1_ID_AMP2:" << em1::EM1_ID_AMP2 << NRM << std::endl;

        sout << YLW << "\tEM1_MAXDEPTH :" << em1::EM1_MAXDEPTH << NRM
             << std::endl;
        sout << YLW << "\tEM1_NUM_REFINE_VARS :" << em1::EM1_NUM_REFINE_VARS
             << NRM << std::endl;
        ...
        sout << em1::EM1_REFINE_VARIABLE_INDICES[em1::EM1_NUM_REFINE_VARS - 1]
             << "]" << NRM << std::endl;

        sout << YLW << "\tEM1_REFINEMENT_MODE :" << em1::EM1_REFINEMENT_MODE
             << NRM << std::endl;
        sout << YLW << "\tEM1_NUM_EVOL_VARS_VTU_OUTPUT :"
        ...
        sout << em1::EM1_VTU_OUTPUT_EVOL_INDICES
