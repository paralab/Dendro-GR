...
RK3_BSSN::RK3_BSSN(ot::Mesh* pMesh, double pTBegin, double pTEnd, double pTh)
...
void RK3_BSSN::writeToVTU(double** evolZipVarIn, double** constrZipVarIn,
...
void RK3_BSSN::intergridTransferVars(double**& zipIn,
...
void RK3_BSSN::performSingleIteration() {
    ...
    do {
        ...
        if (m_uiMesh->isActive()) {
            ...
            for (unsigned int stage = 0; stage < (bssn::BSSN_RK3_STAGES - 1);
                 stage++) {
                ...
                for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd;
                    ...
                    for (unsigned int index = 0; index < bssn::BSSN_NUM_VARS;
                        ...
                        for (unsigned int s = 0; s < (stage + 1); s++) {
                            ...
                            // "<<RK3_U[stage+1][s]<<std::endl;
                            m_uiVarIm[index][node] +=
                                (RK3_U[stage + 1][s] * m_uiT_h *
            ...
            current_t_adv =
                current_t + RK3_T[(bssn::BSSN_RK3_STAGES - 1)] * m_uiT_h;

#ifdef DEBUG_RK_SOLVER
            if (!rank)
                std::cout << " stage: " << (bssn::BSSN_RK3_STAGES - 1)
                          << " begin: " << std::endl;
            ...
            for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd;
                ...
                for (unsigned int index = 0; index < bssn::BSSN_NUM_VARS;
                    ...
                    for (unsigned int s = 0; s < (bssn::BSSN_RK3_STAGES); s++) {
                        m_uiVar[index][node] +=
                            (RK3_C[s] * m_uiT_h * m_uiStage[s][index][node]);
                    }
                }
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiCurrentStep % bssn::BSSN_REMESH_TEST_FREQ) == 0) {
            ...
            if (isRefine) {
                ...
                for (unsigned int stage = 0; stage < bssn::BSSN_RK3_STAGES;
                     stage++)
                    for (unsigned int index = 0; index < bssn::BSSN_NUM_VARS;
                         index++) {
                        delete[] m_uiStage[stage][index];
