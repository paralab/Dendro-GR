 ...
 *@brief Runge-Kutta 45 (RK45) method implementation for BSSN formulation of GR
 *equations.
 */
//
...
RK45_BSSN::RK45_BSSN(ot::Mesh* pMesh, double pTBegin, double pTEnd, double pTh)
    ...
    for (unsigned int index = 0; index < bssn::BSSN_CONSTRAINT_NUM_VARS;
        ...
        m_uiUnzipConstraintVars[index] =
            m_uiMesh->createUnZippedVector<double>();
...
void RK45_BSSN::initialGridConverge() {
    ...
    do {
        ...
        if (isRefine) {
            ...
            if (m_uiMesh->isActive()) {
                double l_min = vecMin(m_uiPrevVar[bssn::VAR::U_ALPHA] +
                                          m_uiMesh->getNodeLocalBegin(),
                                      (m_uiMesh->getNumLocalMeshNodes()),
                                      m_uiMesh->getMPICommunicator());
                double l_max = vecMax(m_uiPrevVar[bssn::VAR::U_ALPHA] +
                                          m_uiMesh->getNodeLocalBegin(),
                                      (m_uiMesh->getNumLocalMeshNodes()),
                                      m_uiMesh->getMPICommunicator());
...
void RK45_BSSN::writeToVTU(double** evolZipVarIn, double** constrZipVarIn,
...
void RK45_BSSN::intergridTransferVars(double**& zipIn,
...
void RK45_BSSN::performSingleIteration() {
    ...
    do {
        ...
        if (m_uiMesh->isActive()) {
            ...
            for (unsigned int stage = 0; stage < (bssn::BSSN_RK45_STAGES - 1);
                 stage++) {
                ...
                for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                    ...
                    bssnrhs_sep(m_uiUnzipVarRHS, (const double**)m_uiUnzipVar,
                                offset, ptmin, ptmax, sz, bflag);
#else
                    bssnrhs(m_uiUnzipVarRHS, (const double**)m_uiUnzipVar,
                            offset, ptmin, ptmax, sz, bflag);
#endif
                }
#endif
            ...
            for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                ...
                bssnrhs_sep(m_uiUnzipVarRHS, (const double**)m_uiUnzipVar,
                            offset, ptmin, ptmax, sz, bflag);
#else
                bssnrhs(m_uiUnzipVarRHS, (const double**)m_uiUnzipVar, offset,
            ...
            for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd;
                ...
                for (unsigned int index = 0; index < bssn::BSSN_NUM_VARS;
                     index++) {
                    m_uiVarIm[index][node] = m_uiPrevVar[index][node];
                    for (unsigned int s = 0; s < (bssn::BSSN_RK45_STAGES - 1);
                         s++) {
                        if (s == 1) continue;
                        m_uiVarIm[index][node] +=
            ...
            // update the m_uiTh bases on the normed diff between m_uiVarIm,
            // m_uiVar.
        ...
        if (n_inf_max > bssn::BSSN_RK45_DESIRED_TOL) {
            ...
            m_uiT_h =
                ...
                (pow(fabs(bssn::BSSN_RK45_DESIRED_TOL / n_inf_max), 0.25));
            if (!m_uiMesh->getMPIRankGlobal())
            ...
            m_uiT_h =
                bssn::BSSN_SAFETY_FAC * m_uiT_h *
                (pow(fabs(bssn::BSSN_RK45_DESIRED_TOL / n_inf_max), 0.20));
        }
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiCurrentStep % bssn::BSSN_REMESH_TEST_FREQ) == 0) {
            ...
            if (isRefine) {
                ...
                for (unsigned int stage = 0; stage < bssn::BSSN_RK45_STAGES;
                     stage++)
                    for (unsigned int index = 0; index < bssn::BSSN_NUM_VARS;
                         index++) {
                        delete[] m_uiStage[stage][index];
        ...
        if ((m_uiCurrentStep % bssn::BSSN_IO_OUTPUT_FREQ) == 0) {
            ...
            bssn::extractConstraints(
                m_uiMesh, (const double**)m_uiConstraintVars, m_uiCurrentStep);
            bssn::extractGravitationlWaves(m_uiMesh,
                                           (const double**)m_uiConstraintVars,
                                           50, m_uiCurrentStep);
