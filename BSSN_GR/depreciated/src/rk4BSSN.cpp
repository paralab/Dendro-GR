...
RK4_BSSN::RK4_BSSN(ot::Mesh* pMesh, double pTBegin, double pTEnd, double pTh)
...
void RK4_BSSN::writeToVTU(double** evolZipVarIn, double** constrZipVarIn,
    ...
    std::function<void(double, double, double, double*)> u_x_t =
        [](double x, double y, double z, double* var) {
            bssn::KerrSchildData(x, y, z, var);
        };
                ...
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    ...
                    if (nodeLookUp_CG >= nodeLocalBegin &&
                        ...
                        diffVec[nodeLookUp_CG] =
                            var[bssn::VAR::U_CHI] -
                            evolZipVarIn[bssn::VAR::U_CHI][nodeLookUp_CG];
    ...
    if (!m_uiMesh->getMPIRank()) {
        ...
        std::cout << YLW << "\t ||VAR::DIFF|| (min, max,l2) : (" << l_min
                  << ", " << l_max << ", " << l2_norm << " ) " << NRM
                  << std::endl;
    }
...
void RK4_BSSN::intergridTransferVars(double**& zipIn,
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiCurrentStep % bssn::BSSN_REMESH_TEST_FREQ) == 0) {
            ...
            if (isRefine) {
                ...
                for (unsigned int stage = 0; stage < bssn::BSSN_RK4_STAGES;
                     stage++)
                    for (unsigned int index = 0; index < bssn::BSSN_NUM_VARS;
                         index++) {
                        delete[] m_uiStage[stage][index];
