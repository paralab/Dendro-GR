...
namespace bssn
{
    ...
    void readParamFile(const char * fName,MPI_Comm comm)
    {
        ...
        if(!infile) {std::cout<<fName<<" parameter file open failed by "<<rank<<"/"<<npes<<std::endl;}
        infile>>parFile;
        ...
        TPID::par_S_plus[0]    = bssn::BH1.getBHSpin() * sin(bssn::BH1.getBHSpinTheta()) * cos(bssn::BH1.getBHSpinPhi()) ; //parFile["TPID_PAR_S_PLUS"]["X"];
        TPID::par_S_plus[1]    = bssn::BH1.getBHSpin() * sin(bssn::BH1.getBHSpinTheta()) * sin(bssn::BH1.getBHSpinPhi()) ; //parFile["TPID_PAR_S_PLUS"]["Y"];
        TPID::par_S_plus[2]    = bssn::BH1.getBHSpin() * cos(bssn::BH1.getBHSpinTheta()) ;                                 //parFile["TPID_PAR_S_PLUS"]["Z"];

        TPID::par_S_minus[0]   = bssn::BH2.getBHSpin() * sin(bssn::BH2.getBHSpinTheta()) * cos(bssn::BH2.getBHSpinPhi()) ; //parFile["TPID_PAR_S_MINUS"]["X"];
        TPID::par_S_minus[1]   = bssn::BH2.getBHSpin() * sin(bssn::BH2.getBHSpinTheta()) * sin(bssn::BH2.getBHSpinPhi()) ; //parFile["TPID_PAR_S_MINUS"]["Y"];
        TPID::par_S_minus[2]   = bssn::BH2.getBHSpin() * cos(bssn::BH2.getBHSpinTheta()) ;                                 //parFile["TPID_PAR_S_MINUS"]["Z"];
    ...
    void dumpParamFile(std::ostream& sout, int root, MPI_Comm comm)
    {
        ...
        if(rank==root)
        {
            ...
            sout<<YLW<<"\tRIT_ETA_FUNCTION: "<<bssn::RIT_ETA_FUNCTION<<NRM<<std::endl;
            sout<<YLW<<"\tRIT_ETA_CENTRAL: "<<bssn::RIT_ETA_CENTRAL<<NRM<<std::endl;
            ...
            sout<<YLW<<"\tAEH_SOLVER_FREQ: "<<AEH::AEH_SOLVER_FREQ<<std::endl;
