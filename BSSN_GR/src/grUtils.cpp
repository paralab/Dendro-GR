...
namespace bssn
{
    ...
    void readParamJSONFile(const char * fName,MPI_Comm comm)
    {
        ...
        TPID::par_S_plus[2] =
            bssn::BH1.getBHSpin() *
            cos(bssn::BH1
                    .getBHSpinTheta());  // parFile["TPID_PAR_S_PLUS"]["Z"];
        ...
        TPID::par_S_minus[2] =
            bssn::BH2.getBHSpin() *
            cos(bssn::BH2
                    .getBHSpinTheta());  // parFile["TPID_PAR_S_MINUS"]["Z"];
    ...
    void dumpParamFile(std::ostream& sout, int root, MPI_Comm comm)
    {
        ...
        if (rank == root) {
            ...
            sout << YLW
                 ...
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_CHECKPT_FREQ :" << bssn::BSSN_CHECKPT_FREQ
                 << NRM << std::endl;
            sout << YLW
            ...
            sout << YLW
                 ...
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_PROFILE_FILE_PREFIX :"
            ...
            sout << YLW
                 ...
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_ASYNC_COMM_K :" << bssn::BSSN_ASYNC_COMM_K
                 << NRM << std::endl;
            sout << YLW
            ...
            sout << YLW << "\tBSSN_WAVELET_TOL :" << bssn::BSSN_WAVELET_TOL
                 << NRM << std::endl;
            ...
            sout << YLW << "\tBSSN_LOAD_IMB_TOL :" << bssn::BSSN_LOAD_IMB_TOL
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_RK_TIME_BEGIN :" << bssn::BSSN_RK_TIME_BEGIN
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_RK_TIME_END :" << bssn::BSSN_RK_TIME_END
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_RK_TYPE :" << bssn::BSSN_RK_TYPE << NRM
                 << std::endl;
            sout << YLW << "\tBSSN_RK45_TIME_STEP_SIZE :"
            ...
            sout << YLW << "\tETA_CONST :" << bssn::ETA_CONST << NRM
                 << std::endl;
            ...
            sout << YLW << "\tRIT_ETA_FUNCTION: " << bssn::RIT_ETA_FUNCTION
                 << NRM << std::endl;
            ...
            sout << YLW << "\tBSSN_XI : (" << bssn::BSSN_XI[0] << " ,"
                 ...
                 << std::endl;
            sout << YLW << "\tCHI_FLOOR :" << bssn::CHI_FLOOR << NRM
                 << std::endl;
            sout << YLW << "\tBSSN_TRK0 :" << bssn::BSSN_TRK0 << NRM
                 << std::endl;
            sout << YLW << "\tDISSIPATION_TYPE :" << bssn::DISSIPATION_TYPE
                 << NRM << std::endl;
            ...
            sout << bssn::BSSN_REFINE_VARIABLE_INDICES
                        [bssn::BSSN_NUM_REFINE_VARS - 1]
                 << "]" << NRM << std::endl;

            sout << YLW
                 << "\tBSSN_REFINEMENT_MODE :" << bssn::BSSN_REFINEMENT_MODE
                 << NRM << std::endl;
            ...
            sout << YLW << "\tBSSN_BH2_AMR_R: " << bssn::BSSN_BH2_AMR_R << NRM
                 << std::endl;
            sout << YLW << "\tBSSN_AMR_R_RATIO: " << bssn::BSSN_AMR_R_RATIO
                 << NRM << std::endl;
            ...
            sout << YLW << "\tBSSN_BH1_MAX_LEV:" << bssn::BSSN_BH1_MAX_LEV
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_BH2_MAX_LEV:" << bssn::BSSN_BH2_MAX_LEV
                 << NRM << std::endl;
            ...
            for (unsigned int i = 0;
                 i < bssn::BSSN_NUM_EVOL_VARS_VTU_OUTPUT - 1; i++)
            ...
            for (unsigned int i = 0;
                 i < bssn::BSSN_NUM_CONST_VARS_VTU_OUTPUT - 1; i++)
                sout << bssn::BSSN_VTU_OUTPUT_CONST_INDICES[i] << ", ";
            sout << bssn::BSSN_VTU_OUTPUT_CONST_INDICES
                        [bssn::BSSN_NUM_CONST_VARS_VTU_OUTPUT - 1]
                 << "]" << NRM << std::endl;

            sout << YLW << "\tTPID_TARGET_M_PLUS :" << TPID::target_M_plus
                 << NRM << std::endl;
            sout << YLW << "\tTPID_TARGET_M_MINUS :" << TPID::target_M_minus
                 << NRM << std::endl;
            ...
            sout << YLW << "\tTPID_CENTER_OFFSET : ( " << TPID::center_offset[0]
                 ...
                 << TPID::center_offset[2] << " )" << NRM << std::endl;
            ...
            sout << YLW << "\tTPID_NPOINTS_PHI :" << TPID::npoints_phi << NRM
                 << std::endl;
            sout << YLW << "\tTPID_GIVE_BARE_MASS :" << TPID::give_bare_mass
                 << NRM << std::endl;
            ...
            sout << YLW
                 ...
                 << NRM << std::endl;
            sout << YLW << "\tTPID_VERBOSE :" << TPID::verbose << NRM
                 << std::endl;
            sout << YLW << "\tTPID_ADM_TOL :" << TPID::adm_tol << NRM
                 << std::endl;
            ...
            sout << YLW << "\tBSSN_GW_NUM_RADAII: " << GW::BSSN_GW_NUM_RADAII
                 << NRM << std::endl;
            sout << YLW << "\tBSSN_GW_NUM_LMODES: " << GW::BSSN_GW_NUM_LMODES
                 << NRM << std::endl;
    ...
    double computeWTolDCoords(double x,double y,double z, double* hx)
    {
        ...
        if (bssn::BSSN_USE_WAVELET_TOL_FUNCTION == 1) {
            ...
            const double dr=sqrt(x*x + y*y + z*z);
                ...
                if(dbh<0.1)
                {
                    const double R0 = (GW::BSSN_GW_RADAII[GW::BSSN_GW_NUM_RADAII-1]+10);
                    const double R1 = (GW::BSSN_GW_RADAII[GW::BSSN_GW_NUM_RADAII-1]+20);

                    const double tolMin = std::min(bssn::BSSN_WAVELET_TOL,bssn::BSSN_GW_REFINE_WTOL);
                    const double tolMax = bssn::BSSN_WAVELET_TOL_MAX;
            ...
            if (T_CURRENT > bssn::BSSN_WAVELET_TOL_FUNCTION_R1 + TIME_OFFSET_FAC )
            {
                ...
                //const double R1 = bssn::BSSN_WAVELET_TOL_FUNCTION_R1;
                const double WTOL_EXP_FAC =(RR-R0)/std::log10(W_RR/bssn::BSSN_WAVELET_TOL);
                if(r < R0)
                    return bssn::BSSN_WAVELET_TOL;
                else
                    return std::min(bssn::BSSN_WAVELET_TOL_MAX, ((std::pow(WTOL_EXP,(r-R0)/WTOL_EXP_FAC)) * bssn::BSSN_WAVELET_TOL) ) ;
            ...
            }else
            {
                ...
                if(dbh0 < R01 || dbh1 < R02)
                    ...
                    double minbheps = std::min(
                         ((std::pow(WTOL_EXP,(dbh0-R01)/W1)) * bssn::BSSN_WAVELET_TOL),
                         ((std::pow(WTOL_EXP,(dbh1-R02)/W2)) * bssn::BSSN_WAVELET_TOL) ) ;
            ...
            const double eps[3] =  {bssn::BSSN_WAVELET_TOL, bssn::BSSN_GW_REFINE_WTOL, bssn::BSSN_WAVELET_TOL_MAX};
            double rad[3];
	        rad[0] = 3.0*m1;
	        rad[1] = 4.0*rad[0];
    ...
    ot::Mesh* weakScalingReMesh(ot::Mesh* pMesh, unsigned int target_npes)
    {
        ...
        {
            if(!rank)
                RAISE_ERROR("target npes "<<target_npes<<" is larger than global npes:"<<npes);
        ...
        // Sequential split of each level, such that elemental grain size is satiesfied. 
        ...
        {   
            ...
            DendroIntL req_l_splits = std::max(1ll, (ELE_SZ_REQ_GL - globalSz) / (14*npes));

            std::vector<unsigned int> ref_flags;
            if(current_mesh->isActive())
            {   
                ref_flags.resize(current_mesh->getNumLocalMeshElements(),OCT_NO_CHANGE);
                ...
                // const DendroIntL lsplit_b = (active_rank)*req_l_splits/active_npes;
                // const DendroIntL lsplit_e = (active_rank+1)*req_l_splits/active_npes;
                // const DendroIntL num_split_rank = lsplit_e-lsplit_b;
                
                const ot::TreeNode* pNodes = current_mesh->getAllElements().data();
                

                // DendroIntL lcount =0;
                // for (unsigned int ele = current_mesh->getElementLocalBegin(); ele < current_mesh->getElementLocalEnd(); ele++)
                // {
                ...
                // auto it_upper = std::upper_bound(num_lev_offsets.begin(),num_lev_offsets.end(),req_l_splits);
                // unsigned int valid_npes=active_npes;

                // if(it_upper != num_lev_offsets.end())
                //     valid_npes = std::distance(num_lev_offsets.begin(),it_upper) +1;
                ...
                //     for (unsigned int ele = current_mesh->getElementLocalBegin(); ele < current_mesh->getElementLocalEnd(); ele++)
                //     {
                //         if( lcount < req_l_splits  && pNodes[ele].getLevel() == LMAX-1)
                //         {
            ...
            if(is_refine_g)
            {
                ot::Mesh* new_mesh = current_mesh->ReMesh(bssn::BSSN_DENDRO_GRAIN_SZ);
                ...
                if(!rank)
                    std::cout<<"weak scaling remesh iter : "<<iter_count<<" num global elements : "<<globalSz<<std::endl;

                is_converged = (globalSz >= ELE_SZ_REQ_GL);//|| (fabs(globalSz/(double)npes - bssn::BSSN_DENDRO_GRAIN_SZ)/(double)bssn::BSSN_DENDRO_GRAIN_SZ < 0.1);//(globalSz >= ELE_SZ_REQ_GL);
                iter_count++;
    ...
    void allocate_bssn_deriv_workspace(const ot::Mesh* pMesh,unsigned int s_fac)
    {
        ...
        bssn::BSSN_DERIV_WORKSPACE=new double[s_fac * max_blk_sz* bssn::BSSN_NUM_DERIVS];
