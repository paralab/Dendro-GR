...
static void relax (CCTK_REAL *  const dv, int const nvar, int const n1, int const n2, int const n3,
       ...
       CCTK_REAL const *  const *  const JFD)
{
  ...
  // This is a one omp version but I think currently active one should be more efficient -milinda. -8/10/20. 
  #if 0
  int i, j, k, n;
  for (k = 0; k < n3; k = k + 2)
  {
...
Newton (int const nvar, int const n1, int const n2, int const n3,
	      derivs v, CCTK_REAL const tol, int const itmax)
{
  ...
  while (dmax > tol && it < itmax)
  {
    ...
    fflush(stdout);
    ii = bicgstab (nvar, n1, n2, n3, v, dv, TPID::verbose, 100, dmax * 1.e-3, &normres);
