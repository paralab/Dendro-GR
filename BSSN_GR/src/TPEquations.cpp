...
void BY_Aijofxyz(CCTK_REAL x, CCTK_REAL y, CCTK_REAL z, CCTK_REAL Aij[3][3]) {
    ...
    if (r2_plus < pow(TPID::TP_Tiny, 2)) r2_plus = pow(TPID::TP_Tiny, 2);
    if (r2_minus < pow(TPID::TP_Tiny, 2)) r2_minus = pow(TPID::TP_Tiny, 2);
    r_plus     = sqrt(r2_plus);
    r_minus    = sqrt(r2_minus);
    r3_plus    = r_plus * r2_plus;
    r3_minus   = r_minus * r2_minus;
...
void NonLinEquations(CCTK_REAL rho_adm, CCTK_REAL A, CCTK_REAL B, CCTK_REAL X,
    ...
    psi     = 1. + 0.5 * TPID::par_m_plus / r_plus +
          0.5 * TPID::par_m_minus / r_minus + U.d0[0];
    psi2      = psi * psi;
    psi4      = psi2 * psi2;
    psi7      = psi * psi2 * psi4;
