...
RK_BSSN::RK_BSSN(ot::Mesh *pMesh, DendroScalar pTBegin, DendroScalar pTEnd,
    ...
    for (unsigned int index = 0; index < bssn::BSSN_CONSTRAINT_NUM_VARS;
         index++)
        m_uiConstraintVars[index] = m_uiMesh->createVector<DendroScalar>();

    m_uiUnzipConstraintVars =
        new DendroScalar *[bssn::BSSN_CONSTRAINT_NUM_VARS];
    for (unsigned int index = 0; index < bssn::BSSN_CONSTRAINT_NUM_VARS;
         index++)
    ...
    do {
        ...
        if (bssn::BSSN_ENABLE_BLOCK_ADAPTIVITY)
            ...
            if (bssn::BSSN_REFINEMENT_MODE == bssn::RefinementMode::WAMR) {
                ...
                // **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,bssn::BSSN_DENDRO_AMR_FAC);
                isRefine = bssn::isReMeshWAMR(
                    m_uiMesh, (const double **)m_uiUnzipVar, refineVarIds,
                    refineNumVars, waveletTolFunc, bssn::BSSN_DENDRO_AMR_FAC);
            } else if (bssn::BSSN_REFINEMENT_MODE == bssn::RefinementMode::EH) {
                isRefine = bssn::isRemeshEH(
                    m_uiMesh, (const double **)m_uiUnzipVar, bssn::VAR::U_ALPHA,
                    bssn::BSSN_EH_REFINE_VAL, bssn::BSSN_EH_COARSEN_VAL, true);

            } else if (bssn::BSSN_REFINEMENT_MODE ==
                       bssn::RefinementMode::EH_WAMR) {
        ...
        if (isRefine) {
            ot::Mesh *newMesh =
                m_uiMesh->ReMesh(bssn::BSSN_DENDRO_GRAIN_SZ,
            ...
            for (unsigned int stage = 0; stage < m_uiNumRKStages; stage++)
                for (unsigned int index = 0; index < bssn::BSSN_NUM_VARS;
                     index++) {
                    delete[] m_uiStage[stage][index];
            ...
            if (m_uiMesh->isActive()) {
                DendroScalar l_min = vecMin(m_uiPrevVar[bssn::VAR::U_ALPHA] +
                                                m_uiMesh->getNodeLocalBegin(),
                                            (m_uiMesh->getNumLocalMeshNodes()),
                                            m_uiMesh->getMPICommunicator());
                DendroScalar l_max = vecMax(m_uiPrevVar[bssn::VAR::U_ALPHA] +
                                                m_uiMesh->getNodeLocalBegin(),
                                            (m_uiMesh->getNumLocalMeshNodes()),
                                            m_uiMesh->getMPICommunicator());
...
void RK_BSSN::writeToVTU(DendroScalar **evolZipVarIn,
    ...
    for (unsigned int i = 0; i < numEvolVars; i++) {
        pDataNames.push_back(
            std::string(bssn::BSSN_VAR_NAMES[evolVarIndices[i]]));
        pData[i] = evolZipVarIn[evolVarIndices[i]];
    }

    for (unsigned int i = 0; i < numConstVars; i++) {
        pDataNames.push_back(
            std::string(bssn::BSSN_CONSTRAINT_VAR_NAMES[constVarIndices[i]]));
...
void RK_BSSN::intergridTransferVars(DendroScalar **&zipIn,
...
void RK_BSSN::unzipVars_async(DendroScalar **zipIn, DendroScalar **uzipOut) {
    ...
    for (unsigned int var = 0; var < bssn::BSSN_NUM_VARS;
         var += bssn::BSSN_ASYNC_COMM_K) {
...
void RK_BSSN::performSingleIteration() {
    ...
    if (m_uiMesh->isActive()) {
        ...
        if (m_uiRKType == RKType::RK3) {
            ...
            bssnRHS(m_uiUnzipVarRHS, (const DendroScalar **)m_uiUnzipVar,
                    &(*(blkList.begin())), blkList.size(), current_t);
            ...
            for (unsigned int stage = 0; stage < (bssn::BSSN_RK4_STAGES - 1);
                 stage++) {
            ...
            current_t_adv =
                current_t + RK4_T[(bssn::BSSN_RK4_STAGES - 1)] * m_uiT_h;

#ifdef DEBUG_RK_SOLVER
            if (!rank)
                std::cout << " stage: " << (bssn::BSSN_RK4_STAGES - 1)
                          << " begin: " << std::endl;
            ...
            do {
                ...
                if (m_uiMesh->isActive()) {
                    ...
                    unsigned int bflag;
                    double dx, dy, dz;
                    const Point pt_min(bssn::BSSN_COMPD_MIN[0],
                                       bssn::BSSN_COMPD_MIN[1],
                                       bssn::BSSN_COMPD_MIN[2]);
                    const Point pt_max(bssn::BSSN_COMPD_MAX[0],
                    ...
                    for (unsigned int stage = 0;
                        ...
                        for (unsigned int index = 0;
                             index < bssn::BSSN_NUM_VARS; index++)
                            ot::test::isUnzipNaN(m_uiMesh, m_uiUnzipVar[index]);
#endif

                        bssnRHS(m_uiUnzipVarRHS,
...
#ifdef DEBUG_RK_SOLVER
                    if (!rank)
                        std::cout << " stage: " << (bssn::BSSN_RK45_STAGES - 1)
                                  << " begin: " << std::endl;
                    ...
                    zipVars(m_uiUnzipVarRHS,
                            m_uiStage[(bssn::BSSN_RK45_STAGES - 1)]);

                    /*for(unsigned int
                       index=0;index<bssn::BSSN_NUM_VARS;index++) for(unsigned
                    ...
                    for (unsigned int node = nodeLocalBegin;
                        ...
                        for (unsigned int index = 0;
                            ...
                            m_uiVar[index][node] = m_uiPrevVar[index][node];
                            for (unsigned int s = 0; s < bssn::BSSN_RK45_STAGES;
                                 s++) {
                                if (s == 1)
                ...
                if (n_inf_max > bssn::BSSN_RK45_DESIRED_TOL) {
                    repeatStep = true;
                    m_uiT_h =
                        bssn::BSSN_SAFETY_FAC * m_uiT_h *
                        (pow(fabs(bssn::BSSN_RK45_DESIRED_TOL / n_inf_max),
                             0.25));
                    if (!m_uiMesh->getMPIRankGlobal())
...
void RK_BSSN::rkSolve() {
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % bssn::BSSN_CHECKPT_FREQ) == 0)
            storeCheckPoint(bssn::BSSN_CHKPT_FILE_PREFIX.c_str());
        bssn::timer::t_ioCheckPoint.stop();

        // write sol to vtu.
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % bssn::BSSN_TIME_STEP_OUTPUT_FREQ) == 0) {
            l_min = vecMin(
                m_uiPrevVar[bssn::VAR::U_ALPHA] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[bssn::VAR::U_ALPHA] + m_uiMesh->getNodeLocalBegin(),
        ...
        if ((m_uiCurrentStep % bssn::BSSN_REMESH_TEST_FREQ) == 0) {
            ...
            // **)m_uiPrevVar,refineVarIds,refineNumVars,bssn::BSSN_WAVELET_TOL);
            unzipVars(m_uiPrevVar, m_uiUnzipVar);
#endif
                ...
                if (bssn::BSSN_REFINEMENT_MODE == bssn::RefinementMode::WAMR) {
                    ...
                    isRefine = bssn::isReMeshWAMR(
                        ...
                        bssn::BSSN_DENDRO_AMR_FAC);
                           ...
                           bssn::RefinementMode::EH) {
                    isRefine = bssn::isRemeshEH(
                        m_uiMesh, (const double **)m_uiUnzipVar,
                        bssn::VAR::U_ALPHA, bssn::BSSN_EH_REFINE_VAL,
                        bssn::BSSN_EH_COARSEN_VAL, true);
                    ...
                    const bool isR2 = bssn::isRemeshEH(
                        m_uiMesh, (const double **)m_uiUnzipVar,
                        bssn::VAR::U_ALPHA, bssn::BSSN_EH_REFINE_VAL,
                        bssn::BSSN_EH_COARSEN_VAL, false);
            ...
            if (isRefine) {
                ...
                ot::Mesh *newMesh = m_uiMesh->ReMesh(bssn::BSSN_DENDRO_GRAIN_SZ,
                ...
                // deallocate constraint vars allocate them for the new mesh.
                for (unsigned int index = 0;
                     index < bssn::BSSN_CONSTRAINT_NUM_VARS; index++) {
                    delete[] m_uiConstraintVars[index];
                    delete[] m_uiUnzipConstraintVars[index];
                ...
                if (m_uiMesh->isActive()) {
                    l_min = vecMin(m_uiPrevVar[bssn::VAR::U_ALPHA] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    l_max = vecMax(m_uiPrevVar[bssn::VAR::U_ALPHA] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    if (!(m_uiMesh->getMPIRank())) {
        ...
        if ((m_uiCurrentStep % bssn::BSSN_GW_EXTRACT_FREQ) == 0) {
            ...
            bssn::extractConstraints(
                m_uiMesh, (const DendroScalar **)m_uiConstraintVars,
                m_uiPrevVar[BHLOC::EXTRACTION_VAR_ID], BHLOC::EXTRACTION_TOL,
                m_uiCurrentStep, m_uiCurrentTime);
#ifndef BSSN_KERR_SCHILD_TEST
#ifdef BSSN_EXTRACT_GRAVITATIONAL_WAVES
            GW::extractFarFieldPsi4(m_uiMesh,
                                    (const DendroScalar **)m_uiConstraintVars,
                                    m_uiCurrentStep, m_uiCurrentTime);
#endif
#endif
            ...
            bssn::writeBHCoordinates((const ot::Mesh *)m_uiMesh,
                                     (const Point *)m_uiBHLoc, 2,
                                     m_uiCurrentStep, m_uiCurrentTime);
#endif
        }
...
#ifdef BSSN_EXTRACT_BH_LOCATIONS
        // bssn::extractBHCoords((const ot::Mesh *)m_uiMesh,(const
...
void RK_BSSN::storeCheckPoint(const char *fNamePrefix) {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiCurrentStep % (2 * bssn::BSSN_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
        ...
        io::checkpoint::writeVecToFile(
            fName, m_uiMesh, (const double **)m_uiPrevVar, bssn::BSSN_NUM_VARS);
    ...
    for (unsigned int cpIndex = restoreFileIndex; cpIndex < 2; cpIndex++) {
        ...
        if (isActive) {
            ...
            restoreStatus = io::checkpoint::readVecFromFile(
                fName, newMesh, m_uiPrevVar, bssn::BSSN_NUM_VARS);
        }
