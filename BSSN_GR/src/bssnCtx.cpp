...
namespace bssn
{
    ...
    {   
        ...
        m_var[VL::CPU_EV_UZ_OUT].create_vector(m_uiMesh,ot::DVEC_TYPE::OCT_LOCAL_WITH_PADDING,ot::DVEC_LOC::HOST,BSSN_NUM_VARS,true);
    ...
    int BSSNCtx::rhs(DVec* in , DVec* out, unsigned int sz , DendroScalar time)
    {
    ...
    #if 0
        int BSSNCtx::rhs_blkwise(DVec in, DVec out, const unsigned int* const blkIDs, unsigned int numIds, DendroScalar*  blk_time)
        {
        ...
        int BSSNCtx::rhs_blk(const DendroScalar* in, DendroScalar* out, unsigned int dof, unsigned int local_blk_id, DendroScalar  blk_time) 
        {
        ...
        int BSSNCtx::pre_stage_blk(DendroScalar* in, unsigned int dof, unsigned int local_blk_id, DendroScalar  blk_time)
        {
        ...
        int BSSNCtx::post_stage_blk(DendroScalar* in, unsigned int dof, unsigned int local_blk_id, DendroScalar  blk_time)
        {
            return 0;
        }

        int BSSNCtx::pre_timestep_blk(DendroScalar* in, unsigned int dof, unsigned int local_blk_id, DendroScalar  blk_time)
        {
            return 0;
        }
        
        int BSSNCtx::post_timestep_blk(DendroScalar* in, unsigned int dof, unsigned int local_blk_id, DendroScalar  blk_time)
        {
            DendroScalar **  unzipIn   = new DendroScalar*[dof];
            const unsigned int blk = local_blk_id;
    ...
    int BSSNCtx::restore_checkpt()
    {
        ...
        m_var[VL::CPU_EV_UZ_OUT].create_vector(newMesh,ot::DVEC_TYPE::OCT_LOCAL_WITH_PADDING,ot::DVEC_LOC::HOST,BSSN_NUM_VARS,true);
    ...
    int BSSNCtx::grid_transfer(const ot::Mesh* m_new)
    {
        ...
        m_var[VL::CPU_EV_UZ_OUT].create_vector(m_new,ot::DVEC_TYPE::OCT_LOCAL_WITH_PADDING,ot::DVEC_LOC::HOST,BSSN_NUM_VARS,true);
    ...
    unsigned int BSSNCtx::compute_lts_ts_offset()
    {
        ...
        for(unsigned int blk = 0; blk < numBlocks; blk++)
        {
            ...
            if (bssn::RIT_ETA_FUNCTION == 0) {
                ...
                double arg = - w*w*w*w;
                eta = (bssn::RIT_ETA_CENTRAL - bssn::RIT_ETA_OUTER)*exp(arg) + bssn::RIT_ETA_OUTER;
            }
