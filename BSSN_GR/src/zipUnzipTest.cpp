...
/**
 * @brief This file contains the tests to measure the efficiency of the
 * zip/unzip computations.
 * */
...
int main(int argc, char** argv) {
    ...
    double regRange = 10;
    if (argc > 2) regRange = atof(argv[2]);
    ...
    if (!rank) {
        ...
        std::cout << "\t" << GRN << " e2e (min,mean,max): "
                  << "( " << t_e2e_g[0] << "\t" << t_e2e_g[1] << "\t"
                  << t_e2e_g[2] << " )" << NRM << std::endl;
        std::cout << "\t" << GRN << " e2n (min,mean,max): "
                  << "( " << t_e2n_g[0] << "\t" << t_e2n_g[1] << "\t"
                  << t_e2n_g[2] << " )" << NRM << std::endl;
        std::cout << "\t" << GRN << " sm (min,mean,max): "
                  << "( " << t_sm_g[0] << "\t" << t_sm_g[1] << "\t" << t_sm_g[2]
                  << " )" << NRM << std::endl;
        std::cout << "\t" << GRN << " blk (min,mean,max): "
                  << "( " << t_blk_g[0] << "\t" << t_blk_g[1] << "\t"
                  << t_blk_g[2] << " )" << NRM << std::endl;
    }

    const std::vector<ot::Block>& blkList = mesh->getLocalBlockList();
    if (!rank)
        std::cout << "number of blocks : " << blkList.size() << std::endl;
    /*if(!rank)
    {
        for(unsigned int blk=0;blk<blkList.size();blk++)
        {
            std::cout<<"blk: "<<blk<<" blkNode:
    "<<blkList[blk].getBlockNode()<<" element begin:
    "<<blkList[blk].getLocalElementBegin()<<" element end :
    "<<blkList[blk].getLocalElementEnd()<<std::endl;
        }
    }*/
    ...
    computeOctreeStats(
        ...
        mesh->getNumLocalMeshElements(), octByLev, octByLev_g,
        regGridPrecentage, mesh->getMPIGlobalCommunicator());

    if (!rank) std::cout << "regGrid param : " << regRange << std::endl;
    if (!rank)
        std::cout << "regGrid precentage: " << regGridPrecentage << std::endl;
    ...
    for (unsigned int iter = 0; iter < NUM_ITER; iter++) {
        ...
        for (unsigned int blk = 0; blk < blkList.size(); blk++) {
            ...
            bssnrhs(m_uiUnzipOut, (const double**)m_uiUnzipIn, offset, ptmin,
                    ptmax, sz, bflag, 0.0);
        }
    ...
    if (!rank)
        std::cout << std::left << std::setw(nameWidth)
                  << std::setfill(separator) << "+runtime(s)";
    if (!rank)
        std::cout << std::left << std::setw(nameWidth)
    ...
    std::cout << "gridRatio\t maxDepth\t numElems\t cgNodes\t unzip\t rhs\t "
                 "zip\t commun\t"
              << std::endl;
    printf("%f\t%d\t%d\t%d\t%f\t%f\t%f\t%f\t\n", regGridPrecentage,
           bssn::BSSN_MAXDEPTH, numBalOcts, numCGNodes, t_unzip, t_rhs, t_zip,
           t_comm);
