...
RK4_NLSM::RK4_NLSM(ot::Mesh *pMesh, double pTBegin, double pTEnd, double pTh)
...
void RK4_NLSM::initialGridConverge() {
    ...
    do {
        ...
        if (nlsm::NLSM_ENABLE_BLOCK_ADAPTIVITY)
            ...
            isRefine = nlsm::isRemeshForce(
                m_uiMesh, (const double **)m_uiUnzipVar, nlsm::VAR::U_CHI,
                nlsm::NLSM_CHI_REFINE_VAL, nlsm::NLSM_CHI_COARSEN_VAL, true);
            // m_uiMesh->isReMeshUnzip((const double
            // **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,nlsm::NLSM_DENDRO_AMR_FAC);
        }

        if (isRefine) {
            ot::Mesh *newMesh =
                m_uiMesh->ReMesh(nlsm::NLSM_DENDRO_GRAIN_SZ,
            ...
            for (unsigned int stage = 0; stage < nlsm::NLSM_RK4_STAGES; stage++)
                for (unsigned int index = 0; index < nlsm::NLSM_NUM_VARS;
                     index++) {
                    delete[] m_uiStage[stage][index];
            ...
            if (m_uiMesh->isActive()) {
                double l_min = vecMin(m_uiPrevVar[nlsm::VAR::U_CHI] +
                                          m_uiMesh->getNodeLocalBegin(),
                                      (m_uiMesh->getNumLocalMeshNodes()),
                                      m_uiMesh->getMPICommunicator());
                double l_max = vecMax(m_uiPrevVar[nlsm::VAR::U_CHI] +
                                          m_uiMesh->getNodeLocalBegin(),
                                      (m_uiMesh->getNumLocalMeshNodes()),
                                      m_uiMesh->getMPICommunicator());
...
void RK4_NLSM::writeToVTU(double **evolZipVarIn, double **constrZipVarIn,
            ...
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    ...
                    if (nodeLookUp_CG >= nodeLocalBegin &&
                        ...
                        u_x_t((double)x, (double)y, (double)z, m_uiCurrentTime,
                              var);
                        diffVec[nodeLookUp_CG] =
                            var[nlsm::VAR::U_CHI] -
                            evolZipVarIn[nlsm::VAR::U_CHI][nodeLookUp_CG];
    ...
    double l_rs          = rsNormLp<double>(m_uiMesh, chiAnalytical,
                                            evolZipVarIn[nlsm::VAR::U_CHI], 2);
    ...
    if (!m_uiMesh->getMPIRank()) {
        ...
        std::cout << YLW << "\t ||VAR::DIFF|| (min, max,l2,l_2rs) : (" << l_min
                  << ", " << l_max << ", " << l2_norm << ", " << l_rs << " ) "
                  << NRM << std::endl;
        ...
        fileGW << m_uiCurrentStep << "\t" << m_uiCurrentTime << "\t" << l_min
               << "\t" << l_max << "\t" << l2_norm << "\t" << total_dof << "\t "
               << l_rs << std::endl;
        fileGW.close();
    }
    ...
    char fPrefix[256];
    sprintf(fPrefix, "%s_%d", nlsm::NLSM_VTU_FILE_PREFIX.c_str(),
            m_uiCurrentStep);

#ifdef NLSM_COMPARE_WITH_ANALYTICAL_SOL
    io::vtk::mesh2vtuFine(m_uiMesh, fPrefix, 2, fDataNames, fData,
                          (numEvolVars + numConstVars + 2),
...
void RK4_NLSM::intergridTransferVars(double **&zipIn,
...
void RK4_NLSM::unzipVars_async(double **zipIn, double **uzipOut) {
    ...
    for (unsigned int var = 0; var < nlsm::NLSM_NUM_VARS;
         var += nlsm::NLSM_ASYNC_COMM_K) {
...
void RK4_NLSM::performSingleIteration() {
    ...
    do {
        ...
        if (m_uiMesh->isActive()) {
            ...
            for (unsigned int stage = 0; stage < (nlsm::NLSM_RK4_STAGES - 1);
                 stage++) {
                ...
                for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                    ...
                    // "<<(1u<<(m_uiMaxDepth-m_uiMesh->getAllElements()[blkList[blk].getLocalElementBegin()].getLevel()))/(double)nlsm::NLSM_ELE_ORDER<<"
                    ...
                    nlsmRhs(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar,
            ...
            current_t_adv =
                current_t + RK4_T[(nlsm::NLSM_RK4_STAGES - 1)] * m_uiT_h;

#ifdef DEBUG_RK_SOLVER
            if (!rank)
                std::cout << " stage: " << (nlsm::NLSM_RK4_STAGES - 1)
                          << " begin: " << std::endl;
            for (unsigned int index = 0; index < nlsm::NLSM_NUM_VARS; index++)
                ot::test::isUnzipNaN(m_uiMesh, m_uiUnzipVar[index]);
#endif
            ...
            for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                ...
                nlsmRhs(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, offset,
...
void RK4_NLSM::rkSolve() {
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % nlsm::NLSM_CHECKPT_FREQ) == 0)
            storeCheckPoint(nlsm::NLSM_CHKPT_FILE_PREFIX.c_str());
        nlsm::timer::t_ioCheckPoint.stop();
        // write sol to vtu.
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % nlsm::NLSM_TIME_STEP_OUTPUT_FREQ) == 0) {
            l_min = vecMin(
                m_uiPrevVar[nlsm::VAR::U_CHI] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[nlsm::VAR::U_CHI] + m_uiMesh->getNodeLocalBegin(),
            ...
            if (!m_uiMesh->getMPIRank()) {
                ...
                std::cout << "\t ||VAR::U_CHI|| (min, max) : (" << l_min << ", "
                          << l_max << " ) " << std::endl;
            }

            nlsm::timer::profileInfoIntermediate(
                nlsm::NLSM_PROFILE_FILE_PREFIX.c_str(), m_uiMesh,
                m_uiCurrentStep);
        }
        ...
        if ((m_uiCurrentStep % nlsm::NLSM_REMESH_TEST_FREQ) == 0) {
            ...
            // **)m_uiPrevVar,refineVarIds,refineNumVars,nlsm::NLSM_WAVELET_TOL);
            unzipVars(m_uiPrevVar, m_uiUnzipVar);
#endif
            ...
            if (nlsm::NLSM_ENABLE_BLOCK_ADAPTIVITY)
                ...
                if (nlsm::NLSM_REFINE_MODE == nlsm::RefineMode::FR)
                    isRefine = nlsm::isRemeshForce(
                        ...
                        nlsm::VAR::U_CHI, nlsm::NLSM_CHI_REFINE_VAL,
                        nlsm::NLSM_CHI_COARSEN_VAL, true);
                ...
                if (nlsm::NLSM_REFINE_MODE == nlsm::RefineMode::WAMR_FR) {
                    const bool isRefine1 = m_uiMesh->isReMeshUnzip(
                        ...
                        refineNumVars, waveletTolFunc,
                        nlsm::NLSM_DENDRO_AMR_FAC);
                    const bool isRefine2 = isRefine = nlsm::isRemeshForce(
                        m_uiMesh, (const double **)m_uiUnzipVar,
                        nlsm::VAR::U_CHI, nlsm::NLSM_CHI_REFINE_VAL,
                        nlsm::NLSM_CHI_COARSEN_VAL, false);
            ...
            if (isRefine) {
                ...
                ot::Mesh *newMesh = m_uiMesh->ReMesh(nlsm::NLSM_DENDRO_GRAIN_SZ,
                ...
                for (unsigned int stage = 0; stage < nlsm::NLSM_RK4_STAGES;
                     stage++)
                    for (unsigned int index = 0; index < nlsm::NLSM_NUM_VARS;
                         index++) {
                        delete[] m_uiStage[stage][index];
                ...
                if (m_uiMesh->isActive()) {
                    l_min = vecMin(m_uiPrevVar[nlsm::VAR::U_CHI] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    l_max = vecMax(m_uiPrevVar[nlsm::VAR::U_CHI] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    if (!(m_uiMesh->getMPIRank())) {
                        std::cout << "transfer completed:    ||VAR::U_CHI|| "
                                     "(min, max) : ("
                                  << l_min << ", " << l_max << " ) "
                                  << std::endl;
                    }
...
void RK4_NLSM::storeCheckPoint(const char *fNamePrefix) {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiCurrentStep % (2 * nlsm::NLSM_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
        ...
        io::checkpoint::writeVecToFile(
            fName, m_uiMesh, (const double **)m_uiPrevVar, nlsm::NLSM_NUM_VARS);
    ...
    for (unsigned int cpIndex = 0; cpIndex < 2; cpIndex++) {
        ...
        if (isActive) {
            ...
            restoreStatus = io::checkpoint::readVecFromFile(
                fName, newMesh, m_uiPrevVar, nlsm::NLSM_NUM_VARS);
        }
