...
InvNLSMCtx::InvNLSMCtx() : invp::InverseCtx<InvNLSMCtx, NLSMCtx>() { return; }
...
int InvNLSMCtx::initialize_forward_ctx() {
    ...
    if (m_uiParFile == std::string(""))
        if (!grank) {
            std::cout << "[InverseCtx]: parameter file is not set : "
                      << std::endl;
    ...
    if (m_uiJobLauncher == NULL)
        if (!grank) {
            std::cout << "[InverseCtx]: job launcher context is not set : "
                      << std::endl;
            MPI_Abort(m_uiCommGlobal, 0);
        }

    if (!grank)
        std::cout << " reading parameter file :" << m_uiParFile << std::endl;
    ...
    nlsm::NLSM_VTU_FILE_PREFIX =
        nlsm::NLSM_VTU_FILE_PREFIX + std::string("_job_id_") +
        std::to_string(m_uiJobLauncher->get_comm_split_index());
    nlsm::NLSM_CHKPT_FILE_PREFIX =
        nlsm::NLSM_CHKPT_FILE_PREFIX + std::string("_job_id_") +
        std::to_string(m_uiJobLauncher->get_comm_split_index());
    nlsm::NLSM_PROFILE_FILE_PREFIX =
        nlsm::NLSM_PROFILE_FILE_PREFIX + std::string("_job_id_") +
        std::to_string(m_uiJobLauncher->get_comm_split_index());
    ...
    function2Octree(
        f_init, nlsm::NLSM_NUM_VARS, nlsm::NLSM_REFINE_VARIABLE_INDICES,
        nlsm::NLSM_NUM_REFINE_VARS, tmpNodes, m_uiMaxDepth,
        nlsm::NLSM_WAVELET_TOL, nlsm::NLSM_ELE_ORDER, m_uiCommLocal);
    ot::Mesh* mesh = ot::createMesh(
        tmpNodes.data(), tmpNodes.size(), nlsm::NLSM_ELE_ORDER, m_uiCommLocal,
        1, ot::SM_TYPE::FDM, nlsm::NLSM_DENDRO_GRAIN_SZ,
...
int InvNLSMCtx::launch_forward_solve() {
    ...
    for (ets->init(); ets->curr_time() < nlsm::NLSM_RK45_TIME_END;
        ...
        if (step % nlsm::NLSM_TIME_STEP_OUTPUT_FREQ == 0) {
            ...
            this->extract_waves(m_uiCntrlVars.WAVE_EXTRAC_LOC,
                                m_uiCntrlVars.NUM_WAVE_EXTRACT_LOC);
        }
        ...
        if (isRemesh) {
            ...
            appCtx->remesh_and_gridtransfer(
                nlsm::NLSM_DENDRO_GRAIN_SZ, nlsm::NLSM_LOAD_IMB_TOL,
                nlsm::NLSM_SPLIT_FIX, true, false, false);
            appCtx->terminal_output();
...
int InvNLSMCtx::launch_adjoint_solve() { return 0; }

int InvNLSMCtx::gradient_approximation() { return 0; }
...
int InvNLSMCtx::extract_waves(const Point* pts, unsigned int n) {
    ...
    if (mesh->isActive()) {
        ...
        grid_limits[0] =
            Point(nlsm::NLSM_OCTREE_MIN[0], nlsm::NLSM_OCTREE_MIN[1],
                  nlsm::NLSM_OCTREE_MIN[2]);
        grid_limits[1] =
            Point(nlsm::NLSM_OCTREE_MAX[0], nlsm::NLSM_OCTREE_MAX[1],
                  nlsm::NLSM_OCTREE_MAX[2]);

        domain_limits[0] =
            Point(nlsm::NLSM_COMPD_MIN[0], nlsm::NLSM_COMPD_MIN[1],
                  nlsm::NLSM_COMPD_MIN[2]);
        domain_limits[1] =
            Point(nlsm::NLSM_COMPD_MAX[0], nlsm::NLSM_COMPD_MAX[1],
        ...
        ot::da::interpolateToCoordsAndGather(
            mesh, &vin[(nlsm::VAR::U_CHI)*nodes_cg], domain_pts.data(),
            domain_pts.size(), grid_limits, domain_limits,
            &(*(extracted_wave.begin())), 0, 1);
        ...
        if (!(mesh->getMPIRank())) {
            ...
            for (unsigned int i = 0; i < numPts; i++) {
                std::ofstream fileGW;
                sprintf(fname, "%s_obs_%d.dat",
                        nlsm::NLSM_PROFILE_FILE_PREFIX.c_str(), i);
                ...
                fileGW << m_uiTSObj->curr_time() << "\t" << extracted_wave[i]
                       << std::endl;
                fileGW.close();
            }
        }
