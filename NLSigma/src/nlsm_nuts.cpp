...
int main(int argc, char** argv) {
    ...
    //     {
    //         if(tmpOct[ele].minX() < mid_x /*&& tmpOct[ele].minY() < mid_x &&
    //         tmpOct[ele].minZ() < mid_x*/ )
    ...
    //     std::cout<<"tmpOct: at lev "<<l<<" size :
    //     "<<tmpOct.size()<<std::endl;
    ...
    // for(unsigned int ele = mesh->getElementLocalBegin(); ele <
    // mesh->getElementLocalEnd(); ele ++)
    ...
    //   {
    //     std::cout<<"local sz: "<<i<<" part_sz: "<<part_sz[i]<<" weight:
    //     "<<weight_sz[i]<<std::endl;
    //   }
    ...
    if (ts_mode == 0) {
        ...
        ts::ExplicitNUTS<DendroScalar, nlsm::NLSMCtx>* enuts =
            new ts::ExplicitNUTS<DendroScalar, nlsm::NLSMCtx>(appCtx);
        ...
        for (enuts->init(); enuts->curr_time() < nlsm::NLSM_RK45_TIME_END;
             enuts->evolve())
        // for(enuts->init(); enuts->curr_time() < nlsm::NLSM_RK45_TIME_END ;
...
// enuts->dump_pt(std::cout);
    ...
    //     ts::ExplicitNUTS<DendroScalar,nlsm::NLSMCtx>*  enuts = new
    //     ts::ExplicitNUTS<DendroScalar,nlsm::NLSMCtx>(appCtx_enuts,false);
    //     ts::ETS<DendroScalar,nlsm::NLSMCtx>*           ets   = new
    //     ts::ETS<DendroScalar,nlsm::NLSMCtx>(appCtx_ets);
    ...
    //     if(!rank_global)
    //       std::cout<<GRN<<"[Explicit NUTS]: Executing step :
    //       "<<enuts->curr_step()<<std::setw(10)<<"\tcurrent time
    //       :"<<enuts->curr_time()<<std::setw(10)<<"\t
    //       dt(min):"<<enuts->get_dt_min()<<std::setw(10)<<"\t
    //       dt(max):"<<enuts->get_dt_max()<<std::setw(10)<<"\t"<<NRM<<std::endl;
    ...
    //       if(!rank)
    //         std::cout<<"[ETS] vec(min, max): ( "<<min<<" \t"<<", "<<max<<" )
    //         "<<std::endl;
    //     }
    ...
    //       evar_diff.VecMinMax(mesh, min, max, v);
    //       if(!rank)
    //         std::cout<<"[ENUTS] vec(min, max): ( "<<min<<" \t"<<", "<<max<<"
    //         ) "<<std::endl;
    //     }
    ...
    //       //evar_diff.VecMinMax(mesh, min, max, v);
    //       if(!rank)
    //         std::cout<<"[diff] vec(min, max): ( "<<min<<" \t"<<", "<<max<<" )
    //         "<<std::endl;
    ...
    //         f_ets.open (fName,std::fstream::app);
    //         if(f_ets.fail()) {std::cout<<fName<<" file open failed
    //         "<<std::endl; MPI_Abort(comm,0);}
    ...
    //       {
    //         f_enuts.open (fName,std::fstream::app);
    //         if(f_enuts.fail()) {std::cout<<fName<<" file open failed
    //         "<<std::endl; MPI_Abort(comm,0);}
    ...
    //           if(diffFile.fail()) {std::cout<<" diff.dat file open failed
    //           "<<std::endl;
    //            MPI_Abort(comm,0);
    //           }
    ...
    //         double l2 = normL2(evar_diff.GetVecArray()+ v*cg_sz +
    //         mesh->getNodeLocalBegin(), mesh->getNumLocalMeshNodes(), comm);

    //         if(!rank_global)
    //         {
    //           l2=l2/sqrt(cg_sz_g);
    //           std::cout<<YLW<<"[diff] vec(min, max): ( "<<min<<" \t"<<",
    //           "<<max<<" ) "<<NRM<<std::endl;
    ...
    //       io::vtk::mesh2vtuFine(mesh,fName, 0, NULL, NULL,
    //       nlsm::NLSM_NUM_VARS, nlsm::NLSM_VAR_NAMES, (const double**) eVec);
    //       evar_diff.destroy_vector();
    //       delete [] eVec;
    ...
    //               appCtx_ets->remesh_and_gridtransfer(nlsm::NLSM_DENDRO_GRAIN_SZ,
    //               nlsm::NLSM_LOAD_IMB_TOL,nlsm::NLSM_SPLIT_FIX,true,false,false);
    //               appCtx_ets->terminal_output();
    //           }
    ...
    //           appCtx_enuts->remesh_and_gridtransfer(nlsm::NLSM_DENDRO_GRAIN_SZ,
    //           nlsm::NLSM_LOAD_IMB_TOL,nlsm::NLSM_SPLIT_FIX,true,false,false);
    //           appCtx_enuts->terminal_output();
