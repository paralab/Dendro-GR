...
void readParamFile(const char* fName, MPI_Comm comm) {
    ...
    if (!rank) {
        ...
        nlsm::NLSM_VTU_FILE_PREFIX =
            parFile["NLSM_VTU_FILE_PREFIX"].get<std::string>();
        nlsm::NLSM_CHKPT_FILE_PREFIX =
            parFile["NLSM_CHKPT_FILE_PREFIX"].get<std::string>();
        nlsm::NLSM_PROFILE_FILE_PREFIX =
            parFile["NLSM_PROFILE_FILE_PREFIX"].get<std::string>();
        nlsm::NLSM_RESTORE_SOLVER = parFile["NLSM_RESTORE_SOLVER"];
        nlsm::NLSM_ENABLE_BLOCK_ADAPTIVITY =
            parFile["NLSM_ENABLE_BLOCK_ADAPTIVITY"];
        ...
        if (parFile.find("NLSM_REFINE_MODE") != parFile.end())
            nlsm::NLSM_REFINE_MODE =
                static_cast<nlsm::RefineMode>(parFile["NLSM_REFINE_MODE"]);

        nlsm::NLSM_NUM_REFINE_VARS = parFile["NLSM_NUM_REFINE_VARS"];

        for (unsigned int i = 0; i < nlsm::NLSM_NUM_REFINE_VARS; i++)
            nlsm::NLSM_REFINE_VARIABLE_INDICES[i] =
                parFile["NLSM_REFINE_VARIABLE_INDICES"][i];

        nlsm::NLSM_NUM_EVOL_VARS_VTU_OUTPUT =
            parFile["NLSM_NUM_EVOL_VARS_VTU_OUTPUT"];

        for (unsigned int i = 0; i < nlsm::NLSM_NUM_EVOL_VARS_VTU_OUTPUT; i++)
            nlsm::NLSM_VTU_OUTPUT_EVOL_INDICES[i] =
                parFile["NLSM_VTU_OUTPUT_EVOL_INDICES"][i];
    ...
    if (NLSM_NUM_REFINE_VARS > NLSM_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of refine variables should "
                     "be less than number of NLSM_NUM_VARS"
                  << std::endl;
        exit(0);
    }
    if (NLSM_NUM_EVOL_VARS_VTU_OUTPUT > NLSM_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of evolution VTU variables "
                     "should be less than number of NLSM_NUM_VARS"
                  << std::endl;
        exit(0);
    }
...
void dumpParamFile(std::ostream& sout, int root, MPI_Comm comm) {
    ...
    if (rank == root || npes == 1) {
        ...
        sout << YLW << "\tNLSM_ELE_ORDER :" << nlsm::NLSM_ELE_ORDER << NRM
             << std::endl;
        sout << YLW << "\tNLSM_PADDING_WIDTH :" << nlsm::NLSM_PADDING_WIDTH
             << NRM << std::endl;
        sout << YLW << "\tNLSM_DIM :" << nlsm::NLSM_DIM << NRM << std::endl;
        sout << YLW << "\tNLSM_IO_OUTPUT_FREQ :" << nlsm::NLSM_IO_OUTPUT_FREQ
             << NRM << std::endl;
        sout << YLW
             << "\tNLSM_REMESH_TEST_FREQ :" << nlsm::NLSM_REMESH_TEST_FREQ
             << NRM << std::endl;
        sout << YLW << "\tNLSM_CHECKPT_FREQ :" << nlsm::NLSM_CHECKPT_FREQ << NRM
             << std::endl;
        sout << YLW << "\tNLSM_RESTORE_SOLVER :" << nlsm::NLSM_RESTORE_SOLVER
             << NRM << std::endl;
        sout << YLW << "\tNLSM_ENABLE_BLOCK_ADAPTIVITY :"
             << nlsm::NLSM_ENABLE_BLOCK_ADAPTIVITY << NRM << std::endl;
        sout << YLW << "\tNLSM_VTU_FILE_PREFIX :" << nlsm::NLSM_VTU_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW
             << "\tNLSM_CHKPT_FILE_PREFIX :" << nlsm::NLSM_CHKPT_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW
             << "\tNLSM_PROFILE_FILE_PREFIX :" << nlsm::NLSM_PROFILE_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW << "\tNLSM_IO_OUTPUT_GAP :" << nlsm::NLSM_IO_OUTPUT_GAP
             << NRM << std::endl;
        sout << YLW << "\tNLSM_DENDRO_GRAIN_SZ :" << nlsm::NLSM_DENDRO_GRAIN_SZ
             << NRM << std::endl;
        sout << YLW << "\tNLSM_ASYNC_COMM_K :" << nlsm::NLSM_ASYNC_COMM_K << NRM
             << std::endl;
        sout << YLW << "\tNLSM_DENDRO_AMR_FAC :" << nlsm::NLSM_DENDRO_AMR_FAC
             << NRM << std::endl;
        sout << YLW << "\tNLSM_CFL_FACTOR:" << nlsm::NLSM_CFL_FACTOR << NRM
             << std::endl;
        sout << YLW << "\tNLSM_WAVELET_TOL :" << nlsm::NLSM_WAVELET_TOL << NRM
             << std::endl;
        sout << YLW << "\tNLSM_LOAD_IMB_TOL :" << nlsm::NLSM_LOAD_IMB_TOL << NRM
             << std::endl;
        sout << YLW << "\tNLSM_RK45_TIME_BEGIN :" << nlsm::NLSM_RK45_TIME_BEGIN
             << NRM << std::endl;
        sout << YLW << "\tNLSM_RK45_TIME_END :" << nlsm::NLSM_RK45_TIME_END
             << NRM << std::endl;
        sout << YLW
             << "\tNLSM_RK45_TIME_STEP_SIZE :" << nlsm::NLSM_RK45_TIME_STEP_SIZE
             << NRM << std::endl;
        sout << YLW
             << "\tNLSM_RK45_DESIRED_TOL :" << nlsm::NLSM_RK45_DESIRED_TOL
             << NRM << std::endl;
        sout << YLW << "\tNLSM_COMPD_MIN : ( :" << nlsm::NLSM_COMPD_MIN[0]
             << " ," << nlsm::NLSM_COMPD_MIN[1] << ","
             << nlsm::NLSM_COMPD_MIN[2] << " )" << NRM << std::endl;
        sout << YLW << "\tNLSM_COMPD_MAX : ( :" << nlsm::NLSM_COMPD_MAX[0]
             << " ," << nlsm::NLSM_COMPD_MAX[1] << ","
             << nlsm::NLSM_COMPD_MAX[2] << " )" << NRM << std::endl;
        sout << YLW << "\tNLSM_BLK_MIN : ( :" << nlsm::NLSM_BLK_MIN_X << " ,"
             << nlsm::NLSM_BLK_MIN_Y << "," << nlsm::NLSM_BLK_MIN_Z << " )"
             << NRM << std::endl;
        sout << YLW << "\tNLSM_BLK_MAX : ( :" << nlsm::NLSM_BLK_MAX_X << " ,"
             << nlsm::NLSM_BLK_MAX_Y << "," << nlsm::NLSM_BLK_MAX_Z << " )"
             << NRM << std::endl;
        sout << YLW << "\tNLSM_OCTREE_MIN : ( :" << nlsm::NLSM_OCTREE_MIN[0]
             << " ," << nlsm::NLSM_OCTREE_MIN[1] << ","
             << nlsm::NLSM_OCTREE_MIN[2] << " )" << NRM << std::endl;
        sout << YLW << "\tNLSM_OCTREE_MAX : ( :" << nlsm::NLSM_OCTREE_MAX[0]
             << " ," << nlsm::NLSM_OCTREE_MAX[1] << ","
             << nlsm::NLSM_OCTREE_MAX[2] << " )" << NRM << std::endl;
        sout << YLW << "\tKO_DISS_SIGMA :" << nlsm::KO_DISS_SIGMA << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_TYPE:" << nlsm::NLSM_ID_TYPE << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_AMP1:" << nlsm::NLSM_ID_AMP1 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_AMP2:" << nlsm::NLSM_ID_AMP2 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_DELTA1:" << nlsm::NLSM_ID_DELTA1 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_DELTA2:" << nlsm::NLSM_ID_DELTA2 << NRM
             << std::endl;
        ...
        sout << YLW << "\tNLSM_ID_ZC2:" << nlsm::NLSM_ID_ZC2 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_EPSX1:" << nlsm::NLSM_ID_EPSX1 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_EPSY1:" << nlsm::NLSM_ID_EPSY1 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_EPSZ1:" << nlsm::NLSM_ID_EPSY1 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_EPSX2:" << nlsm::NLSM_ID_EPSX2 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_EPSY2:" << nlsm::NLSM_ID_EPSY2 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_EPSZ2:" << nlsm::NLSM_ID_EPSY2 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_R1:" << nlsm::NLSM_ID_R1 << NRM << std::endl;
        sout << YLW << "\tNLSM_ID_R2:" << nlsm::NLSM_ID_R2 << NRM << std::endl;
        sout << YLW << "\tNLSM_ID_NU1:" << nlsm::NLSM_ID_NU1 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_NU2:" << nlsm::NLSM_ID_NU2 << NRM
             << std::endl;
        sout << YLW << "\tNLSM_ID_OMEGA:" << nlsm::NLSM_ID_OMEGA << NRM
             << std::endl;

        sout << YLW << "\tNLSM_DIM :" << nlsm::NLSM_DIM << NRM << std::endl;
        sout << YLW << "\tNLSM_MAXDEPTH :" << nlsm::NLSM_MAXDEPTH << NRM
             << std::endl;
        sout << YLW << "\tNLSM_MINDEPTH :" << nlsm::NLSM_MINDEPTH << NRM
             << std::endl;

        sout << YLW << "\tNLSM_NUM_REFINE_VARS :" << nlsm::NLSM_NUM_REFINE_VARS
             << NRM << std::endl;
        ...
        sout << nlsm::NLSM_REFINE_VARIABLE_INDICES[nlsm::NLSM_NUM_REFINE_VARS -
                                                   1]
             << "]" << NRM << std::endl;

        sout << YLW << "\tNLSM_NUM_EVOL_VARS_VTU_OUTPUT :"
             << nlsm::NLSM_NUM_EVOL_VARS_VTU_OUTPUT << NRM << std::endl;
        sout << YLW << "\tNLSM_VTU_OUTPUT_EVOL_INDICES :[";
        for (unsigned int i = 0; i < nlsm::NLSM_NUM_EVOL_VARS_VTU_OUTPUT - 1;
             i++)
            sout << nlsm::NLSM_VTU_OUTPUT_EVOL_INDICES[i] << ", ";
        sout << nlsm::NLSM_VTU_OUTPUT_EVOL_INDICES
                    [nlsm::NLSM_NUM_EVOL_VARS_VTU_OUTPUT - 1]
             << "]" << NRM << std::endl;
    }
}
...
void analyticalSol(const double xx1, const double yy1, const double zz1,
    ...
    if (nlsm::NLSM_ID_TYPE == 0) {
        ...
        double rt =
            sqrt(epsx * (x - xc) * (x - xc) + epsy * (y - yc) * (y - yc) +
                 epsz * (z - zc) * (z - zc));
        double rt_pt = sqrt(epsx * (x + nlsm::NLSM_WAVE_SPEED_X * t - xc) *
                                (x + nlsm::NLSM_WAVE_SPEED_X * t - xc) +
                            epsy * (y + nlsm::NLSM_WAVE_SPEED_Y * t - yc) *
                                (y + nlsm::NLSM_WAVE_SPEED_Y * t - yc) +
                            epsz * (z + nlsm::NLSM_WAVE_SPEED_Z * t - zc) *
                                (z + nlsm::NLSM_WAVE_SPEED_Z * t - zc));
        double rt_mt = sqrt(epsx * (x - nlsm::NLSM_WAVE_SPEED_X * t - xc) *
                                (x - nlsm::NLSM_WAVE_SPEED_X * t - xc) +
                            epsy * (y - nlsm::NLSM_WAVE_SPEED_Y * t - yc) *
                                (y - nlsm::NLSM_WAVE_SPEED_Y * t - yc) +
                            epsz * (z - nlsm::NLSM_WAVE_SPEED_Z * t - zc) *
                                (z - nlsm::NLSM_WAVE_SPEED_Z * t - zc));
        chi          = 0.5 * amp *
              (exp(-(rt_pt - R) * (rt_pt - R) / (delta * delta)) +
               exp(-(rt_mt - R) * (rt_mt - R) / (delta * delta)));
        phi = 0.0;
        ...
        const double R     = 0.0;
        double rt = (epsx * (x - xc) * (x - xc) + epsy * (y - yc) * (y - yc) +
                     epsz * (z - zc) * (z - zc));
        double rt_plus_t  = (epsx * (x + t - xc) * (x + t - xc) +
                            epsy * (y + t - yc) * (y + t - yc) +
                            epsz * (z + t - zc) * (z + t - zc));
        double rt_minus_t = (epsx * (x - t - xc) * (x - t - xc) +
                             epsy * (y - t - yc) * (y - t - yc) +
                             epsz * (z - t - zc) * (z - t - zc));
        ...
        if (rt1 < 1.0e-7) {
            ...
            chi = amp1 *
                  exp(-(rt1 - R1 - t) * (rt1 - R1 - t) / (delta1 * delta1)) /
                  rt1;
