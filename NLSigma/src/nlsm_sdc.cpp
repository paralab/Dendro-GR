...
int main(int argc, char** argv) {
    ...
    ts::SDC<DendroScalar, nlsm::NLSMCtx>* sdc_solver =
        new ts::SDC<DendroScalar, nlsm::NLSMCtx>();
    sdc_solver->set_application_ctx(appCtx);
    sdc_solver->set_time_integration_order(1);

    for (sdc_solver->init(); sdc_solver->curr_time() < nlsm::NLSM_RK45_TIME_END;
         sdc_solver->evolve())
    // for(sdc_solver->init(); sdc_solver->curr_step() < 2;
    // sdc_solver->evolve())
    {
        ...
        if (!rank_global)
            std::cout << "[ETS] : Executing step :  " << sdc_solver->curr_step()
                      << "\tcurrent time :" << sdc_solver->curr_time()
                      << "\t dt:" << sdc_solver->ts_size() << "\t" << std::endl;
