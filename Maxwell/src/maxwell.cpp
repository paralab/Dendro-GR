...
int main(int argc, char** argv) {
    ...
    if (maxwell::MAXWELL_NUM_VARS % maxwell::MAXWELL_ASYNC_COMM_K != 0) {
        if (!rank)
            std::cout
                << "[overlap communication error]: total MAXWELL_NUM_VARS: "
                << maxwell::MAXWELL_NUM_VARS
                << " is not divisable by MAXWELL_ASYNC_COMM_K: "
                << maxwell::MAXWELL_ASYNC_COMM_K << std::endl;
        exit(0);
    }

    maxwell::MAXWELL_RK45_TIME_STEP_SIZE =
        maxwell::MAXWELL_CFL_FACTOR *
        (maxwell::MAXWELL_COMPD_MAX[0] - maxwell::MAXWELL_COMPD_MIN[0]) *
        (1.0 / (double)(1u << maxwell::MAXWELL_MAXDEPTH));
    ...
    std::function<void(double, double, double, double*)> f_init =
        [](double x, double y, double z, double* var) {
            maxwell::initData(x, y, z, var);
        };
    // std::function<void(double,double,double,double*)> f_init=[](double
    // x,double y,double z,double*var){maxwell::KerrSchildData(x,y,z,var);};
    ...
    if (maxwell::MAXWELL_ENABLE_BLOCK_ADAPTIVITY) {
        if (!rank)
            std::cout << YLW << "Using block adaptive mesh. AMR disabled "
                      << NRM << std::endl;
        const Point pt_min(maxwell::MAXWELL_BLK_MIN_X,
                           maxwell::MAXWELL_BLK_MIN_Y,
                           maxwell::MAXWELL_BLK_MIN_Z);
        const Point pt_max(maxwell::MAXWELL_BLK_MAX_X,
                           maxwell::MAXWELL_BLK_MAX_Y,
                           maxwell::MAXWELL_BLK_MAX_Z);

        maxwell::blockAdaptiveOctree(tmpNodes, pt_min, pt_max, m_uiMaxDepth - 2,
                                     m_uiMaxDepth, comm);
    } else {
        if (!rank)
            std::cout << YLW << "Using function2Octree. AMR enabled " << NRM
                      << std::endl;
        function2Octree(f_init, maxwell::MAXWELL_NUM_VARS, varIndex, interpVars,
                        tmpNodes, m_uiMaxDepth, maxwell::MAXWELL_WAVELET_TOL,
                        maxwell::MAXWELL_ELE_ORDER, comm);
    }
    ...
    const unsigned int grainSz =
        maxwell::MAXWELL_DENDRO_GRAIN_SZ;  // DENDRO_DEFAULT_GRAIN_SZ;
    ...
    shrinkOrExpandOctree(tmpNodes, maxwell::MAXWELL_LOAD_IMB_TOL,
    ...
    if (isActive) {
        ...
        SFC::parSort::SFC_treeSort(tmpNodes, tmpVec, tmpVec, tmpVec,
                                   maxwell::MAXWELL_LOAD_IMB_TOL, m_uiMaxDepth,
                                   root, ROOT_ROTATION, 1, TS_REMOVE_DUPLICATES,
                                   maxwell::MAXWELL_SPLIT_FIX, commActive);
    ...
    ot::Mesh* mesh =
        new ot::Mesh(balOct, 1, maxwell::MAXWELL_ELE_ORDER, comm,
    ...
    ode::solver::RK4_MAXWELL rk_maxwell(mesh, maxwell::MAXWELL_RK45_TIME_BEGIN,
                                        maxwell::MAXWELL_RK45_TIME_END,
                                        maxwell::MAXWELL_RK45_TIME_STEP_SIZE);
    // ode::solver::RK3_MAXWELL
    // rk_maxwell(mesh,maxwell::MAXWELL_RK45_TIME_BEGIN,maxwell::MAXWELL_RK45_TIME_END,maxwell::MAXWELL_RK45_TIME_STEP_SIZE);

    if (maxwell::MAXWELL_RESTORE_SOLVER == 1)
        rk_maxwell.restoreCheckPoint(maxwell::MAXWELL_CHKPT_FILE_PREFIX.c_str(),
                                     comm);
