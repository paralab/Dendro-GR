...
RK4_MAXWELL::RK4_MAXWELL(ot::Mesh* pMesh, double pTBegin, double pTEnd,
                         double pTh)
    ...
    for (unsigned int index = 0; index < maxwell::MAXWELL_ASYNC_COMM_K;
         index++) {
            ...
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    ...
                    if (nodeLookUp_CG >= nodeLocalBegin &&
                        ...
                        for (unsigned int v = 0; v < maxwell::MAXWELL_NUM_VARS;
                             v++)
...
void RK4_MAXWELL::writeToVTU(double** evolZipVarIn, double** constrZipVarIn,
    ...
    std::function<void(double, double, double, double, double*)> u_x_t =
        [](double t, double x, double y, double z, double* var) {
            maxwell::solToroidalDipole(t, x, y, z, var);
        };
            ...
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    ...
                    if (nodeLookUp_CG >= nodeLocalBegin &&
                        ...
                        u_x_t(m_uiCurrentTime, (double)x, (double)y, (double)z,
                              var);
                        Axdf[nodeLookUp_CG] =
                            var[maxwell::VAR::U_AX] -
                            evolZipVarIn[maxwell::VAR::U_AX][nodeLookUp_CG];
                        Aydf[nodeLookUp_CG] =
                            var[maxwell::VAR::U_AY] -
                            evolZipVarIn[maxwell::VAR::U_AY][nodeLookUp_CG];
                        Axdf[nodeLookUp_CG] =
                            var[maxwell::VAR::U_EX] -
                            evolZipVarIn[maxwell::VAR::U_EX][nodeLookUp_CG];
                        Aydf[nodeLookUp_CG] =
                            var[maxwell::VAR::U_EY] -
                            evolZipVarIn[maxwell::VAR::U_EY][nodeLookUp_CG];
    ...
    double Axl_min       = vecMin(Axdf + m_uiMesh->getNodeLocalBegin(),
                                  (m_uiMesh->getNumLocalMeshNodes()),
                                  m_uiMesh->getMPICommunicator());
    double Axl_max       = vecMax(Axdf + m_uiMesh->getNodeLocalBegin(),
                                  (m_uiMesh->getNumLocalMeshNodes()),
                                  m_uiMesh->getMPICommunicator());
    double Axl2_norm     = normL2(Axdf + m_uiMesh->getNodeLocalBegin(),
                                  (m_uiMesh->getNumLocalMeshNodes()),
                                  m_uiMesh->getMPICommunicator());
    ...
    double Ayl_min   = vecMin(Aydf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());
    double Ayl_max   = vecMax(Aydf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());
    double Ayl2_norm = normL2(Aydf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());

    double Exl_min   = vecMin(Exdf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());
    double Exl_max   = vecMax(Exdf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());
    double Exl2_norm = normL2(Exdf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());

    double Eyl_min   = vecMin(Eydf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());
    double Eyl_max   = vecMax(Eydf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());
    double Eyl2_norm = normL2(Eydf + m_uiMesh->getNodeLocalBegin(),
                              (m_uiMesh->getNumLocalMeshNodes()),
                              m_uiMesh->getMPICommunicator());
    ...
    if (!m_uiMesh->getMPIRank()) {
        ...
        std::cout << YLW << "\t ||VAR::DIFF AX|| (min, max,l2) : (" << Axl_min
                  << ", " << Axl_max << ", " << Axl2_norm << ")" << NRM
                  << std::endl;
        std::cout << YLW << "\t ||VAR::DIFF AY|| (min, max,l2) : (" << Ayl_min
                  << ", " << Ayl_max << ", " << Ayl2_norm << ")" << NRM
                  << std::endl;
        std::cout << YLW << "\t ||VAR::DIFF EX|| (min, max,l2) : (" << Exl_min
                  << ", " << Exl_max << ", " << Exl2_norm << ")" << NRM
                  << std::endl;
        std::cout << YLW << "\t ||VAR::DIFF EY|| (min, max,l2) : (" << Eyl_min
                  << ", " << Eyl_max << ", " << Eyl2_norm << ")" << NRM
                  << std::endl;
    }
    // initialize diff end
#endif
    ...
    for (unsigned int i = 0; i < numEvolVars; i++) {
        pDataNames.push_back(
            std::string(maxwell::MAXWELL_VAR_NAMES[evolVarIndices[i]]));
    ...
    char fPrefix[256];
    sprintf(fPrefix, "%s_%d", maxwell::MAXWELL_VTU_FILE_PREFIX.c_str(),
            m_uiCurrentStep);

#ifdef MAXWELL_ANALYTIC_SOL_TEST
    io::vtk::mesh2vtuFine(m_uiMesh, fPrefix, 2, fDataNames, fData,
                          (numEvolVars + numConstVars + 8),
...
void RK4_MAXWELL::intergridTransferVars(double**& zipIn,
...
void RK4_MAXWELL::unzipVars_async(double** zipIn, double** uzipOut) {
    ...
    for (unsigned int var = 0; var < maxwell::MAXWELL_NUM_VARS;
         var += maxwell::MAXWELL_ASYNC_COMM_K) {
...
void RK4_MAXWELL::performSingleIteration() {
    ...
    do {
        ...
        if (m_uiMesh->isActive()) {
            ...
            unsigned int bflag;
            double dx, dy, dz;
            const Point pt_min(maxwell::MAXWELL_COMPD_MIN[0],
                               maxwell::MAXWELL_COMPD_MIN[1],
                               maxwell::MAXWELL_COMPD_MIN[2]);
            const Point pt_max(maxwell::MAXWELL_COMPD_MAX[0],
                               maxwell::MAXWELL_COMPD_MAX[1],
                               maxwell::MAXWELL_COMPD_MAX[2]);

            for (unsigned int stage = 0;
                 stage < (maxwell::MAXWELL_RK4_STAGES - 1); stage++) {
#ifdef DEBUG_RK_SOLVER
                if (!rank)
                    std::cout << " stage: " << stage << " begin: " << std::endl;
                for (unsigned int index = 0; index < maxwell::MAXWELL_NUM_VARS;
                     index++)
                ...
                for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                    ...
                    ptmin[0] =
                        GRIDX_TO_X(blkList[blk].getBlockNode().minX()) - 3 * dx;
                    ptmin[1] =
                        GRIDY_TO_Y(blkList[blk].getBlockNode().minY()) - 3 * dy;
                    ptmin[2] =
                        GRIDZ_TO_Z(blkList[blk].getBlockNode().minZ()) - 3 * dz;

                    ptmax[0] =
                        GRIDX_TO_X(blkList[blk].getBlockNode().maxX()) + 3 * dx;
                    ptmax[1] =
                        GRIDY_TO_Y(blkList[blk].getBlockNode().maxY()) + 3 * dy;
                    ptmax[2] =
                        GRIDZ_TO_Z(blkList[blk].getBlockNode().maxZ()) + 3 * dz;

                    maxwellRhs(current_t_adv, m_uiUnzipVarRHS,
                ...
                /*for(unsigned int
                   index=0;index<maxwell::MAXWELL_NUM_VARS;index++) for(unsigned
                ...
                for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd;
                    ...
                    for (unsigned int index = 0;
                         index < maxwell::MAXWELL_NUM_VARS; index++) {
            ...
            current_t_adv =
                current_t + RK4_T[(maxwell::MAXWELL_RK4_STAGES - 1)] * m_uiT_h;

#ifdef DEBUG_RK_SOLVER
            if (!rank)
                std::cout << " stage: " << (maxwell::MAXWELL_RK4_STAGES - 1)
                          << " begin: " << std::endl;
            ...
            for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                ...
                maxwellRhs(current_t_adv, m_uiUnzipVarRHS,
                           (const double**)m_uiUnzipVar, offset, ptmin, ptmax,
                           sz, bflag);
            }
            ...
            zipVars(m_uiUnzipVarRHS,
                    m_uiStage[(maxwell::MAXWELL_RK4_STAGES - 1)]);
            ...
            for (unsigned int node = nodeLocalBegin; node < nodeLocalEnd;
                ...
                for (unsigned int index = 0; index < maxwell::MAXWELL_NUM_VARS;
                    ...
                    for (unsigned int s = 0; s < (maxwell::MAXWELL_RK4_STAGES);
                         s++) {
                        m_uiVar[index][node] +=
...
void RK4_MAXWELL::rkSolve() {
    ...
    std::function<double(double, double, double)> waveletTolFunc =
        [wTol](double x, double y, double z) {
            return maxwell::computeWTol(x, y, z, wTol);
        };
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % maxwell::MAXWELL_CHECKPT_FREQ) == 0)
            storeCheckPoint(maxwell::MAXWELL_CHKPT_FILE_PREFIX.c_str());
        maxwell::timer::t_ioCheckPoint.stop();
        // write sol to vtu.
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % maxwell::MAXWELL_TIME_STEP_OUTPUT_FREQ) == 0) {
            l_min = vecMin(
                m_uiPrevVar[maxwell::VAR::U_AX] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[maxwell::VAR::U_AX] + m_uiMesh->getNodeLocalBegin(),
            ...
            if (!m_uiMesh->getMPIRank()) {
                ...
                std::cout << "\t ||VAR::U_AX|| (min, max) : (" << l_min << ", "
                          << l_max << " ) " << std::endl;
            }

            maxwell::timer::profileInfoIntermediate(
                maxwell::MAXWELL_PROFILE_FILE_PREFIX.c_str(), m_uiMesh,
                m_uiCurrentStep);
        }
        ...
        if ((m_uiCurrentStep % maxwell::MAXWELL_REMESH_TEST_FREQ) == 0) {
            ...
            // **)m_uiPrevVar,refineVarIds,refineNumVars,maxwell::MAXWELL_WAVELET_TOL);
            unzipVars(m_uiPrevVar, m_uiUnzipVar);
#endif

            //                     char fPrefix[256];
            //                     sprintf(fPrefix,"%s_wavelets_%d",maxwell::MAXWELL_VTU_FILE_PREFIX.c_str(),m_uiCurrentStep);
            //                     io::vtk::waveletsToVTU(m_uiMesh,(const double
            //                     **)m_uiPrevVar,(const
            //                     double**)m_uiUnzipVar,refineVarIds,refineNumVars,(const
            //                     char*)fPrefix);
            ...
            if (maxwell::MAXWELL_ENABLE_BLOCK_ADAPTIVITY)
                ...
                isRefine = m_uiMesh->isReMeshUnzip(
                    ...
                    waveletTolFunc, maxwell::MAXWELL_DENDRO_AMR_FAC);
            ...
            if (isRefine) {
                ...
                ot::Mesh* newMesh = m_uiMesh->ReMesh(
                    maxwell::MAXWELL_DENDRO_GRAIN_SZ,
                    maxwell::MAXWELL_LOAD_IMB_TOL, maxwell::MAXWELL_SPLIT_FIX);
                ...
                for (unsigned int stage = 0;
                     stage < maxwell::MAXWELL_RK4_STAGES; stage++)
                    for (unsigned int index = 0;
                         index < maxwell::MAXWELL_NUM_VARS; index++) {
                        delete[] m_uiStage[stage][index];
                ...
                if (m_uiMesh->isActive()) {
                    l_min = vecMin(m_uiPrevVar[maxwell::VAR::U_AX] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    l_max = vecMax(m_uiPrevVar[maxwell::VAR::U_AX] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    if (!(m_uiMesh->getMPIRank())) {
                        std::cout << "transfer completed:    ||VAR::U_AX|| "
                                     "(min, max) : ("
                                  << l_min << ", " << l_max << " ) "
                                  << std::endl;
                    }
        ...
        if ((m_uiCurrentStep % maxwell::MAXWELL_IO_OUTPUT_FREQ) == 0) {
            ...
            writeToVTU(m_uiPrevVar, m_uiConstraintVars,
                       maxwell::MAXWELL_NUM_EVOL_VARS_VTU_OUTPUT, 0,
                       maxwell::MAXWELL_VTU_OUTPUT_EVOL_INDICES, NULL);
            // #endif
        }
...
void RK4_MAXWELL::storeCheckPoint(const char* fNamePrefix) {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiCurrentStep % (2 * maxwell::MAXWELL_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
        ...
        if (!rank) {
            ...
            checkPoint["DENDRO_RK45_WAVELET_TOLERANCE"] =
                maxwell::MAXWELL_WAVELET_TOL;
            checkPoint["DENDRO_RK45_LOAD_IMB_TOLERANCE"] =
                maxwell::MAXWELL_LOAD_IMB_TOL;
...
void RK4_MAXWELL::restoreCheckPoint(const char* fNamePrefix, MPI_Comm comm) {
    ...
    for (unsigned int cpIndex = 0; cpIndex < 2; cpIndex++) {
        ...
        for (unsigned int stage = 0; stage < maxwell::MAXWELL_RK4_STAGES;
             stage++)
            for (unsigned int index = 0; index < maxwell::MAXWELL_NUM_VARS;
                 index++) {
                delete[] m_uiStage[stage][index];
        ...
        if (isActive) {
            ...
            restoreStatus = io::checkpoint::readVecFromFile(
                fName, newMesh, m_uiPrevVar, maxwell::MAXWELL_NUM_VARS);
        }
