...
//DAVE'S CODE STARTS HERE
  ...
  for (unsigned int k = 3; k < nz-3; k++) {
    ...
    for (unsigned int j = 3; j < ny-3; j++) {
	 ...
	 double kk = 3.14/5;
         pp = i + nx*(j + ny*k);
	 if( (x*x < 1e-8) && (y < 0.0) && (z*z < 1e-8) && (y > -5.0) ){
		jx[pp] = sin(0.5*t-rr*kk);
	 }
	 else if( ((y-x)*(y-x) < 1e-10) && (z*z < 1e-10) && (x>0) && (y>0) && (x<3) && (y<3) ){
		jx[pp] = sin(0.5*t-rr*kk);
	 }
	 else if( ((y+x)*(y+x) < 1e-10) && (z*z < 1e-10) && (x<0) && (y>0) && (y<3) ){
		jx[pp] = sin(0.5*t-rr*kk);
	 }
	 else{
		jx[pp] = 0;
...
//DAVE'S CODE ENDS HERE
  ...
  for (unsigned int k = 3; k < nz-3; k++) {
    ...
    for (unsigned int j = 3; j < ny-3; j++) {
	 ...
	 EX_rhs[pp] = grad_0_GAM[pp] - (grad2_0_0_AX[pp] + grad2_1_1_AX[pp] + grad2_2_2_AX[pp]) - 4*3.14*jx[pp];
	 EY_rhs[pp] = grad_1_GAM[pp] - (grad2_0_0_AY[pp] + grad2_1_1_AY[pp] + grad2_2_2_AY[pp]);
	 EZ_rhs[pp] = grad_2_GAM[pp] - (grad2_0_0_AZ[pp] + grad2_1_1_AZ[pp] + grad2_2_2_AZ[pp]);

	 GAM_rhs[pp] = -(grad2_0_0_PSI[pp] + grad2_1_1_PSI[pp] + grad2_2_2_PSI[pp]);
  ...
  if (bflag != 0) {
    ...
    maxwell_bcs(GAM_rhs, GAM, grad_0_GAM, grad_1_GAM, grad_2_GAM, pmin, pmax,
             2.0, 0.0, sz, bflag);

    maxwell_bcs(PSI_rhs, PSI, grad_0_PSI, grad_1_PSI, grad_2_PSI, pmin, pmax,
             1.0, 0.0, sz, bflag);
