...
RK4_EM4::RK4_EM4(ot::Mesh *pMesh, double pTBegin, double pTEnd, double pTh)
...
RK4_EM4::~RK4_EM4() {
    ...
    for (unsigned int index = 0; index < em4::EM4_CONSTRAINT_NUM_VARS;
         index++) {
...
void RK4_EM4::initialGridConverge() {
    ...
    do {
        ...
        if (em4::EM4_ENABLE_BLOCK_ADAPTIVITY)
            ...
            isRefine = em4::isRemeshForce(
                m_uiMesh, (const double **)m_uiUnzipVar, em4::VAR::U_B0,
                em4::EM4_CHI_REFINE_VAL, em4::EM4_CHI_COARSEN_VAL, true);
            // m_uiMesh->isReMeshUnzip((const double
            // **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,em4::EM4_DENDRO_AMR_FAC);
        }

        if (isRefine) {
            ot::Mesh *newMesh =
                m_uiMesh->ReMesh(em4::EM4_DENDRO_GRAIN_SZ,
            ...
            for (unsigned int stage = 0; stage < em4::EM4_RK4_STAGES; stage++)
                for (unsigned int index = 0; index < em4::EM4_NUM_VARS;
                     index++) {
                    delete[] m_uiStage[stage][index];
            ...
            if (m_uiMesh->isActive()) {
                double l_min = vecMin(
                    m_uiPrevVar[em4::VAR::U_B0] + m_uiMesh->getNodeLocalBegin(),
...
// void RK4_EM4::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn,
...
void RK4_EM4::writeToVTU(double **evolZipVarIn, double **constrZipVarIn,
            ...
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    ...
                    if (nodeLookUp_CG >= nodeLocalBegin &&
                        ...
                        em4::analyticalSol((double)x, (double)y, (double)z,
                                           m_uiCurrentTime, var);
                        for (unsigned int v = 0; v < em4::EM4_NUM_VARS; v++)
    ...
    for (unsigned int v = 0; v < em4::EM4_NUM_VARS; v++) {
        ...
        if (!m_uiMesh->getMPIRank()) {
            std::cout << " var : " << em4::EM4_VAR_NAMES[v]
    ...
    for (unsigned int i = 0; i < numEvolVars; i++) {
        pDataNames.push_back(
            std::string(em4::EM4_VAR_NAMES[evolVarIndices[i]]));
        pDataNames.push_back(
            "diff" + std::string(em4::EM4_VAR_NAMES[evolVarIndices[i]]));
    ...
    for (unsigned int i = 0; i < numConstVars; i++) {
        pDataNames.push_back(
            std::string(em4::EM4_CONSTRAINT_VAR_NAMES[constVarIndices[i]]));
    ...
    char fPrefix[256];
    sprintf(fPrefix, "%s_%d", em4::EM4_VTU_FILE_PREFIX.c_str(),
            m_uiCurrentStep);
    ...
    for (unsigned int v = 0; v < em4::EM4_NUM_VARS; v++) delete[] diff[v];
...
void RK4_EM4::unzipVars_async(double **zipIn, double **uzipOut) {
    ...
    for (unsigned int var = 0; var < em4::EM4_NUM_VARS;
         var += em4::EM4_ASYNC_COMM_K) {
...
void RK4_EM4::performSingleIteration() {
    if (m_uiMesh->isActive()) {
        ...
        unsigned int bflag;
        double dx, dy, dz;
        const Point pt_min(em4::EM4_COMPD_MIN[0], em4::EM4_COMPD_MIN[1],
                           em4::EM4_COMPD_MIN[2]);
        const Point pt_max(em4::EM4_COMPD_MAX[0], em4::EM4_COMPD_MAX[1],
                           em4::EM4_COMPD_MAX[2]);
        const unsigned int PW = em4::EM4_PADDING_WIDTH;

        for (unsigned int stage = 0; stage < (em4::EM4_RK4_STAGES - 1);
             stage++) {
            ...
            for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                ...
                // "<<(1u<<(m_uiMaxDepth-m_uiMesh->getAllElements()[blkList[blk].getLocalElementBegin()].getLevel()))/(double)em4::EM4_ELE_ORDER<<"
                ...
                em4rhs(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, offset,
...
#ifdef DEBUG_RK_SOLVER
        if (!rank)
            std::cout << " stage: " << (em4::EM4_RK4_STAGES - 1)
                      << " begin: " << std::endl;
        for (unsigned int index = 0; index < em4::EM4_NUM_VARS; index++)
            ot::test::isUnzipNaN(m_uiMesh, m_uiUnzipVar[index]);
#endif
...
void RK4_EM4::rkSolve() {
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % em4::EM4_CHECKPT_FREQ) == 0)
            storeCheckPoint(em4::EM4_CHKPT_FILE_PREFIX.c_str());
        em4::timer::t_ioCheckPoint.stop();
        // write sol to vtu.
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % em4::EM4_TIME_STEP_OUTPUT_FREQ) == 0) {
            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_B0] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            ...
            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_B1] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em4::VAR::U_B1] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_B2] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em4::VAR::U_B2] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_E0] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em4::VAR::U_E0] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_E1] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em4::VAR::U_E1] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_E2] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em4::VAR::U_E2] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_PHI] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em4::VAR::U_PHI] + m_uiMesh->getNodeLocalBegin(),
                ...
                m_uiMesh->getMPICommunicator());
            if (!m_uiMesh->getMPIRank()) {
                std::cout << "\t ||VAR::U_PHI|| (min, max) : (" << l_min << ", "
                          << l_max << " ) " << std::endl;
            }

            l_min = vecMin(
                m_uiPrevVar[em4::VAR::U_PSI] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em4::VAR::U_PSI] + m_uiMesh->getNodeLocalBegin(),
            ...
            em4::timer::profileInfoIntermediate(
                em4::EM4_PROFILE_FILE_PREFIX.c_str(), m_uiMesh,
                m_uiCurrentStep);
        }
        ...
        if ((m_uiCurrentStep % em4::EM4_REMESH_TEST_FREQ) == 0) {
            ...
            if (em4::EM4_ENABLE_BLOCK_ADAPTIVITY)
                ...
                if (em4::EM4_REFINEMENT_MODE == em4::RefinementMode::WAMR)
                    isRefine = m_uiMesh->isReMeshUnzip(
                        ...
                        refineNumVars, waveletTolFunc, em4::EM4_DENDRO_AMR_FAC);

                if (em4::EM4_REFINEMENT_MODE == em4::RefinementMode::FR)
                    isRefine = em4::isRemeshForce(
                        m_uiMesh, (const double **)m_uiUnzipVar, em4::VAR::U_B0,
                        em4::EM4_CHI_REFINE_VAL, em4::EM4_CHI_COARSEN_VAL,
                        true);
                ...
                if (em4::EM4_REFINEMENT_MODE == em4::RefinementMode::WAMR_FR) {
                    const bool isRefine1 = m_uiMesh->isReMeshUnzip(
                        (const double **)m_uiUnzipVar, refineVarIds,
                        refineNumVars, waveletTolFunc, em4::EM4_DENDRO_AMR_FAC);
                    const bool isRefine2 = isRefine = em4::isRemeshForce(
                        m_uiMesh, (const double **)m_uiUnzipVar, em4::VAR::U_B0,
                        em4::EM4_CHI_REFINE_VAL, em4::EM4_CHI_COARSEN_VAL,
                        false);
            ...
            if (isRefine) {
                ...
                for (unsigned int stage = 0; stage < em4::EM4_RK4_STAGES;
                     stage++)
                    for (unsigned int index = 0; index < em4::EM4_NUM_VARS;
                         index++) {
                        delete[] m_uiStage[stage][index];
                ...
                for (unsigned int index = 0;
                     index < em4::EM4_CONSTRAINT_NUM_VARS; index++) {
                    delete[] m_uiConstraintVars[index];
                    delete[] m_uiUnzipConstraintVars[index];
                ...
                if (m_uiMesh->isActive()) {
                    l_min = vecMin(m_uiPrevVar[em4::VAR::U_B0] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    l_max = vecMax(m_uiPrevVar[em4::VAR::U_B0] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    if (!(m_uiMesh->getMPIRank())) {
        ...
        if ((m_uiCurrentStep % em4::EM4_IO_OUTPUT_FREQ) == 0) {
            ...
            if (m_uiMesh->isActive()) {
                ...
                l_min = vecMin(m_uiConstraintVars[em4::VAR_CONSTRAINT::C_DIVE] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                l_max = vecMax(m_uiConstraintVars[em4::VAR_CONSTRAINT::C_DIVE] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                // ewh added l2_norm calc 2020.5.26
                double l2_norm =
                    normL2(m_uiConstraintVars[em4::VAR_CONSTRAINT::C_DIVE] +
                               m_uiMesh->getNodeLocalBegin(),
                           (m_uiMesh->getNumLocalMeshNodes()),
                           m_uiMesh->getMPICommunicator());
                l2_norm = l2_norm / sqrt(total_dof);
                ...
                l_min = vecMin(m_uiConstraintVars[em4::VAR_CONSTRAINT::C_DIVB] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                l_max = vecMax(m_uiConstraintVars[em4::VAR_CONSTRAINT::C_DIVB] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                l2_norm =
                    normL2(m_uiConstraintVars[em4::VAR_CONSTRAINT::C_DIVB] +
                               m_uiMesh->getNodeLocalBegin(),
                           (m_uiMesh->getNumLocalMeshNodes()),
                           m_uiMesh->getMPICommunicator());
                l2_norm = l2_norm / sqrt(total_dof);
                ...
                writeToVTU(m_uiPrevVar, m_uiConstraintVars,
                           ...
                           em4::EM4_VTU_OUTPUT_EVOL_INDICES,
                           em4::EM4_VTU_OUTPUT_CONST_INDICES,
                           em4::EM4_VTU_Z_SLICE_ONLY);
            }
        }
...
void RK4_EM4::storeCheckPoint(const char *fNamePrefix) {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiCurrentStep % (2 * em4::EM4_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
        ...
        io::checkpoint::writeVecToFile(
            fName, m_uiMesh, (const double **)m_uiPrevVar, em4::EM4_NUM_VARS);
    ...
    for (unsigned int cpIndex = 0; cpIndex < 2; cpIndex++) {
        ...
        if (isActive) {
            ...
            restoreStatus = io::checkpoint::readVecFromFile(
                fName, newMesh, m_uiPrevVar, em4::EM4_NUM_VARS);
        }
