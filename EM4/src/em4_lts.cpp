...
int main(int argc, char** argv) {
    ...
    //     EM4_NUM_VARS: "<<em4::EM4_NUM_VARS<<" is not divisable by
    //     EM4_ASYNC_COMM_K: "<<em4::EM4_ASYNC_COMM_K<<std::endl; exit(0);
    // }
    ...
    ot::Mesh* mesh =
        ot::createMesh(tmpNodes.data(), tmpNodes.size(), em4::EM4_ELE_ORDER,
                       comm, 1, ot::SM_TYPE::FDM, em4::EM4_DENDRO_GRAIN_SZ,
                       em4::EM4_LOAD_IMB_TOL, em4::EM4_SPLIT_FIX);

    if (!em4::EM4_ENABLE_BLOCK_ADAPTIVITY)
        ot::meshWAMRConvergence(mesh, f_init, em4::EM4_WAVELET_TOL,
                                em4::EM4_NUM_VARS, em4::EM4_ELE_ORDER,
                                em4::EM4_REFINE_VARIABLE_INDICES,
                                em4::EM4_NUM_REFINE_VARS, 5);
    ...
    if (TS_MODE == 0) {
        ...
        ts::ExplicitNUTS<DendroScalar, em4::EM4Ctx>* enuts =
            new ts::ExplicitNUTS<DendroScalar, em4::EM4Ctx>(appCtx);
        ...
        for (enuts->init(); enuts->curr_time() < em4::EM4_RK45_TIME_END;
            ...
            if (isRemesh) {
                ...
                appCtx->remesh_and_gridtransfer(
                    em4::EM4_DENDRO_GRAIN_SZ, em4::EM4_LOAD_IMB_TOL,
                    em4::EM4_SPLIT_FIX, true, false, false);
                // appCtx->terminal_output();
            }

            enuts->sync_with_mesh();

            if ((step % em4::EM4_IO_OUTPUT_FREQ) == 0) appCtx->write_vtu();

            if ((step % em4::EM4_CHECKPT_FREQ) == 0) appCtx->write_checkpt();
        ...
        // GTS code here.
        em4::EM4Ctx* appCtx = new em4::EM4Ctx(mesh);
        ts::ETS<DendroScalar, em4::EM4Ctx>* ets =
            new ts::ETS<DendroScalar, em4::EM4Ctx>(appCtx);
        ets->set_evolve_vars(appCtx->get_evolution_vars());
        ets->set_ets_coefficients(tsType);

        for (ets->init(); ets->curr_time() < em4::EM4_RK45_TIME_END;
             ets->evolve()) {
