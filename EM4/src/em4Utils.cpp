...
void readParamFile(const char* fName, MPI_Comm comm) {
    ...
    if (!rank) {
        ...
        em4::EM4_VTU_FILE_PREFIX =
            parFile["EM4_VTU_FILE_PREFIX"].get<std::string>();
        em4::EM4_CHKPT_FILE_PREFIX =
            parFile["EM4_CHKPT_FILE_PREFIX"].get<std::string>();
        em4::EM4_PROFILE_FILE_PREFIX =
            parFile["EM4_PROFILE_FILE_PREFIX"].get<std::string>();
        em4::EM4_RESTORE_SOLVER = parFile["EM4_RESTORE_SOLVER"];
        em4::EM4_ENABLE_BLOCK_ADAPTIVITY =
            parFile["EM4_ENABLE_BLOCK_ADAPTIVITY"];
        ...
        if (parFile.find("EM4_REFINEMENT_MODE") != parFile.end())
            em4::EM4_REFINEMENT_MODE = static_cast<em4::RefinementMode>(
                parFile["EM4_REFINEMENT_MODE"]);

        em4::EM4_NUM_REFINE_VARS = parFile["EM4_NUM_REFINE_VARS"];

        for (unsigned int i = 0; i < em4::EM4_NUM_REFINE_VARS; i++)
            em4::EM4_REFINE_VARIABLE_INDICES[i] =
                parFile["EM4_REFINE_VARIABLE_INDICES"][i];

        em4::EM4_NUM_EVOL_VARS_VTU_OUTPUT =
            parFile["EM4_NUM_EVOL_VARS_VTU_OUTPUT"];
        em4::EM4_NUM_CONST_VARS_VTU_OUTPUT =
            parFile["EM4_NUM_CONST_VARS_VTU_OUTPUT"];

        for (unsigned int i = 0; i < em4::EM4_NUM_EVOL_VARS_VTU_OUTPUT; i++)
            em4::EM4_VTU_OUTPUT_EVOL_INDICES[i] =
                parFile["EM4_VTU_OUTPUT_EVOL_INDICES"][i];

        for (unsigned int i = 0; i < em4::EM4_NUM_CONST_VARS_VTU_OUTPUT; i++)
            em4::EM4_VTU_OUTPUT_CONST_INDICES[i] =
                parFile["EM4_VTU_OUTPUT_CONST_INDICES"][i];
    ...
    if (EM4_NUM_REFINE_VARS > EM4_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of refine variables should "
                     "be less than number of EM4_NUM_VARS"
                  << std::endl;
        exit(0);
    }
    if (EM4_NUM_EVOL_VARS_VTU_OUTPUT > EM4_NUM_VARS) {
        std::cout << "Error[parameter file]: Number of evolution VTU variables "
                     "should be less than number of EM4_NUM_VARS"
                  << std::endl;
        exit(0);
    }
    ...
    par::Mpi_Bcast(EM4_VTU_OUTPUT_CONST_INDICES, EM4_CONSTRAINT_NUM_VARS, 0,
                   comm);
}
...
void dumpParamFile(std::ostream& sout, int root, MPI_Comm comm) {
    ...
    if (rank == root) {
        ...
        sout << YLW << "\tEM4_ELE_ORDER :" << em4::EM4_ELE_ORDER << NRM
             << std::endl;
        sout << YLW << "\tEM4_PADDING_WIDTH :" << em4::EM4_PADDING_WIDTH << NRM
             << std::endl;
        sout << YLW << "\tEM4_DIM :" << em4::EM4_DIM << NRM << std::endl;
        sout << YLW << "\tEM4_IO_OUTPUT_FREQ :" << em4::EM4_IO_OUTPUT_FREQ
             << NRM << std::endl;
        sout << YLW << "\tEM4_REMESH_TEST_FREQ :" << em4::EM4_REMESH_TEST_FREQ
             << NRM << std::endl;
        sout << YLW << "\tEM4_CHECKPT_FREQ :" << em4::EM4_CHECKPT_FREQ << NRM
             << std::endl;
        sout << YLW << "\tEM4_RESTORE_SOLVER :" << em4::EM4_RESTORE_SOLVER
             << NRM << std::endl;
        sout << YLW << "\tEM4_ENABLE_BLOCK_ADAPTIVITY :"
             << em4::EM4_ENABLE_BLOCK_ADAPTIVITY << NRM << std::endl;
        sout << YLW << "\tEM4_VTU_FILE_PREFIX :" << em4::EM4_VTU_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW << "\tEM4_CHKPT_FILE_PREFIX :" << em4::EM4_CHKPT_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW
             << "\tEM4_PROFILE_FILE_PREFIX :" << em4::EM4_PROFILE_FILE_PREFIX
             << NRM << std::endl;
        sout << YLW << "\tEM4_VTU_Z_SLICE_ONLY :" << em4::EM4_VTU_Z_SLICE_ONLY
             << NRM << std::endl;
        sout << YLW << "\tEM4_IO_OUTPUT_GAP :" << em4::EM4_IO_OUTPUT_GAP << NRM
             << std::endl;
        sout << YLW << "\tEM4_DENDRO_GRAIN_SZ :" << em4::EM4_DENDRO_GRAIN_SZ
             << NRM << std::endl;
        sout << YLW << "\tEM4_ASYNC_COMM_K :" << em4::EM4_ASYNC_COMM_K << NRM
             << std::endl;
        sout << YLW << "\tEM4_DENDRO_AMR_FAC :" << em4::EM4_DENDRO_AMR_FAC
             << NRM << std::endl;
        sout << YLW << "\tEM4_CFL_FACTOR:" << em4::EM4_CFL_FACTOR << NRM
             << std::endl;
        sout << YLW << "\tEM4_WAVELET_TOL :" << em4::EM4_WAVELET_TOL << NRM
             << std::endl;
        sout << YLW << "\tEM4_LOAD_IMB_TOL :" << em4::EM4_LOAD_IMB_TOL << NRM
             << std::endl;
        sout << YLW << "\tEM4_RK45_TIME_BEGIN :" << em4::EM4_RK45_TIME_BEGIN
             << NRM << std::endl;
        sout << YLW << "\tEM4_RK45_TIME_END :" << em4::EM4_RK45_TIME_END << NRM
             << std::endl;
        sout << YLW
             << "\tEM4_RK45_TIME_STEP_SIZE :" << em4::EM4_RK45_TIME_STEP_SIZE
             << NRM << std::endl;
        sout << YLW << "\tEM4_RK45_DESIRED_TOL :" << em4::EM4_RK45_DESIRED_TOL
             << NRM << std::endl;
        sout << YLW << "\tEM4_COMPD_MIN : ( :" << em4::EM4_COMPD_MIN[0] << " ,"
             << em4::EM4_COMPD_MIN[1] << "," << em4::EM4_COMPD_MIN[2] << " )"
             << NRM << std::endl;
        sout << YLW << "\tEM4_COMPD_MAX : ( :" << em4::EM4_COMPD_MAX[0] << " ,"
             << em4::EM4_COMPD_MAX[1] << "," << em4::EM4_COMPD_MAX[2] << " )"
             << NRM << std::endl;
        sout << YLW << "\tEM4_BLK_MIN : ( :" << em4::EM4_BLK_MIN_X << " ,"
             << em4::EM4_BLK_MIN_Y << "," << em4::EM4_BLK_MIN_Z << " )" << NRM
             << std::endl;
        sout << YLW << "\tEM4_BLK_MAX : ( :" << em4::EM4_BLK_MAX_X << " ,"
             << em4::EM4_BLK_MAX_Y << "," << em4::EM4_BLK_MAX_Z << " )" << NRM
             << std::endl;
        sout << YLW << "\tEM4_OCTREE_MIN : ( :" << em4::EM4_OCTREE_MIN[0]
             << " ," << em4::EM4_OCTREE_MIN[1] << "," << em4::EM4_OCTREE_MIN[2]
             << " )" << NRM << std::endl;
        sout << YLW << "\tEM4_OCTREE_MAX : ( :" << em4::EM4_OCTREE_MAX[0]
             << " ," << em4::EM4_OCTREE_MAX[1] << "," << em4::EM4_OCTREE_MAX[2]
             << " )" << NRM << std::endl;
        sout << YLW << "\tKO_DISS_SIGMA :" << em4::KO_DISS_SIGMA << NRM
             << std::endl;
        sout << YLW << "\tEM4_ID_TYPE:" << em4::EM4_ID_TYPE << NRM << std::endl;
        sout << YLW << "\tEM4_ID_AMP1:" << em4::EM4_ID_AMP1 << NRM << std::endl;
        sout << YLW << "\tEM4_ID_LAMBDA1:" << em4::EM4_ID_LAMBDA1 << NRM
             << std::endl;
        ...
        // sout<<YLW<<"\tEM4_ID_EPSZ2:"<<em4::EM4_ID_EPSY2<<NRM<<std::endl; //
        // sout<<YLW<<"\tEM4_ID_R1:"<<em4::EM4_ID_R1<<NRM<<std::endl;
        ...
        sout << YLW << "\tEM4_MAXDEPTH :" << em4::EM4_MAXDEPTH << NRM
             << std::endl;

        sout << YLW << "\tEM4_NUM_REFINE_VARS :" << em4::EM4_NUM_REFINE_VARS
             << NRM << std::endl;
        ...
        sout << em4::EM4_REFINE_VARIABLE_INDICES[em4::EM4_NUM_REFINE_VARS - 1]
             << "]" << NRM << std::endl;

        sout << YLW << "\tEM4_REFINEMENT_MODE :" << em4::EM4_REFINEMENT_MODE
             << NRM << std::endl;

        sout << YLW << "\tEM4_NUM_EVOL_VARS_VTU_OUTPUT :"
        ...
        sout << em4::EM4_VTU_OUTPUT_EVOL_INDICES
