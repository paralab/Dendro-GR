...
void deriv22_xx(double *const DxDxu, const double *const u, const double dx,
    ...
    for (int k = kb; k < ke; k++) {
        for (int j = jb; j < je; j++) {
            for (int i = ib; i < ie; i++) {
                ...
                DxDxu[pp]    = (u[pp - 1] - 2.0 * u[pp] + u[pp + 1]
    ...
    if (bflag & (1u << OCT_DIR_LEFT)) {
        for (int k = kb; k < ke; k++) {
            for (int j = jb; j < je; j++) {
                DxDxu[IDX(3, j, k)] = (u[IDX(3, j, k)] - 2.0 * u[IDX(4, j, k)] +
                                       u[IDX(5, j, k)]) *
                                      idx_sqrd;
            }
        }
...
void deriv22_yy(double *const DyDyu, const double *const u, const double dy,
    ...
    for (int k = kb; k < ke; k++) {
        for (int i = ib; i < ie; i++) {
            for (int j = jb; j < je; j++) {
                const int pp = IDX(i, j, k);
                DyDyu[pp] = (u[pp - nx] - 2.0 * u[pp] + u[pp + nx]) * idy_sqrd;
            }
        }
    }
    ...
    if (bflag & (1u << OCT_DIR_DOWN)) {
        for (int k = kb; k < ke; k++) {
            for (int i = ib; i < ie; i++) {
                DyDyu[IDX(i, 3, k)] = (u[IDX(i, 3, k)] - 2.0 * u[IDX(i, 4, k)] +
                                       u[IDX(i, 5, k)]) *
                                      idy_sqrd;
            }
        }
...
void deriv22_zz(double *const DzDzu, const double *const u, const double dz,
    ...
    if (bflag & (1u << OCT_DIR_BACK)) {
        for (int j = jb; j < je; j++) {
            for (int i = ib; i < ie; i++) {
                DzDzu[IDX(i, j, 3)] = (u[IDX(i, j, 3)] - 2.0 * u[IDX(i, j, 4)] +
                                       u[IDX(i, j, 5)]) *
                                      idz_sqrd;
            }
        }
