...
void readParamFile(const char* fName, MPI_Comm comm) {
    ...
    if (!rank) {
        ...
        fluid::FLUID_VTU_FILE_PREFIX =
            parFile["FLUID_VTU_FILE_PREFIX"].get<std::string>();
        fluid::FLUID_CHKPT_FILE_PREFIX =
            parFile["FLUID_CHKPT_FILE_PREFIX"].get<std::string>();
        fluid::FLUID_PROFILE_FILE_PREFIX =
            parFile["FLUID_PROFILE_FILE_PREFIX"].get<std::string>();
        fluid::FLUID_RESTORE_SOLVER = parFile["FLUID_RESTORE_SOLVER"];
        fluid::FLUID_ENABLE_BLOCK_ADAPTIVITY =
            parFile["FLUID_ENABLE_BLOCK_ADAPTIVITY"];
        ...
        for (unsigned int i = 0; i < fluid::FLUID_NUM_REFINE_VARS; i++)
            fluid::FLUID_REFINE_VARIABLE_INDICES[i] =
                parFile["FLUID_REFINE_VARIABLE_INDICES"][i];

        fluid::FLUID_NUM_EVOL_VARS_VTU_OUTPUT =
            parFile["FLUID_NUM_EVOL_VARS_VTU_OUTPUT"];

        for (unsigned int i = 0; i < fluid::FLUID_NUM_EVOL_VARS_VTU_OUTPUT; i++)
            fluid::FLUID_VTU_OUTPUT_EVOL_INDICES[i] =
                parFile["FLUID_VTU_OUTPUT_EVOL_INDICES"][i];
    ...
    if (FLUID_NUM_REFINE_VARS > FLUID_NUM_PRIM_VARS) {
        std::cout << "Error[parameter file]: Number of refine variables should "
                     "be less than number of FLUID_NUM_PRIM_VARS"
                  << std::endl;
        exit(0);
    }
    if (FLUID_NUM_EVOL_VARS_VTU_OUTPUT > FLUID_NUM_PRIM_VARS) {
        std::cout << "Error[parameter file]: Number of evolution VTU variables "
                     "should be less than number of FLUID_NUM_PRIM_VARS"
                  << std::endl;
        exit(0);
    }
...
void initData(const DendroScalar xx1, const DendroScalar yy1,
    ...
    // std::cout<<"initData: type="<<fluid::FLUID_ID_TYPE<<", "<<x<<", "<<y<<",
    // "<<z<<std::endl;

    if (fluid::FLUID_ID_TYPE == 0) {
        // For the sake of testing, let's just put a Gaussian here for the time
        // being.
        ...
        var[PVAR::V_VZ]  = 0.0;
        var[PVAR::V_P] =
            FLUID_VACUUM_TAU_RESET + 100.0 * pow(var[PVAR::V_RHO], FLUID_GAMMA);
        ...
        double k      = 2.0;
        double r0     = 0.01;
        double ls     = pow((3.0 - k) * E / (4 * M_PI * pow(r0, k) * rho0),
                            1.0 / (3.0 - k));
        ...
        double R      = 0.10;
        double Ws     = sqrt(pow(R, k) * (17.0 - 4.0 * k) * E /
                             (8 * M_PI * Ak * t * t * t));
        ...
        // If we are effectively at the shock, then it needs to be represented
        // in our data.
        ...
        if (fabs(r - R) < 1e-15) {
            ...
            double vt        = sqrt(W * W - 1.0) / W;
            // Set the density
            var[PVAR::V_RHO] = pow(2.0, 1.5) * Ak * pow(R, -k) * Ws *
                               pow(chi, -(10.0 - 3.0 * k) / (2.0 * (4.0 - k)));
            // Set the pressure
        ...
        if (r < rmin) {
            var[PVAR::V_RHO] =
                FLUID_VACUUM_RESET + rho0 * std::pow(r0 / rmin, k0);
        } else if (r < r0) {
        ...
        if (r < rexp) {
            var[PVAR::V_P] =
                FLUID_VACUUM_TAU_RESET + var[PVAR::V_RHO] * e0 / 3.0;
        } else {
        ...
        if (r < rmin) {
            ...
            switch (angle_dependence) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    // Shell is dipolar and open along the poles with an
                    // additional octopole angle dependence around the equator.
                    double cosphi = x / s;
                    Ashell        = 1.0 + (A - 1.0) * (s / r) * (s / r) *
                                       pow(2.5 * cosphi * cosphi * cosphi -
                                               1.5 * cosphi,
                                           2);
                    break;
            }
            ...
            var[PVAR::V_RHO] =
                FLUID_VACUUM_RESET + Ashell * rho0 * std::pow(r0 / r, k);
