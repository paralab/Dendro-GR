...
namespace ode
{
    namespace solver
    {
        ...
        RK_FLUID::RK_FLUID(ot::Mesh *pMesh, double pTBegin, double pTEnd,double pTh,RKType rkType): RK(pMesh,pTBegin,pTEnd,pTh)
        {
            ...
            for(unsigned int index=0;index<fluid::FLUID_NUM_EVOL_VARS;index++)
            {
                ...
                m_uiEvolUnzipVar[index]=m_uiMesh->createUnZippedVector<DendroScalar>();
                m_uiEvolUnzipVarRHS[index]=m_uiMesh->createUnZippedVector<DendroScalar>();
            }
            ...
            for(unsigned int index=0;index<fluid::FLUID_NUM_PRIM_VARS;index++)
            {
                ...
                m_uiPrimUnzipVar[index]=m_uiMesh->createUnZippedVector<DendroScalar>();
            }
            ...
            for(unsigned int stage=0;stage<m_uiNumRKStages;stage++)
            {
                ...
                for(unsigned int index=0;index<fluid::FLUID_NUM_EVOL_VARS;index++)
                    m_uiEvolStage[stage][index]=m_uiMesh->createVector<DendroScalar>();
        ...
        void RK_FLUID::applyInitialConditions(DendroScalar** zipIn,DendroScalar** zipPrimIn)
        {
                    ...
                    for(unsigned int j=0;j<(eleOrder+1);j++ )
                        for(unsigned int i=0;i<(eleOrder+1);i++)
                        {
                            ...
                            if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                            {
                                ...
                                fluid::initData((DendroScalar)x,(DendroScalar)y,(DendroScalar)z,pvar);
                                // Calculate the metric and apply prim to con.
                                const double pos[] = {GRIDX_TO_X((DendroScalar)x), GRIDY_TO_Y((DendroScalar)y), GRIDZ_TO_Z((DendroScalar)z)};
                                metric_vars(gu,&detg,gd,pos);
                                sdetg=sqrt(detg);
                                fluid_math::prim_to_con_pt(pvar,var,gd,sdetg);

                                for(unsigned int v=0;v<fluid::FLUID_NUM_EVOL_VARS;v++){
                                    zipIn[v][nodeLookUp_CG]=var[v];
                                }

                                for(unsigned int v=0;v<fluid::FLUID_NUM_PRIM_VARS;v++){
        ...
        void RK_FLUID::initialGridConverge()
        {
            ...
            std::function<double(double,double,double)> waveletTolFunc =[wTol](double x,double y, double z){ return fluid::computeWTol(x,y,z,wTol);};
            unsigned int iterCount=1;
            do
            {

                unzipVars_async(m_uiEvolPrevVar,m_uiEvolUnzipVar,fluid::FLUID_NUM_EVOL_VARS);
                ...
                if(fluid::FLUID_ENABLE_BLOCK_ADAPTIVITY)
                    ...
                    //isRefine=m_uiMesh->isReMeshUnzip((const double **)m_uiEvolUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,fluid::FLUID_DENDRO_AMR_FAC);
                    isRefine=fluid::computeFluidRemeshFlags(m_uiMesh, refine_flags, 
                              (const double**)m_uiPrimUnzipVar, refineVarIds, refineNumVars, 
                              waveletTolFunc, 0.0*fluid::FLUID_DENDRO_AMR_FAC);
                    m_uiMesh->setMeshRefinementFlags(refine_flags);
                }

                if(isRefine)
                {
                    ot::Mesh* newMesh=m_uiMesh->ReMesh(fluid::FLUID_DENDRO_GRAIN_SZ,fluid::FLUID_LOAD_IMB_TOL,fluid::FLUID_SPLIT_FIX);
                    ...
                    intergridTransferVars(m_uiEvolPrevVar,newMesh,fluid::FLUID_NUM_EVOL_VARS);
                    ...
                    for(unsigned int index=0;index<fluid::FLUID_NUM_EVOL_VARS;index++)
                    {
                        ...
                        m_uiEvolUnzipVar[index]=newMesh->createUnZippedVector<DendroScalar>();
                        m_uiEvolUnzipVarRHS[index]=newMesh->createUnZippedVector<DendroScalar>();
                    ...
                    for(unsigned int index=0;index<fluid::FLUID_NUM_PRIM_VARS;index++)
                    {
                        ...
                        m_uiPrimUnzipVar[index]=newMesh->createUnZippedVector<DendroScalar>();
                    ...
                    for(unsigned int stage=0;stage<m_uiNumRKStages;stage++)
                    {
                        for(unsigned int index=0;index<fluid::FLUID_NUM_EVOL_VARS;index++)
                        {
                            ...
                            m_uiEvolStage[stage][index]=newMesh->createVector<DendroScalar>();
                        }
                    ...
                    if(m_uiMesh->isActive())
                    {
                        double l_min=vecMin(m_uiEvolPrevVar[fluid::PVAR::V_RHO]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        double l_max=vecMax(m_uiEvolPrevVar[fluid::PVAR::V_RHO]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        if(!(m_uiMesh->getMPIRank())) {
                            std::cout << "transfer completed:    ||PVAR::V_RHO|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                        }
        ...
        void RK_FLUID::writeToVTU(DendroScalar **evolZipVarIn, DendroScalar **primVars, DendroScalar **constrZipVarIn, unsigned int numEvolVars, unsigned int numPrimVars, unsigned int numConstVars, const unsigned int *evolVarIndices, const unsigned int *primVarIndices, const unsigned int *constVarIndices, bool zslice)
        {
            ...
            {
                pDataNames.push_back(std::string(fluid::FLUID_EVOL_VAR_NAMES[evolVarIndices[i]]));
                pData[i]=evolZipVarIn[evolVarIndices[i]];
            }

            for(unsigned int i=0;i<numPrimVars;i++)
            {
                pDataNames.push_back(std::string(fluid::FLUID_PRIM_VAR_NAMES[primVarIndices[i]]));
                pData[numEvolVars+i]=primVars[primVarIndices[i]];
            }

            for(unsigned int i=0;i<numConstVars;i++)
            {
                pDataNames.push_back(std::string(fluid::FLUID_CONS_VAR_NAMES[constVarIndices[i]]));
                pData[numEvolVars+numPrimVars+i]=constrZipVarIn[constVarIndices[i]];
            }
            ...
            char fPrefix[256];
            sprintf(fPrefix,"%s_%d",fluid::FLUID_VTU_FILE_PREFIX.c_str(),m_uiCurrentStep);
            if(!zslice){
              io::vtk::mesh2vtuFine(m_uiMesh,fPrefix,2,fDataNames,fData,(numEvolVars+numPrimVars+numConstVars),(const char **)&pDataNames_char[0],(const double **)pData);
            }
            else{
              unsigned int s_val[3] = { 1u<<(m_uiMaxDepth-1), 1u<<(m_uiMaxDepth-1), 1u<<(m_uiMaxDepth-1)};
              unsigned int s_norm[3] = {0,0,1};
              
              io::vtk::mesh2vtu_slice(m_uiMesh,s_val,s_norm,fPrefix,2,fDataNames,fData,(numEvolVars+numPrimVars+numConstVars),(const char**)&pDataNames_char[0],(const double **)pData);
            }
        ...
        void RK_FLUID::performGhostExchangeVars(DendroScalar** zipIn,unsigned int numVars)
        {
        ...
        void RK_FLUID::intergridTransferVars(DendroScalar**& zipIn, const ot::Mesh* pnewMesh,unsigned int numVars)
        {
        ...
        void RK_FLUID::intergridTransferPrimitiveVars(DendroScalar **& zipPrimIn, ot::Mesh* pnewMesh)
        {
        ...
        void RK_FLUID::unzipVars(DendroScalar** zipIn , DendroScalar** uzipOut,unsigned int numVars)
        {
        ...
        void RK_FLUID::unzipVars_async(DendroScalar ** zipIn , DendroScalar **uzipOut,unsigned int numVars)
        {
        ...
        void RK_FLUID::unzipPrimVars(DendroScalar ** zipIn, DendroScalar** unZipOut)
        {
            ...
            double * zipPrim[]={zipIn[fluid::PVAR::V_RHO], zipIn[fluid::PVAR::V_U1], zipIn[fluid::PVAR::V_U2],zipIn[fluid::PVAR::V_U3], zipIn[fluid::PVAR::V_P] };
            double * unzipPrim[]={unZipOut[fluid::PVAR::V_RHO],unZipOut[fluid::PVAR::V_U1], unZipOut[fluid::PVAR::V_U2],unZipOut[fluid::PVAR::V_U3], unZipOut[fluid::PVAR::V_P] };    
                ...
                for(unsigned int k=0;k<sz[2];k++)
                {
                    ...
                    for(unsigned int j=0;j<sz[1];j++)
                    {
                        ...
                        for(unsigned int i=0;i<sz[0];i++)
                        {
                            ...
                            if(unZipOut[fluid::PVAR::V_RHO][offset+pp] < fluid::FLUID_VACUUM_RESET){
                              unZipOut[fluid::PVAR::V_RHO][offset+pp] = fluid::FLUID_VACUUM_RESET;
                            ...
                            unZipOut[fluid::PVAR::V_P][offset+pp] = 
                              fmax(unZipOut[fluid::PVAR::V_P][offset+pp], fluid::FLUID_VACUUM_TAU_RESET);
        ...
        void RK_FLUID::zipVars(DendroScalar** uzipIn , DendroScalar** zipOut,unsigned int numVars)
        {
        ...
        void RK_FLUID::performSingleIteration()
        {
            ...
            if(m_uiMesh->isActive())
            {
                ...
                // enforce boundary conditions
                fluid::fluid_bcs_cons(m_uiMesh, m_uiEvolUnzipVar);
                fluid::fluid_bcs_prim(m_uiMesh, m_uiPrimUnzipVar); // FIXME: Possibly unnecessary?
                ...
                if(m_uiRKType==RKType::RK3)
                {
                    ...
                    fluid::fluid_rhs(m_uiMesh,current_t_adv,m_uiEvolUnzipVarRHS,(const DendroScalar**)m_uiEvolUnzipVar,(const DendroScalar**)m_uiPrimUnzipVar);
                    zipVars(m_uiEvolUnzipVarRHS,m_uiEvolStage[0],fluid::FLUID_NUM_EVOL_VARS);
                    ...
                    for(unsigned int index=0; index<fluid::FLUID_NUM_EVOL_VARS; index++)
                    {
                        ...
                        for(unsigned int node=nodeLocalBegin; node<nodeLocalEnd; node++)    
                        {
                            m_uiEvolStage[0][index][node]=m_uiEvolPrevVar[index][node] + m_uiT_h * m_uiEvolStage[0][index][node];
                        }

                    }

                    // FIXME: Application of boundary conditions should come before con_to_prim and grid sync.
                    //fluid::fluid_bcs_cons(m_uiMesh,m_uiEvolStage);

                    fluid_math::con_to_prim(m_uiMesh,m_uiEvolStage[0],m_uiPrimVar,badBounds);

                    // unzip both primitive and evolve variables.
                    unzipVars_async(m_uiEvolStage[0],m_uiEvolUnzipVar,fluid::FLUID_NUM_EVOL_VARS);
                    ...
                    fluid::fluid_rhs(m_uiMesh,current_t_adv,m_uiEvolUnzipVarRHS,(const DendroScalar**)m_uiEvolUnzipVar,(const DendroScalar**)m_uiPrimUnzipVar);
                    zipVars(m_uiEvolUnzipVarRHS,m_uiEvolStage[1],fluid::FLUID_NUM_EVOL_VARS);

                    for(unsigned int index=0; index<fluid::FLUID_NUM_EVOL_VARS; index++)
                    {
                        for(unsigned int node=nodeLocalBegin; node<nodeLocalEnd; node++)
                        {
                            m_uiEvolStage[1][index][node]=(0.75)*m_uiEvolPrevVar[index][node] + 0.25*m_uiEvolStage[0][index][node]+m_uiT_h * 0.25 *  m_uiEvolStage[1][index][node];
                        }
                    }

                    // FIXME: Application of boundary conditions should come before con_to_prim and grid sync.

                    fluid_math::con_to_prim(m_uiMesh,m_uiEvolStage[1],m_uiPrimVar,badBounds);
                    // unzip both primitive and evolve variables.
                    unzipVars_async(m_uiEvolStage[1],m_uiEvolUnzipVar,fluid::FLUID_NUM_EVOL_VARS);
                    unzipPrimVars(m_uiPrimVar,m_uiPrimUnzipVar);
                    ...
                    current_t_adv+=m_uiT_h;
                    fluid::fluid_rhs(m_uiMesh,current_t_adv,m_uiEvolUnzipVarRHS,(const DendroScalar**)m_uiEvolUnzipVar,(const DendroScalar**)m_uiPrimUnzipVar);
                    zipVars(m_uiEvolUnzipVarRHS,m_uiEvolVar,fluid::FLUID_NUM_EVOL_VARS);
                    ...
                    for(unsigned int index=0; index<fluid::FLUID_NUM_EVOL_VARS; index++)
                    {
                        ...
                        for(unsigned int node=nodeLocalBegin; node<nodeLocalEnd; node++)
                            m_uiEvolVar[index][node]=(1.0/3.0)*m_uiEvolPrevVar[index][node] + (2.0/3.0)*m_uiEvolStage[1][index][node]+m_uiT_h * (2.0/3.0) *  m_uiEvolVar[index][node];

                    }

                    fluid_math::con_to_prim(m_uiMesh,m_uiEvolVar,m_uiPrimVar,badBounds);

                } else if (m_uiRKType==RKType::RK4)
                {


                    for(unsigned int stage=0; stage<(fluid::FLUID_RK4_STAGES-1); stage++)
                    {
                        // compute fluid rhs (evol vars)
                        fluid::fluid_rhs(m_uiMesh,current_t_adv,m_uiEvolUnzipVarRHS,(const DendroScalar**)m_uiEvolUnzipVar,(const DendroScalar**)m_uiPrimUnzipVar);
                        zipVars(m_uiEvolUnzipVarRHS,m_uiEvolStage[stage],fluid::FLUID_NUM_EVOL_VARS);
                        ...
                        for(unsigned int index=0; index<fluid::FLUID_NUM_EVOL_VARS; index++)
                        {
                            ...
                            for(unsigned int node=nodeLocalBegin; node<nodeLocalEnd; node++)
                            {
                                m_uiEvolVarIm[index][node]=m_uiEvolPrevVar[index][node];
                                m_uiEvolVarIm[index][node] += (RK4_U[stage + 1] * m_uiT_h * m_uiEvolStage[stage][index][node]);
                                /*if(index==fluid::VAR::U_TAU){
                                  if(m_uiEvolVarIm[fluid::VAR::U_TAU][node] < 0.0){
                                    ...
                                    std::cout << "  D = " << D << "\n";
                                    std::cout << "  tau = " << tau << "\n";
                                    std::cout << "  rho = " << m_uiPrimVar[fluid::PVAR::V_RHO][node] << "\n";
                                    std::cout << "  P   = " << m_uiPrimVar[fluid::PVAR::V_P][node] << "\n";
                                    std::cout << "  Ssq = " << Ssq << "\n";
                                    std::cout << "  tau + D " << a << "\n";
                                    std::cout << "  Ssq - (tau + D)^2 " << Ssq - a*a << "\n";
                                    std::cout << "  Ssq - (tau + D)^2 - D^2 " << Ssq - a*a - D*D << "\n";
                       ...
                       fluid_math::con_to_prim(m_uiMesh,m_uiEvolVarIm,m_uiPrimVar,badBounds);

                       // unzip both primitive and evolve variables.
                       unzipVars_async(m_uiEvolVarIm,m_uiEvolUnzipVar,fluid::FLUID_NUM_EVOL_VARS);
                       unzipPrimVars(m_uiPrimVar,m_uiPrimUnzipVar);
                    ...
                    // compute fluid rhs (evol vars)
                    fluid::fluid_rhs(m_uiMesh,current_t_adv,m_uiEvolUnzipVarRHS,(const DendroScalar**)m_uiEvolUnzipVar,(const DendroScalar**)m_uiPrimUnzipVar);
                    zipVars(m_uiEvolUnzipVarRHS,m_uiEvolStage[(fluid::FLUID_RK4_STAGES-1)],fluid::FLUID_NUM_EVOL_VARS);
                    ...
                    for(unsigned int index=0; index<fluid::FLUID_NUM_EVOL_VARS; index++)
                    {
                        ...
                        for(unsigned int node=nodeLocalBegin; node<nodeLocalEnd; node++)
                        {
                            ...
                            for(unsigned int s=0; s<(fluid::FLUID_RK4_STAGES); s++)
                                m_uiEvolVar[index][node]+=(RK4_C[s]*m_uiT_h*m_uiEvolStage[s][index][node]);
                            if(m_uiEvolVarIm[fluid::VAR::U_TAU][node] < 0.0){
                                  double D = m_uiEvolVarIm[fluid::VAR::U_D][node];
                                  double Sx = m_uiEvolVarIm[fluid::VAR::U_SX][node];
                                  double Sy = m_uiEvolVarIm[fluid::VAR::U_SY][node];
                ...
                {   // rk45 solver
                    //@jacob may be you can write this, if not tell me I will write this - Milinda.
        ...
        void RK_FLUID::rkSolve()
        {
            ...
            for(double t=m_uiCurrentTime;t<m_uiTimeEnd;t=t+m_uiT_h)
            {
                ...
                if((m_uiMesh->isActive()) && (m_uiCurrentStep%fluid::FLUID_CHECKPT_FREQ)==0)
                    storeCheckPoint(fluid::FLUID_CHKPT_FILE_PREFIX.c_str());
                fluid::timer::t_ioCheckPoint.stop();
                // write sol to vtu.
                if((m_uiMesh->isActive()) && (m_uiCurrentStep%fluid::FLUID_TIME_STEP_OUTPUT_FREQ)==0)
                {
                    ...
                    for(unsigned int var=0;var<fluid::FLUID_NUM_EVOL_VARS;var++)
                    {
                        l_min=vecMin(m_uiEvolPrevVar[var]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        l_max=vecMax(m_uiEvolPrevVar[var]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        if(!(m_uiMesh->getMPIRank())) {
                            std::cout << "\tconserve variable:    ||"<<fluid::FLUID_EVOL_VAR_NAMES[var]<<"|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                        }
                    }
                    ...
                    for(unsigned int var=0;var<fluid::FLUID_NUM_PRIM_VARS;var++)
                    {
                        l_min=vecMin(m_uiPrimPrevVar[var]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        l_max=vecMax(m_uiPrimPrevVar[var]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        if(!(m_uiMesh->getMPIRank())) {
                           std::cout << "\tprimitive variable:    ||"<<fluid::FLUID_PRIM_VAR_NAMES[var]<<"|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                        }
                    }

                    fluid::timer::profileInfoIntermediate(fluid::FLUID_PROFILE_FILE_PREFIX.c_str(),m_uiMesh,m_uiCurrentStep);
                ...
                if((m_uiCurrentStep%fluid::FLUID_REMESH_TEST_FREQ)==0)
                {
                    ...
                    // enforce boundary conditions
                    fluid::fluid_bcs_cons(m_uiMesh, m_uiEvolUnzipVar);
                    fluid::fluid_bcs_prim(m_uiMesh, m_uiPrimUnzipVar);  //FIXME: Possible unnecessary?
                    ...
                    if(fluid::FLUID_ENABLE_BLOCK_ADAPTIVITY)
                        ...
                        isRefine=fluid::computeFluidRemeshFlags(m_uiMesh, refine_flags, 
                              (const double**)m_uiPrimUnzipVar, refineVarIds, refineNumVars, 
                              waveletTolFunc, fluid::FLUID_DENDRO_AMR_FAC);
                        m_uiMesh->setMeshRefinementFlags(refine_flags);
                    }
                    ...
                    if(isRefine)
                    {
                        ...
                        if(m_uiMesh->isActive())
                        {
                            ...
                            for(unsigned int var=0;var<fluid::FLUID_NUM_EVOL_VARS;var++)
                            {
                                ...
                                l_max=vecMax(m_uiEvolPrevVar[var]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                                if(!(m_uiMesh->getMPIRank())) {
                                    std::cout << "transfer completed:    ||"<<fluid::FLUID_EVOL_VAR_NAMES[var]<<"|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                                }
                            }
                            ...
                            for(unsigned int var=0;var<fluid::FLUID_NUM_PRIM_VARS;var++)
                            {
                                ...
                                l_max=vecMax(m_uiPrimPrevVar[var]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                                if(!(m_uiMesh->getMPIRank())) {
                                    std::cout << "transfer completed:    ||"<<fluid::FLUID_PRIM_VAR_NAMES[var]<<"|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                                }
                            }
                ...
                if((m_uiCurrentStep%fluid::FLUID_IO_OUTPUT_FREQ)==0)
                {
                    ...
                    writeToVTU(m_uiEvolPrevVar,m_uiPrimPrevVar,NULL,fluid::FLUID_NUM_EVOL_VARS,fluid::FLUID_NUM_PRIM_VARS,0,evarIndices,pvarIndices,NULL,fluid::FLUID_VTU_Z_SLICE_ONLY);
        ...
        void RK_FLUID::storeCheckPoint(const char * fNamePrefix)
        {
            ...
            if(m_uiMesh->isActive())
            {
                ...
                (m_uiCurrentStep%(2*fluid::FLUID_CHECKPT_FREQ)==0) ? cpIndex=0 : cpIndex=1; // to support alternate file writing.
                ...
                for(unsigned int i=0;i<numEvolVars;i++)
                {
                    sprintf(fName,"%s_%s_%d_%d.var",fNamePrefix,varEvolNames[i],cpIndex,rank);
                ...
                for(unsigned int i=0;i<numPrimVars;i++)
                {
                    sprintf(fName,"%s_%s_%d_%d.var",fNamePrefix,varPrimNames[i],cpIndex,rank);
                    io::checkpoint::writeVecToFile(fName,m_uiMesh,m_uiPrimPrevVar[i]);
                }

                sprintf(fName,"%s_%d_%d.var",fNamePrefix,cpIndex,rank);
                io::checkpoint::writeVecToFile(fName,m_uiMesh,(const double **)m_uiPrimPrevVar,fluid::FLUID_NUM_EVOL_VARS);
                ...
                if(!rank)
                {
                    ...
                    checkPoint["DENDRO_RK_WAVELET_TOLERANCE"]=fluid::FLUID_WAVELET_TOL;
                    checkPoint["DENDRO_RK_LOAD_IMB_TOLERANCE"]=fluid::FLUID_LOAD_IMB_TOL;
            ...
            for(unsigned int cpIndex=0;cpIndex<2;cpIndex++)
            {
                ...
                if(!rank)
                {
                    ...
                    if(restoreStatus==0)
                    {
                        ...
                        fluid::FLUID_WAVELET_TOL=checkPoint["DENDRO_RK_WAVELET_TOLERANCE"];
                        fluid::FLUID_LOAD_IMB_TOL=checkPoint["DENDRO_RK_LOAD_IMB_TOLERANCE"];
                ...
                if(isActive) {
                    ...
                    restoreStatus=io::checkpoint::readVecFromFile(fName,newMesh,m_uiEvolPrevVar,fluid::FLUID_NUM_EVOL_VARS);
                    restoreStatus=io::checkpoint::readVecFromFile(fName,newMesh,m_uiPrimPrevVar,fluid::FLUID_NUM_PRIM_VARS);
