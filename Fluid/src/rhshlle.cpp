...
// hllerhs {{{
void hlle::rhshlle(double *dtu[], const double *u[], const double *v[], const double *pmin,
  ...
  #pragma message("Primitive variable count hard-coded. Works now, but could break in the future.")
  ...
  if(!check_padding(dtu, sz)){
    printf("An error occurred in the padding points for the righthand side in DIR_X!");
  }
  #endif
  cal_nflux(dtu, u, v, ul1, ur1, vl1, vr1, F1, v1, DIR_Y, pmin, pmax, sz);
  #ifdef DEBUG_FLUX
  if(!check_padding(dtu, sz)){
    printf("An error occurred in the padding points for the righthand side in DIR_Y!");
  }
  #endif
  cal_nflux(dtu, u, v, ul1, ur1, vl1, vr1, F1, v1, DIR_Z, pmin, pmax, sz);
  #ifdef DEBUG_FLUX
  if(!check_padding(dtu, sz)){
    printf("An error occurred in the padding points for the righthand side in DIR_Z!");
  }
  #endif
  ...
  for(unsigned int i = nb; i < sz[0] - nb; i++){
    for(unsigned int j = nb; j < sz[1] - nb; j++){
      for(unsigned int k = nb; k < sz[2] - nb; k++){
        ...
        double Sd[3] = {u[fluid::VAR::U_SX][pp],u[fluid::VAR::U_SY][pp],u[fluid::VAR::U_SZ][pp]};
        double tau = u[fluid::VAR::U_TAU][pp];
        double dtD = dtu[fluid::VAR::U_D][pp];
        double dtSd[3] = {dtu[fluid::VAR::U_SX][pp],dtu[fluid::VAR::U_SY][pp],dtu[fluid::VAR::U_SZ][pp]};
        ...
        // Get the absolute value of S as well as the up components of dtS and S.
...
// cal_nflux {{{
void hlle::cal_nflux(double *dtu[],
               const double   *u[],
               const double   *v[],
               double *ul1[],
               double *ur1[],
  ...
  unsigned int (*index)(unsigned int, unsigned int, unsigned int, unsigned int, unsigned int, unsigned int);
  // A position calculating function.
  void (*cal_pos)(unsigned int, unsigned int, unsigned int,
  ...
  case DIR_Z:
    ...
    break;
  default:
    printf("cal_nflux: dir=%d does not correspond to a valid direction.\n",dir);
    MPI_Abort(MPI_COMM_WORLD,0);
    break;
  }
  // }}}
  ...
  for(unsigned int k = nbp; k < np - nbp; k++){
    ...
    for(unsigned int j = nbp; j < nl - nbp; j++){
      ...
      for(unsigned int m = 0; m < FLUID_NUM_EVOL_VARS; m++){
        ...
        // I think this is not needed. (the behaviour is different with this and without this, hence I reverted it to the previous version. @jacob can you please make sure this is correct. )
        //v1[m][0] = v1[m][1];
        //v1[m][nx-1] = v1[m][nx-2];
      }
      ...
      for(unsigned int i = 0; i < nx; i++){
        double ulpt[FLUID_NUM_EVOL_VARS],urpt[FLUID_NUM_EVOL_VARS],vlpt[FLUID_NUM_EVOL_VARS],vrpt[FLUID_NUM_EVOL_VARS];
...
// cal_source {{{
void hlle::cal_source(double *dtu[],
                const double *u[],
                const double *v[],
                const double *pmin,
                const double *pmax,
...
// lf_flux {{{
void hlle::lf_flux(double *F[], double *ul1[], double *ur1[],
...
// hlle_flux {{{
void hlle::hlle_flux(double *F[], double *ul1[], double *ur1[],
               double *vl1[], double *vr1[],
               const int dir, const double pos_in[], const double dx,
               const unsigned int *sz){
  int n;
  ...
  for(int i = 0; i < n; i++){
    ...
    for(int m = 0; m < FLUID_NUM_EVOL_VARS; m++){
      F[m][i] = ((bp*fl[m] - bm*fr[m]) + bp*bm*(ur[m] - ul[m])) / (bp - bm);
    }
  }
}
// }}} 
...
// flux_x {{{
void hlle::flux_x(double f[], double u[], double v[],
  ...
  f[VAR::U_SX ] = Alpha*(Sd[0]*vu[0] + sdetg*P - Beta[0]/Alpha*Sd[0]);
...
// flux_y {{{
void hlle::flux_y(double f[], double u[], double v[],
            double sdetg, double gd[3][3], double gu[3][3]){
  const double Alpha = 1.0;
  const double Beta[3] = {0.0, 0.0, 0.0};
  ...
  f[VAR::U_SY ] = Alpha*(Sd[1]*vu[1] + sdetg*P - Beta[1]/Alpha*Sd[1]);
...
// flux_z {{{
void hlle::flux_z(double f[], double u[], double v[],
            double sdetg, double gd[3][3], double gu[3][3]){
  const double Alpha = 1.0;
  const double Beta[3] = {0.0, 0.0, 0.0};
  ...
  f[VAR::U_SZ ] = Alpha*(Sd[2]*vu[2] + sdetg*P - Beta[2]/Alpha*Sd[2]);
...
int hlle::cal_max_speeds(double *bp, double *bm, double vl[], double vr[],
  ...
  double Lpl = Alpha/(1.0-vlsq*cslsq)*(vlk*(1.0-cslsq) + sqrt(dis)) - betak;
  double Lml = Alpha/(1.0-vlsq*cslsq)*(vlk*(1.0-cslsq) - sqrt(dis)) - betak;

  dis = csrsq*(1.0-vrsq)*(hkk*(1.0-vrsq*csrsq) - vrk*vrk*(1.0-csrsq));
  if (dis < 0.0){
    return -1;
  }
  double Lpr = Alpha/(1.0-vrsq*csrsq)*(vrk*(1.0-csrsq) + sqrt(dis)) - betak;
  double Lmr = Alpha/(1.0-vrsq*csrsq)*(vrk*(1.0-csrsq) - sqrt(dis)) - betak;
...
// default_speeds {{{
void hlle::default_speeds(double *bp, double *bm, double gu[3][3], const int dir){
  const double Alpha = 1.0;
  const double Beta[3] = {0.0, 0.0, 0.0};
    ...
    for(unsigned int k = 0; k < sz[2]; k++){
      for(unsigned int j = 0; j < sz[1]; j++){
        for(unsigned int i = 0; i < sz[0]; i++){
          ...
          if(i >= nb && i < sz[0]-nb && j >= nb && j < sz[1]-nb && k >= nb && k < sz[2]-nb){
            continue;
          }
          if(dtu[m][pp] != 0.0){
