 ...
 *
 *-------------------------------------------------------------------------*/
int froot::rbrac(int (*func)(double *, double, double *), double *x1,
                 double *x2, double *par, int trace) {
    int rc, j;
    double f1, f2;
    ...
    if (*x1 == *x2) {
        if (trace)
            printf("RBRAC.  Initially x1==x2.  x1=%g, x2=%g\n", *x1, *x2);
        return -1;
    }
    ...
    for (j = 0; j < MAXIT; j++) {
        ...
        if (fabs(f1) < fabs(f2)) {
            *x1 += FACTOR * (*x1 - *x2);
            ...
            if (rc < 0) {
                if (trace)
                    printf("RBRAC (loop): x1 out of range, x1=%g, f1=%g\n", *x1,
                           f1);
                return -1;
            }
        } else {
            *x2 += FACTOR * (*x2 - *x1);
            ...
            if (rc < 0) {
                if (trace)
                    printf("RBRAC (loop): x2 out of range, x2=%g, f2=%g\n", *x2,
                           f2);
                return -1;
            }
        }
...
// bisec {{{
int froot::bisec(int (*func)(double *, double, double *), double *rtb,
                 double x1, double x2, double tol, double *fpar, int trace) {
    int rc, j;
    double dx, f, fm, xm;
    const int MAXIT = 100;
 ...
 *
 *-------------------------------------------------------------------------*/
int froot::zbrent(int (*func)(double *, double, double *), double *rts,
                  double x1, double x2, double tol, double *fpar) {
    const int ltrace  = 0;
    const int ltrace2 = 1;
    const int ITMAX   = 100;
    ...
    for (iter = 1; iter <= ITMAX; iter++) {
        ...
        if (fabs(xm) <= tol1 || fb == 0.0) {
            if (fb >= 0.0) {
                ...
                *rts = b;
                printf(
                    "ZBRENT. Failed to distinguish the proper root. Should not "
                    "happen.\n");
                exit(2);
            }
            //*rts = b;
