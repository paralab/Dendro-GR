...
#define GRIDX_TO_X(xg) (((Rx/RgX)*(xg-fluid::FLUID_OCTREE_MIN[0]))+fluid::FLUID_COMPD_MIN[0])
#define GRIDY_TO_Y(yg) (((Ry/RgY)*(yg-fluid::FLUID_OCTREE_MIN[1]))+fluid::FLUID_COMPD_MIN[1])
#define GRIDZ_TO_Z(zg) (((Rz/RgZ)*(zg-fluid::FLUID_OCTREE_MIN[2]))+fluid::FLUID_COMPD_MIN[2])

#define X_TO_GRIDX(xc) (((RgX/Rx)*(xc-fluid::FLUID_COMPD_MIN[0]))+fluid::FLUID_OCTREE_MIN[0])
#define Y_TO_GRIDY(yc) (((RgY/Ry)*(yc-fluid::FLUID_COMPD_MIN[1]))+fluid::FLUID_OCTREE_MIN[1])
#define Z_TO_GRIDZ(zc) (((RgZ/Rz)*(zc-fluid::FLUID_COMPD_MIN[2]))+fluid::FLUID_OCTREE_MIN[2])
...
static const char * FLUID_PRIM_VAR_NAMES[]={"V_RHO","V_VX","V_VY","V_VZ","V_P","V_U1","V_U2","V_U3"};
static const char * FLUID_EVOL_VAR_NAMES[]={"U_D","U_SX","U_SY","U_SZ","U_TAU"};
static const char * FLUID_CONS_VAR_NAMES[]={"C_HAM","C_MOM0","C_MOM1","C_MOM2"};
 ...
 **/
 ...
 /**
  * @brief: Generates block adaptive octree for the given binary blackhole problem.
  ...
  * */
  ...
  double computeFluidWavelet(const ot::Mesh* pMesh, const wavelet::WaveletEl* wRefEl,
                             const T*eleUnzip, double *maxima, double wavelet_tol, unsigned int dof=1, 
                             bool isBdyEle=false);
...
namespace util
{
  ...
  inline unsigned int index_xm(unsigned int i,  unsigned int j,  unsigned int k,
                               unsigned int np, unsigned int nl, unsigned int nx){
    return i + nx*(j + k*nl);
  }
  ...
  inline void cal_pos_xm(unsigned int i, unsigned int j, unsigned int k,
                         double dx, double dl, double dp,
                         double sx, double sl, double sp,
                         double *pos){
    pos[0] = sx + (i - 0.5)*dx;
    pos[1] = sl + j*dl;
    pos[2] = sp + k*dp;
