...
#define MINMOD4(w, x, y, z)                          \
    (0.125 * (copysign(1.0, w) + copysign(1.0, x)) * \
     fabs((copysign(1.0, w) + copysign(1.0, y)) *    \
          (copysign(1.0, w) + copysign(1.0, z))) *   \
     fmin(fabs(w), fmin(fabs(x), fmin(fabs(y), fabs(z)))))
...
static inline double MP5(const double am2, const double am1, const double a,
    ...
    const double vl =
        (2.0 * am2 - 13.0 * am1 + 47.0 * a + 27.0 * ap1 - 3.0 * ap2) / 60.0;
    const double x   = ap1 - a;
    const double y   = mp5_alpha * (a - am1);
    const double vmp = a + (0.5 * (copysign(1.0, x) + copysign(1.0, y)) *
                            fmin(fabs(x), fabs(y)));
    if ((vl - a) * (vl - vmp) <= mp5_eps * anorm) {
        ...
        const double dm4jph =
            MINMOD4(4.0 * dj - djp1, 4.0 * djp1 - dj, dj, djp1);
        const double dm4jmh =
            MINMOD4(4.0 * dj - djm1, 4.0 * djm1 - dj, dj, djm1);
        ...
        const double vmin =
            fmax(fmin(a, fmin(ap1, vmd)), fmin(a, fmin(vul, vlc)));
        const double vmax =
            fmin(fmax(a, fmax(ap1, vmd)), fmax(a, fmax(vul, vlc)));
        return vl +
               (0.5 * (copysign(1.0, vmin - vl) + copysign(1.0, vmax - vl)) *
                fmin(fabs(vmin - vl), fabs(vmax - vl)));
    }
}
}  // namespace mp5
