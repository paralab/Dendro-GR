...
/**
 * @brief Error codes and their meaning for the fluid solvers (i.e. cons to prim
 * and vice versa. )
 */
enum Error {
    ...
    FLUID_CONS_TO_PRIM_RESCALED  // con to prim solver succeeded, but rescaled
                                 // in the process.
};
...
/**
 * @brief compute conserved variables form primitive variables. operates on the
 * zip version of the vector.
...
/**
 * @brief compute conserved variables form primitive variables for a single grid
 * point
...
/***
 * @brief computes the prim vars from conserved variables. (operates on the
 * zipped version. )
 ...
 * @note that in the case of the cons to prim solver failure we use
 * interpolation from neighboring points.
 */
