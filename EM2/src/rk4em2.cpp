...
RK4_EM2::RK4_EM2(ot::Mesh *pMesh, double pTBegin, double pTEnd, double pTh)
...
RK4_EM2::~RK4_EM2() {
    ...
    for (unsigned int index = 0; index < em2::EM2_CONSTRAINT_NUM_VARS;
         index++) {
...
void RK4_EM2::initialGridConverge() {
    ...
    do {
        ...
        if (em2::EM2_ENABLE_BLOCK_ADAPTIVITY)
            ...
            isRefine = em2::isRemeshForce(
                m_uiMesh, (const double **)m_uiUnzipVar, em2::VAR::U_A0,
                em2::EM2_CHI_REFINE_VAL, em2::EM2_CHI_COARSEN_VAL, true);
            // m_uiMesh->isReMeshUnzip((const double
            // **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,em2::EM2_DENDRO_AMR_FAC);
        }

        if (isRefine) {
            ot::Mesh *newMesh =
                m_uiMesh->ReMesh(em2::EM2_DENDRO_GRAIN_SZ,
            ...
            for (unsigned int stage = 0; stage < em2::EM2_RK4_STAGES; stage++)
                for (unsigned int index = 0; index < em2::EM2_NUM_VARS;
                     index++) {
                    delete[] m_uiStage[stage][index];
            ...
            if (m_uiMesh->isActive()) {
                double l_min = vecMin(
                    m_uiPrevVar[em2::VAR::U_A0] + m_uiMesh->getNodeLocalBegin(),
...
// void RK4_EM2::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn,
// unsigned int numEvolVars,unsigned int numConstVars,const unsigned int *
...
void RK4_EM2::writeToVTU(double **evolZipVarIn, double **constrZipVarIn,
            ...
            for (unsigned int j = 0; j < (eleOrder + 1); j++)
                for (unsigned int i = 0; i < (eleOrder + 1); i++) {
                    ...
                    if (nodeLookUp_CG >= nodeLocalBegin &&
                        ...
                        em2::analyticalSol((double)x, (double)y, (double)z,
                                           m_uiCurrentTime, var);
                        for (unsigned int v = 0; v < em2::EM2_NUM_VARS; v++)
    ...
    for (unsigned int v = 0; v < em2::EM2_NUM_VARS; v++) {
        ...
        if (!m_uiMesh->getMPIRank()) {
            std::cout << " var : " << em2::EM2_VAR_NAMES[v]
    ...
    for (unsigned int i = 0; i < numEvolVars; i++) {
        pDataNames.push_back(
            std::string(em2::EM2_VAR_NAMES[evolVarIndices[i]]));
        pDataNames.push_back(
            "diff" + std::string(em2::EM2_VAR_NAMES[evolVarIndices[i]]));
    ...
    for (unsigned int i = 0; i < numConstVars; i++) {
        pDataNames.push_back(
            std::string(em2::EM2_CONSTRAINT_VAR_NAMES[constVarIndices[i]]));
    ...
    char fPrefix[256];
    sprintf(fPrefix, "%s_%d", em2::EM2_VTU_FILE_PREFIX.c_str(),
            m_uiCurrentStep);
    ...
    for (unsigned int v = 0; v < em2::EM2_NUM_VARS; v++) delete[] diff[v];
...
void RK4_EM2::unzipVars_async(double **zipIn, double **uzipOut) {
    ...
    for (unsigned int var = 0; var < em2::EM2_NUM_VARS;
         var += em2::EM2_ASYNC_COMM_K) {
...
void RK4_EM2::performSingleIteration() {
    if (m_uiMesh->isActive()) {
        ...
        unsigned int bflag;
        double dx, dy, dz;
        const Point pt_min(em2::EM2_COMPD_MIN[0], em2::EM2_COMPD_MIN[1],
                           em2::EM2_COMPD_MIN[2]);
        const Point pt_max(em2::EM2_COMPD_MAX[0], em2::EM2_COMPD_MAX[1],
                           em2::EM2_COMPD_MAX[2]);
        const unsigned int PW = em2::EM2_PADDING_WIDTH;

        for (unsigned int stage = 0; stage < (em2::EM2_RK4_STAGES - 1);
             stage++) {
            ...
            for (unsigned int blk = 0; blk < blkList.size(); blk++) {
                ...
                // "<<(1u<<(m_uiMaxDepth-m_uiMesh->getAllElements()[blkList[blk].getLocalElementBegin()].getLevel()))/(double)em2::EM2_ELE_ORDER<<"
                ...
                em2rhs(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, offset,
...
#ifdef DEBUG_RK_SOLVER
        if (!rank)
            std::cout << " stage: " << (em2::EM2_RK4_STAGES - 1)
                      << " begin: " << std::endl;
...
void RK4_EM2::rkSolve() {
    ...
    for (double t = m_uiCurrentTime; t < m_uiTimeEnd; t = t + m_uiT_h) {
        ...
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % em2::EM2_CHECKPT_FREQ) == 0)
            storeCheckPoint(em2::EM2_CHKPT_FILE_PREFIX.c_str());
        em2::timer::t_ioCheckPoint.stop();
        // write sol to vtu.
        if ((m_uiMesh->isActive()) &&
            (m_uiCurrentStep % em2::EM2_TIME_STEP_OUTPUT_FREQ) == 0) {
            l_min = vecMin(
                m_uiPrevVar[em2::VAR::U_E0] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em2::VAR::U_E0] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_min = vecMin(
                m_uiPrevVar[em2::VAR::U_E1] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em2::VAR::U_E1] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_min = vecMin(
                m_uiPrevVar[em2::VAR::U_E2] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em2::VAR::U_E2] + m_uiMesh->getNodeLocalBegin(),
            ...
            l_max = vecMax(
                ...
                m_uiMesh->getMPICommunicator());
            if (!m_uiMesh->getMPIRank()) {
                std::cout << "\t ||VAR::U_A0|| (min, max) : (" << l_min << ", "
                          << l_max << " ) " << std::endl;
            }

            l_min = vecMin(
                m_uiPrevVar[em2::VAR::U_A1] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em2::VAR::U_A1] + m_uiMesh->getNodeLocalBegin(),
                ...
                m_uiMesh->getMPICommunicator());
            if (!m_uiMesh->getMPIRank()) {
                std::cout << "\t ||VAR::U_A1|| (min, max) : (" << l_min << ", "
                          << l_max << " ) " << std::endl;
            }

            l_min = vecMin(
                m_uiPrevVar[em2::VAR::U_A2] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em2::VAR::U_A2] + m_uiMesh->getNodeLocalBegin(),
                ...
                m_uiMesh->getMPICommunicator());
            if (!m_uiMesh->getMPIRank()) {
                std::cout << "\t ||VAR::U_A2|| (min, max) : (" << l_min << ", "
                          << l_max << " ) " << std::endl;
            }

            l_min = vecMin(
                m_uiPrevVar[em2::VAR::U_PSI] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em2::VAR::U_PSI] + m_uiMesh->getNodeLocalBegin(),
                ...
                m_uiMesh->getMPICommunicator());
            if (!m_uiMesh->getMPIRank()) {
                std::cout << "\t ||VAR::U_PSI|| (min, max) : (" << l_min << ", "
                          << l_max << " ) " << std::endl;
            }

            l_min = vecMin(
                m_uiPrevVar[em2::VAR::U_GAMMA] + m_uiMesh->getNodeLocalBegin(),
                (m_uiMesh->getNumLocalMeshNodes()),
                m_uiMesh->getMPICommunicator());
            l_max = vecMax(
                m_uiPrevVar[em2::VAR::U_GAMMA] + m_uiMesh->getNodeLocalBegin(),
                ...
                m_uiMesh->getMPICommunicator());
            if (!m_uiMesh->getMPIRank()) {
                std::cout << "\t ||VAR::U_GAMMA|| (min, max) : (" << l_min
                          << ", " << l_max << " ) " << std::endl;
            }

            em2::timer::profileInfoIntermediate(
                em2::EM2_PROFILE_FILE_PREFIX.c_str(), m_uiMesh,
                m_uiCurrentStep);
        }
        ...
        if ((m_uiCurrentStep % em2::EM2_REMESH_TEST_FREQ) == 0) {
            ...
            if (em2::EM2_ENABLE_BLOCK_ADAPTIVITY)
                ...
                if (em2::EM2_REFINEMENT_MODE == em2::RefinementMode::WAMR)
                    isRefine = m_uiMesh->isReMeshUnzip(
                        ...
                        refineNumVars, waveletTolFunc, em2::EM2_DENDRO_AMR_FAC);

                if (em2::EM2_REFINEMENT_MODE == em2::RefinementMode::FR)
                    isRefine = em2::isRemeshForce(
                        m_uiMesh, (const double **)m_uiUnzipVar, em2::VAR::U_A0,
                        em2::EM2_CHI_REFINE_VAL, em2::EM2_CHI_COARSEN_VAL,
                        true);
                ...
                if (em2::EM2_REFINEMENT_MODE == em2::RefinementMode::WAMR_FR) {
                    const bool isRefine1 = m_uiMesh->isReMeshUnzip(
                        (const double **)m_uiUnzipVar, refineVarIds,
                        refineNumVars, waveletTolFunc, em2::EM2_DENDRO_AMR_FAC);
                    const bool isRefine2 = isRefine = em2::isRemeshForce(
                        m_uiMesh, (const double **)m_uiUnzipVar, em2::VAR::U_A0,
                        em2::EM2_CHI_REFINE_VAL, em2::EM2_CHI_COARSEN_VAL,
                        false);
            ...
            if (isRefine) {
                ...
                for (unsigned int stage = 0; stage < em2::EM2_RK4_STAGES;
                     stage++)
                    for (unsigned int index = 0; index < em2::EM2_NUM_VARS;
                         index++) {
                        delete[] m_uiStage[stage][index];
                ...
                for (unsigned int index = 0;
                     index < em2::EM2_CONSTRAINT_NUM_VARS; index++) {
                    delete[] m_uiConstraintVars[index];
                    delete[] m_uiUnzipConstraintVars[index];
                ...
                if (m_uiMesh->isActive()) {
                    l_min = vecMin(m_uiPrevVar[em2::VAR::U_A0] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    l_max = vecMax(m_uiPrevVar[em2::VAR::U_A0] +
                                       m_uiMesh->getNodeLocalBegin(),
                                   (m_uiMesh->getNumLocalMeshNodes()),
                                   m_uiMesh->getMPICommunicator());
                    if (!(m_uiMesh->getMPIRank())) {
                        std::cout << "transfer completed:    ||VAR::U_A0|| "
                                     "(min, max) : ("
                                  << l_min << ", " << l_max << " ) "
                                  << std::endl;
                    }
        ...
        if ((m_uiCurrentStep % em2::EM2_IO_OUTPUT_FREQ) == 0) {
            ...
            if (m_uiMesh->isActive()) {
                ...
                l_min = vecMin(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVA] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                l_max = vecMax(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVA] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                double l2_norm =
                    normL2(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVA] +
                               m_uiMesh->getNodeLocalBegin(),
                           (m_uiMesh->getNumLocalMeshNodes()),
                           m_uiMesh->getMPICommunicator());
                l2_norm = l2_norm / sqrt(total_dof);
                ...
                if (!m_uiMesh->getMPIRank()) {
                    std::cout << "executing step: " << m_uiCurrentStep
                              << " dt: " << m_uiT_h
                              << " rk_time : " << m_uiCurrentTime << std::endl;
                    std::cout << "\t ||VAR::C_DIVA|| (min, max, l2) : ("
                              << l_min << ", " << l_max << ", " << l2_norm
                              << " ) " << std::endl;
                }

                l_min = vecMin(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVE] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                l_max = vecMax(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVE] +
                                   m_uiMesh->getNodeLocalBegin(),
                               (m_uiMesh->getNumLocalMeshNodes()),
                               m_uiMesh->getMPICommunicator());
                l2_norm =
                    normL2(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVE] +
                               m_uiMesh->getNodeLocalBegin(),
                           (m_uiMesh->getNumLocalMeshNodes()),
                           m_uiMesh->getMPICommunicator());
                l2_norm = l2_norm / sqrt(total_dof);
                ...
                writeToVTU(m_uiPrevVar, m_uiConstraintVars,
                           ...
                           em2::EM2_VTU_OUTPUT_EVOL_INDICES,
                           em2::EM2_VTU_OUTPUT_CONST_INDICES,
                           em2::EM2_VTU_Z_SLICE_ONLY);
            }
        }
...
void RK4_EM2::storeCheckPoint(const char *fNamePrefix) {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiCurrentStep % (2 * em2::EM2_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
        ...
        io::checkpoint::writeVecToFile(
            fName, m_uiMesh, (const double **)m_uiPrevVar, em2::EM2_NUM_VARS);
    ...
    for (unsigned int cpIndex = 0; cpIndex < 2; cpIndex++) {
        ...
        if (isActive) {
            ...
            restoreStatus = io::checkpoint::readVecFromFile(
                fName, newMesh, m_uiPrevVar, em2::EM2_NUM_VARS);
        }
