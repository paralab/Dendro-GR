...
namespace ode
{
    namespace solver
    {
        ...
        RK4_EM2::RK4_EM2(ot::Mesh *pMesh, double pTBegin, double pTEnd,double pTh): RK(pMesh,pTBegin,pTEnd,pTh)
        {

            // allocate memory for the variables.
            m_uiVar=new double*[em2::EM2_NUM_VARS];
            for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                m_uiVar[index]=m_uiMesh->createVector<double>();

            m_uiPrevVar=new double*[em2::EM2_NUM_VARS];
            for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                m_uiPrevVar[index]=m_uiMesh->createVector<double>();

            m_uiVarIm=new double*[em2::EM2_NUM_VARS];
            for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                m_uiVarIm[index]=m_uiMesh->createVector<double>();


            m_uiStage=new double**[em2::EM2_RK4_STAGES];
            for(unsigned int stage=0;stage<em2::EM2_RK4_STAGES;stage++)
            {
                m_uiStage[stage]=new double*[em2::EM2_NUM_VARS];
                for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                    m_uiStage[stage][index]=m_uiMesh->createVector<double>();
            }

            m_uiUnzipVar=new double*[em2::EM2_NUM_VARS];
            for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                m_uiUnzipVar[index]=m_uiMesh->createUnZippedVector<double>();


            m_uiUnzipVarRHS=new double*[em2::EM2_NUM_VARS];
            for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
            ...
            m_uiConstraintVars=new DendroScalar*[em2::EM2_CONSTRAINT_NUM_VARS];
            for(unsigned int index=0; index<em2::EM2_CONSTRAINT_NUM_VARS; index++)
                m_uiConstraintVars[index]=m_uiMesh->createVector<DendroScalar>();

        
            m_uiUnzipConstraintVars=new DendroScalar*[em2::EM2_CONSTRAINT_NUM_VARS];
            for(unsigned int index=0; index<em2::EM2_CONSTRAINT_NUM_VARS; index++)
            ...
            // mpi communication
            m_uiSendNodeBuf=new double*[em2::EM2_ASYNC_COMM_K];
            m_uiRecvNodeBuf=new double*[em2::EM2_ASYNC_COMM_K];

            m_uiSendReqs=new MPI_Request*[em2::EM2_ASYNC_COMM_K];
            m_uiRecvReqs=new MPI_Request*[em2::EM2_ASYNC_COMM_K];
            m_uiSendSts=new MPI_Status*[em2::EM2_ASYNC_COMM_K];
            m_uiRecvSts=new MPI_Status*[em2::EM2_ASYNC_COMM_K];

            for(unsigned int index=0;index<em2::EM2_ASYNC_COMM_K;index++)
            {
        ...
        void RK4_EM2::applyInitialConditions(double** zipIn)
        {
            ...
            double* var=new double[em2::EM2_NUM_VARS];
                    ...
                    for(unsigned int j=0;j<(eleOrder+1);j++ )
                        for(unsigned int i=0;i<(eleOrder+1);i++)
                        {
                            ...
                            if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                            {
                                ...
                                em2::initData((double)x,(double)y,(double)z,var);
                                for(unsigned int v=0;v<em2::EM2_NUM_VARS;v++)
        ...
        void RK4_EM2::initialGridConverge()
        {
            ...
            do
            {
                ...
                {
                    isRefine=em2::isRemeshForce(m_uiMesh,(const double **)m_uiUnzipVar,em2::VAR::U_A0,em2::EM2_CHI_REFINE_VAL,em2::EM2_CHI_COARSEN_VAL,true);
                    //m_uiMesh->isReMeshUnzip((const double **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,em2::EM2_DENDRO_AMR_FAC);
                }
                    

                if(isRefine)
                {
                    ot::Mesh* newMesh=m_uiMesh->ReMesh(em2::EM2_DENDRO_GRAIN_SZ,em2::EM2_LOAD_IMB_TOL,em2::EM2_SPLIT_FIX);
                ...
                for(unsigned int index=0; index<em2::EM2_CONSTRAINT_NUM_VARS; index++)
                {
                    ...
                    if(m_uiMesh->isActive())
                    {
                        double l_min=vecMin(m_uiPrevVar[em2::VAR::U_A0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                        double l_max=vecMax(m_uiPrevVar[em2::VAR::U_A0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
        ...
        void RK4_EM2::reallocateMPIResources()
        {
            for(unsigned int index=0;index<em2::EM2_ASYNC_COMM_K;index++)
            {
                delete [] m_uiSendNodeBuf[index];
        ...
        //void RK4_EM2::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn, unsigned int numEvolVars,unsigned int numConstVars,const unsigned int * evolVarIndices, const unsigned int * constVarIndices)
        void RK4_EM2::writeToVTU(double **evolZipVarIn, double ** constrZipVarIn, unsigned int numEvolVars,unsigned int numConstVars,const unsigned int * evolVarIndices, const unsigned int * constVarIndices, bool zslice)
        {
            if(!m_uiMesh->isActive())
                return;

            em2::timer::t_ioVtu.start();
            double ** diff = new double*[em2::EM2_NUM_VARS];
            for(unsigned int v=0; v < em2::EM2_NUM_VARS;v++)
                    ...
                    for(unsigned int j=0;j<(eleOrder+1);j++ )
                        for(unsigned int i=0;i<(eleOrder+1);i++)
                        {
                            ...
                            if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                            {
                                ...
                                em2::analyticalSol((double)x,(double)y,(double)z,m_uiCurrentTime,var);
                                for(unsigned int v=0; v < em2::EM2_NUM_VARS; v++)
                                    diff[v][nodeLookUp_CG] = evolZipVarIn[v][nodeLookUp_CG] -var[v];
            ...
            if(!m_uiMesh->getMPIRank())
                std::cout<<" time : "<<m_uiCurrentTime<<" step: "<<m_uiCurrentStep<<" difference with analytical "<<std::endl;
            ...
            for(unsigned int v=0; v < em2::EM2_NUM_VARS; v++)
            {
                double l_min=vecMin(diff[v]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                double l_max=vecMax(diff[v]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                double l2_norm=normL2(diff[v]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());

                l2_norm=l2_norm/sqrt(total_dof);

                if(!m_uiMesh->getMPIRank()){
                    std::cout<<" var : "<<em2::EM2_VAR_NAMES[v]<<" (min, max, l2): \t"<<l_min<<"\t"<<l_max<<"\t"<<l2_norm<<std::endl;
                }
            ...
            {
                pDataNames.push_back(std::string(em2::EM2_VAR_NAMES[evolVarIndices[i]]));
                pDataNames.push_back("diff" + std::string(em2::EM2_VAR_NAMES[evolVarIndices[i]]));
            ...
            {
                pDataNames.push_back(std::string(em2::EM2_CONSTRAINT_VAR_NAMES[constVarIndices[i]]));
            ...
            char fPrefix[256];
            sprintf(fPrefix,"%s_%d",em2::EM2_VTU_FILE_PREFIX.c_str(),m_uiCurrentStep);
            ...
            { 
               ...
               io::vtk::mesh2vtu_slice(m_uiMesh,s_val,s_norm,fPrefix,2,fDataNames,fData,(2*numEvolVars+numConstVars),(const char **)&pDataNames_char[0],(const double **)pData);
            }else{ 
               io::vtk::mesh2vtuFine(m_uiMesh,fPrefix,2,fDataNames,fData,(2*numEvolVars+numConstVars),(const char **)&pDataNames_char[0],(const double **)pData);
            }
        ...
        void RK4_EM2::performGhostExchangeVars(double** zipIn)
        {

            em2::timer::t_ghostEx_sync.start();

            for(unsigned int v=0;v<em2::EM2_NUM_VARS;v++)
                m_uiMesh->performGhostExchange(zipIn[v]);

            em2::timer::t_ghostEx_sync.stop();

        }

        void RK4_EM2::intergridTransferVars(double**& zipIn, const ot::Mesh* pnewMesh)
        {
            em2::timer::t_gridTransfer.start();

            for(unsigned int v=0;v<em2::EM2_NUM_VARS;v++)
                m_uiMesh->interGridTransfer(zipIn[v],pnewMesh);

            em2::timer::t_gridTransfer.stop();

        }

        void RK4_EM2::unzipVars(double** zipIn , double** uzipOut)
        {
            em2::timer::t_unzip_sync.start();

            for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                m_uiMesh->unzip(zipIn[index],uzipOut[index]);

            em2::timer::t_unzip_sync.stop();

        }

        void RK4_EM2::unzipVars_async(double ** zipIn , double **uzipOut)
        {

            em2::timer::t_unzip_async.start();

            for(unsigned int var=0;var<em2::EM2_NUM_VARS;var+=em2::EM2_ASYNC_COMM_K){

                for(unsigned int i=0;(i<em2::EM2_ASYNC_COMM_K);i++)
            ...
            em2::timer::t_unzip_async.stop();


        }


        void RK4_EM2::zipVars(double** uzipIn , double** zipOut)
        {
            em2::timer::t_zip.start();

            for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                m_uiMesh->zip(uzipIn[index],zipOut[index]);

            em2::timer::t_zip.stop();

        }


        void RK4_EM2::applyBoundaryConditions()
        {

        }

        void RK4_EM2::performSingleIteration()
        {
            ...
            if(m_uiMesh->isActive())
            {
                ...
                unsigned int bflag;
                double dx,dy,dz;
                const Point pt_min(em2::EM2_COMPD_MIN[0],em2::EM2_COMPD_MIN[1],em2::EM2_COMPD_MIN[2]);
                const Point pt_max(em2::EM2_COMPD_MAX[0],em2::EM2_COMPD_MAX[1],em2::EM2_COMPD_MAX[2]);
                const unsigned int PW=em2::EM2_PADDING_WIDTH;


                for(unsigned int stage=0;stage<(em2::EM2_RK4_STAGES-1);stage++)
                {
                    ...
                    for(unsigned int blk=0;blk<blkList.size();blk++)
                    {
                        ...
                        //std::cout<<" dx: "<<dx<<" ele x : "<<(1u<<(m_uiMaxDepth-m_uiMesh->getAllElements()[blkList[blk].getLocalElementBegin()].getLevel()))/(double)em2::EM2_ELE_ORDER<<" computed blk: "<<blkList[blk].computeGridDx()<<std::endl;
                        ...
                        em2rhs(m_uiUnzipVarRHS, (const double **)m_uiUnzipVar, offset, ptmin, ptmax, sz, bflag);
                    ...
                    current_t_adv=current_t+RK4_T[(em2::EM2_RK4_STAGES-1)]*m_uiT_h;


                    #ifdef DEBUG_RK_SOLVER
                        if(!rank)std::cout<<" stage: "<<(em2::EM2_RK4_STAGES-1)<<" begin: "<<std::endl;
                    ...
                    #ifdef DEBUG_RK_SOLVER
                                        if(!rank)std::cout<<" stage: "<<(em2::EM2_RK4_STAGES-1)<<" af rhs UNZIP RHS TEST:"<<std::endl;
                                        for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                                            ot::test::isUnzipInternalNaN(m_uiMesh,m_uiUnzipVarRHS[index]);
                    #endif

                    zipVars(m_uiUnzipVarRHS,m_uiStage[(em2::EM2_RK4_STAGES-1)]);
                    ...
                    for(unsigned int node=nodeLocalBegin;node<nodeLocalEnd;node++)
                    {
                        ...
                        for(unsigned int index=0;index<em2::EM2_NUM_VARS;index++)
                        {
                            ...
                            for(unsigned int s=0;s<(em2::EM2_RK4_STAGES);s++)
                            {
        ...
        void RK4_EM2::rkSolve()
        {
            ...
            for(double t=m_uiCurrentTime;t<m_uiTimeEnd;t=t+m_uiT_h)
            {
                ...
                em2::timer::t_ioCheckPoint.start();
                if((m_uiMesh->isActive()) && (m_uiCurrentStep%em2::EM2_CHECKPT_FREQ)==0)
                    storeCheckPoint(em2::EM2_CHKPT_FILE_PREFIX.c_str());
                em2::timer::t_ioCheckPoint.stop();
                // write sol to vtu.
                if((m_uiMesh->isActive()) && (m_uiCurrentStep%em2::EM2_TIME_STEP_OUTPUT_FREQ)==0)
                {

                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_E0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_E0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "executing step: " << m_uiCurrentStep << " dt: " << m_uiT_h << " rk_time : "<< m_uiCurrentTime << std::endl;
                        std::cout << "\t ||VAR::U_E0|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }
                    
                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_E1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_E1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_E1|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_E2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_E2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_E2|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_A0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_A0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_A0|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_A1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_A1]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_A1|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_A2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_A2]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_A2|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_PSI]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_PSI]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_PSI|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    l_min=vecMin(m_uiPrevVar[em2::VAR::U_GAMMA]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    l_max=vecMax(m_uiPrevVar[em2::VAR::U_GAMMA]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                    if(!m_uiMesh->getMPIRank()) {
                        std::cout << "\t ||VAR::U_GAMMA|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                    }

                    em2::timer::profileInfoIntermediate(em2::EM2_PROFILE_FILE_PREFIX.c_str(),m_uiMesh,m_uiCurrentStep);

                }

                if((m_uiCurrentStep%em2::EM2_TIME_STEP_OUTPUT_FREQ)==0)
                    em2::timer::resetSnapshot();

                if((m_uiCurrentStep%em2::EM2_REMESH_TEST_FREQ)==0)
                {
                    #ifdef RK_SOLVER_OVERLAP_COMM_AND_COMP
                                        unzipVars_async(m_uiPrevVar,m_uiUnzipVar);
                    #else
                    ...
                    else 
                    {
                        ...
                        if(em2::EM2_REFINEMENT_MODE == em2::RefinementMode::WAMR)
                            isRefine=m_uiMesh->isReMeshUnzip((const double **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,em2::EM2_DENDRO_AMR_FAC);
                        

                        if(em2::EM2_REFINEMENT_MODE == em2::RefinementMode::FR)
                            isRefine=em2::isRemeshForce(m_uiMesh,(const double **)m_uiUnzipVar,em2::VAR::U_A0,em2::EM2_CHI_REFINE_VAL,em2::EM2_CHI_COARSEN_VAL,true);


                        if(em2::EM2_REFINEMENT_MODE == em2::RefinementMode::WAMR_FR)
                        {
                            const bool isRefine1 = m_uiMesh->isReMeshUnzip((const double **)m_uiUnzipVar,refineVarIds,refineNumVars,waveletTolFunc,em2::EM2_DENDRO_AMR_FAC);
                            const bool isRefine2 = isRefine=em2::isRemeshForce(m_uiMesh,(const double **)m_uiUnzipVar,em2::VAR::U_A0,em2::EM2_CHI_REFINE_VAL,em2::EM2_CHI_COARSEN_VAL,false);
                    ...
                    if(isRefine)
                    {
                       ...
                       for(unsigned int index=0; index<em2::EM2_CONSTRAINT_NUM_VARS; index++)
                       {
                        ...
                        if(m_uiMesh->isActive())
                        {
                            ...
                            l_max=vecMax(m_uiPrevVar[em2::VAR::U_A0]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                            if(!(m_uiMesh->getMPIRank())) {
                                std::cout << "transfer completed:    ||VAR::U_A0|| (min, max) : ("<<l_min<<", "<<l_max<<" ) "<<std::endl;
                            }
                ...
                if((m_uiCurrentStep%em2::EM2_IO_OUTPUT_FREQ)==0)
                {
                    ...
                    if(m_uiMesh->isActive())
                    {
                            ...
                            double constraintMaskedL2[em2::EM2_CONSTRAINT_NUM_VARS];
                            for(unsigned int index=0; index<em2::EM2_CONSTRAINT_NUM_VARS; index++)
                            {
                            ...
                            l_min=vecMin(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVA]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                            l_max=vecMax(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVA]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                            double l2_norm=normL2(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVA]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                            l2_norm = l2_norm/sqrt(total_dof);

                            if(!m_uiMesh->getMPIRank()) {
                                        std::cout << "executing step: " << m_uiCurrentStep << " dt: " << m_uiT_h << " rk_time : "<< m_uiCurrentTime << std::endl;
                                        std::cout << "\t ||VAR::C_DIVA|| (min, max, l2) : ("<<l_min<<", "<<l_max<<", "<<l2_norm<<" ) "<<std::endl;
                                    }

                            l_min=vecMin(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVE]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                            l_max=vecMax(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVE]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                            l2_norm=normL2(m_uiConstraintVars[em2::VAR_CONSTRAINT::C_DIVE]+m_uiMesh->getNodeLocalBegin(),(m_uiMesh->getNumLocalMeshNodes()),m_uiMesh->getMPICommunicator());
                            l2_norm = l2_norm/sqrt(total_dof);

                            if(!m_uiMesh->getMPIRank()) {
                                        std::cout << "\t ||VAR::C_DIVE|| (min, max, l2) : ("<<l_min<<", "<<l_max<<", "<<l2_norm<<" ) "<<std::endl;
                            }
                        ...
                        //writeToVTU(m_uiPrevVar,m_uiConstraintVars,em2::EM2_NUM_EVOL_VARS_VTU_OUTPUT,0,em2::EM2_VTU_OUTPUT_EVOL_INDICES,NULL);
                        writeToVTU(m_uiPrevVar,m_uiConstraintVars,em2::EM2_NUM_EVOL_VARS_VTU_OUTPUT,em2::EM2_NUM_CONST_VARS_VTU_OUTPUT,em2::EM2_VTU_OUTPUT_EVOL_INDICES,em2::EM2_VTU_OUTPUT_CONST_INDICES,em2::EM2_VTU_Z_SLICE_ONLY);

                    }

                }

                em2::timer::t_rkStep.start();
                performSingleIteration();
                em2::timer::t_rkStep.stop();
        ...
        void RK4_EM2::storeCheckPoint(const char * fNamePrefix)
        {
            ...
            if(m_uiMesh->isActive())
            {
                ...
                (m_uiCurrentStep%(2*em2::EM2_CHECKPT_FREQ)==0) ? cpIndex=0 : cpIndex=1; // to support alternate file writing.
                ...
                unsigned int numVars=em2::EM2_NUM_VARS;
                const char ** varNames=em2::EM2_VAR_NAMES;

                sprintf(fName,"%s_%d_%d.var",fNamePrefix,cpIndex,rank);
                io::checkpoint::writeVecToFile(fName,m_uiMesh,(const double **)m_uiPrevVar,em2::EM2_NUM_VARS);
                ...
                if(!rank)
                {
                    ...
                    checkPoint["DENDRO_RK45_WAVELET_TOLERANCE"]=em2::EM2_WAVELET_TOL;
                    checkPoint["DENDRO_RK45_LOAD_IMB_TOLERANCE"]=em2::EM2_LOAD_IMB_TOL;
        ...
        void RK4_EM2::restoreCheckPoint(const char * fNamePrefix,MPI_Comm comm)
        {
            ...
            for(unsigned int cpIndex=0;cpIndex<2;cpIndex++)
            {
                ...
                if(!rank)
                {
                    ...
                    if(restoreStatus==0)
                    {
                        ...
                        em2::EM2_WAVELET_TOL=checkPoint["DENDRO_RK45_WAVELET_TOLERANCE"];
                        em2::EM2_LOAD_IMB_TOL=checkPoint["DENDRO_RK45_LOAD_IMB_TOLERANCE"];
                ...
                par::Mpi_Bcast(& em2::EM2_WAVELET_TOL,1,0,comm);
                par::Mpi_Bcast(&em2::EM2_LOAD_IMB_TOL,1,0,comm);
                ...
                if(isActive) {
                    ...
                    restoreStatus=io::checkpoint::readVecFromFile(fName,newMesh,m_uiPrevVar,em2::EM2_NUM_VARS);
