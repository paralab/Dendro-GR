...
int main(int argc, char** argv) {
    ...
    //     EM2_NUM_VARS: "<<em2::EM2_NUM_VARS<<" is not divisable by
    //     EM2_ASYNC_COMM_K: "<<em2::EM2_ASYNC_COMM_K<<std::endl; exit(0);
    // }
    ...
    ot::Mesh* mesh =
        ot::createMesh(tmpNodes.data(), tmpNodes.size(), em2::EM2_ELE_ORDER,
                       comm, 1, ot::SM_TYPE::FDM, em2::EM2_DENDRO_GRAIN_SZ,
                       em2::EM2_LOAD_IMB_TOL, em2::EM2_SPLIT_FIX);

    if (!em2::EM2_ENABLE_BLOCK_ADAPTIVITY)
        ot::meshWAMRConvergence(mesh, f_init, em2::EM2_WAVELET_TOL,
                                em2::EM2_NUM_VARS, em2::EM2_ELE_ORDER,
                                em2::EM2_REFINE_VARIABLE_INDICES,
                                em2::EM2_NUM_REFINE_VARS, 5);
    ...
    if (TS_MODE == 0) {
        ...
        ts::ExplicitNUTS<DendroScalar, em2::EM2Ctx>* enuts =
            new ts::ExplicitNUTS<DendroScalar, em2::EM2Ctx>(appCtx);
        ...
        for (enuts->init(); enuts->curr_time() < em2::EM2_RK45_TIME_END;
            ...
            if (isRemesh) {
                ...
                appCtx->remesh_and_gridtransfer(
                    em2::EM2_DENDRO_GRAIN_SZ, em2::EM2_LOAD_IMB_TOL,
                    em2::EM2_SPLIT_FIX, true, false, false);
                // appCtx->terminal_output();
            }

            enuts->sync_with_mesh();

            if ((step % em2::EM2_IO_OUTPUT_FREQ) == 0) appCtx->write_vtu();

            if ((step % em2::EM2_CHECKPT_FREQ) == 0) appCtx->write_checkpt();
        ...
        // GTS code here.
        em2::EM2Ctx* appCtx = new em2::EM2Ctx(mesh);
        ts::ETS<DendroScalar, em2::EM2Ctx>* ets =
            new ts::ETS<DendroScalar, em2::EM2Ctx>(appCtx);
        ets->set_evolve_vars(appCtx->get_evolution_vars());
        ets->set_ets_coefficients(tsType);

        for (ets->init(); ets->curr_time() < em2::EM2_RK45_TIME_END;
             ets->evolve()) {
