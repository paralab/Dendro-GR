...
EM2Ctx::EM2Ctx(ot::Mesh* pMesh) : Ctx() {
    ...
    m_uiCVar = this->create_vec(ts::CTXVType::CONSTRAINT, true, false, false,
                                EM2_CONSTRAINT_NUM_VARS);
    m_uiPVar =
        this->create_vec(ts::CTXVType::PRIMITIVE, true, false, false,
                         EM2_NUM_VARS);  // used as to compare the analytical
                                         // solution with the evolved one.
    ...
    m_uiCUnzip[0] = this->create_vec(ts::CTXVType::CONSTRAINT, false, true,
                                     false, EM2_CONSTRAINT_NUM_VARS);
    m_uiCUnzip[1] = this->create_vec(ts::CTXVType::CONSTRAINT, false, true,
                                     false, EM2_CONSTRAINT_NUM_VARS);
...
int EM2Ctx::rhs(DVec* in, DVec* out, unsigned int sz, DendroScalar time) {
    ...
    for (unsigned int blk = 0; blk < numBlocks; blk++) {
        ...
        em2rhs(unzipOut, (const DendroScalar**)unzipIn, offset, ptmin, ptmax,
               lsz, bflag);
    ...
    //     {
    //         std::cout<<" cg::v: "<<v<<" n: "<<n<< " val:
    //         "<<(out[0].GetVecArray()+v*m_uiMesh->getDegOfFreedom())[n]<<std::endl;;
    //     }
    // }
...
int EM2Ctx::rhs_blk(const DendroScalar* in, DendroScalar* out, unsigned int dof,
    ...
    em2rhs(unzipOut, (const DendroScalar**)unzipIn, 0, ptmin, ptmax, lsz,
           bflag);
...
int EM2Ctx::write_vtu() {
    ...
    for (unsigned int i = 0; i < numConstVars; i++) {
        pDataNames.push_back(std::string(
            EM2_CONSTRAINT_VAR_NAMES[EM2_VTU_OUTPUT_CONST_INDICES[i]]));
        pData[i] = consVar[EM2_VTU_OUTPUT_CONST_INDICES[i]];
    }

    for (unsigned int i = 0; i < numEvolVars; i++) {
        pDataNames.push_back(
            std::string(EM2_VAR_NAMES[EM2_VTU_OUTPUT_EVOL_INDICES[i]]));
    ...
    for (unsigned int i = 0; i < numPrimVars; i++) {
        ...
        pData[numConstVars + numEvolVars + i] =
            primVar[EM2_VTU_OUTPUT_EVOL_INDICES[i]];
    }
...
int EM2Ctx::write_checkpt() {
    if (m_uiMesh->isActive()) {
        ...
        (m_uiTinfo._m_uiStep % (2 * EM2_CHECKPT_FREQ) == 0)
            ? cpIndex = 0
        ...
        sprintf(fName, "%s_octree_%d_%d.oct", EM2_CHKPT_FILE_PREFIX.c_str(),
                cpIndex, rank);
        ...
        sprintf(fName, "%s_%d_%d.var", EM2_CHKPT_FILE_PREFIX.c_str(), cpIndex,
                rank);
        io::checkpoint::writeVecToFile(fName, m_uiMesh, (const double**)eVar,
                                       em2::EM2_NUM_VARS);

        if (!rank) {
            sprintf(fName, "%s_step_%d.cp", EM2_CHKPT_FILE_PREFIX.c_str(),
                    cpIndex);
...
int EM2Ctx::restore_checkpt() {
    ...
    if (isActive) {
        ...
        sprintf(fName, "%s_%d_%d.var", EM2_CHKPT_FILE_PREFIX.c_str(),
                restoreFileIndex, activeRank);
...
int EM2Ctx::pre_timestep(DVec sIn) { return 0; }

int EM2Ctx::pre_stage(DVec sIn) { return 0; }

int EM2Ctx::post_stage(DVec sIn) { return 0; }

int EM2Ctx::post_timestep(DVec sIn) { return 0; }
...
bool EM2Ctx::is_remesh() {
    ...
    if (EM2_ENABLE_BLOCK_ADAPTIVITY) return isRefine;
    ...
    isRefine = m_uiMesh->isReMeshUnzip((const double**)unzipVar, refineVarIds,
                                       EM2_NUM_REFINE_VARS, waveletTolFunc,
                                       EM2_DENDRO_AMR_FAC);
...
DVec EM2Ctx::get_evolution_vars() { return m_uiEVar; }

DVec EM2Ctx::get_constraint_vars() { return m_uiCVar; }

DVec EM2Ctx::get_primitive_vars() { return m_uiPVar; }
...
int EM2Ctx::compute_constraints() {
    ...
    for (unsigned int blk = 0; blk < blkList.size(); blk++) {
        ...
        physical_constraints(cunzipOut, (const DendroScalar**)eunzipIn, offset,
    ...
    if (!m_uiMesh->getMPIRank()) {
        std::cout << "executing step: " << m_uiTinfo._m_uiStep
                  << " dt: " << m_uiTinfo._m_uiTh
                  << " rk_time : " << m_uiTinfo._m_uiT << std::endl;

        for (unsigned int v = 0; v < EM2_CONSTRAINT_NUM_VARS; v++)
            std::cout << "\t ||" << EM2_CONSTRAINT_VAR_NAMES[v]
                      << "|| (min, max, l2,l2rs) : (" << vmin[v] << ", "
                      << vmax[v] << ", " << l2_norm[v] << ", " << l2_rs[v]
                      << " ) " << std::endl;
    }
    ...
    if (!m_uiMesh->getMPIRank()) {
        ...
        if (m_uiTinfo._m_uiStep == 0) {
            ...
            for (unsigned int v = 0; v < EM2_CONSTRAINT_NUM_VARS; v++)
                fileGW << EM2_CONSTRAINT_VAR_NAMES[v] << "_l2\t"
                       << EM2_CONSTRAINT_VAR_NAMES[v] << "_l2rs\t";
...
int EM2Ctx::compute_primitives() {
    ...
    if (!m_uiMesh->getMPIRank()) {
        std::ofstream fileGW;
        char fName[256];
        sprintf(fName, "%s_Primitives.dat",
                em2::EM2_PROFILE_FILE_PREFIX.c_str());
        fileGW.open(fName, std::ofstream::app);
        // writes the header
        if (m_uiTinfo._m_uiStep == 0) {
            fileGW << "timestep\t"
    ...
    if (m_uiMesh->isActive()) {
        ...
        if ((m_uiMesh->isActive()) &&
            ...
            if (!m_uiMesh->getMPIRank()) {
                ...
                for (unsigned int v = 0; v < EM2_NUM_VARS; v++)
                    std::cout << "\t ||diff_[" << EM2_VAR_NAMES[v]
                              << "]|| (min, max, l2,l2rs) : (" << vmin[v]
                              << ", " << vmax[v] << ", " << l2_norm[v] << ", "
                              << l2_rs[v] << " ) " << std::endl;
            }
...
unsigned int EM2Ctx::getBlkTimestepFac(unsigned int blev, unsigned int lmin,
                                       unsigned int lmax) {
