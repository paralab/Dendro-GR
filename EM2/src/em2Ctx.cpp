 ...
 * @brief Context file for EM2 project. 
 * @version 0.1
 * @date 2020-07-18
 * 
...
namespace em2
{
    EM2Ctx::EM2Ctx(ot::Mesh* pMesh) : Ctx()
    {
        m_uiMesh = pMesh;
        // variable allocation for evolution variables
        m_uiEVar = this->create_vec(ts::CTXVType::EVOLUTION,true,false,false,EM2_NUM_VARS);
        m_uiCVar = this->create_vec(ts::CTXVType::CONSTRAINT,true,false,false,EM2_CONSTRAINT_NUM_VARS);
        m_uiPVar = this->create_vec(ts::CTXVType::PRIMITIVE,true,false,false,EM2_NUM_VARS); // used as to compare the analytical solution with the evolved one. 

        m_uiEUnzip[0] = this->create_vec(ts::CTXVType::EVOLUTION,false,true,false,EM2_NUM_VARS);
        m_uiEUnzip[1] = this->create_vec(ts::CTXVType::EVOLUTION,false,true,false,EM2_NUM_VARS);

        m_uiCUnzip[0] = this->create_vec(ts::CTXVType::CONSTRAINT,false,true,false,EM2_CONSTRAINT_NUM_VARS);
        m_uiCUnzip[1] = this->create_vec(ts::CTXVType::CONSTRAINT,false,true,false,EM2_CONSTRAINT_NUM_VARS);

        m_uiTinfo._m_uiStep=0;
        m_uiTinfo._m_uiT = 0;
        m_uiTinfo._m_uiTb = EM2_RK45_TIME_BEGIN;
        m_uiTinfo._m_uiTe = EM2_RK45_TIME_END;
        m_uiTinfo._m_uiTh = EM2_RK45_TIME_STEP_SIZE;     

        m_uiElementOrder = EM2_ELE_ORDER;

        m_uiMinPt = Point(EM2_GRID_MIN_X,EM2_GRID_MIN_Y,EM2_GRID_MIN_Z);
        m_uiMaxPt = Point(EM2_GRID_MAX_X,EM2_GRID_MAX_Y,EM2_GRID_MAX_Z);
    ...
    int EM2Ctx::initialize()
    {

        if(EM2_RESTORE_SOLVER)
        {
        ...
        const unsigned int numEVars = EM2_NUM_VARS;
    ...
    int EM2Ctx::rhs(DVec* in , DVec* out, unsigned int sz , DendroScalar time)
    {
        ...
        this->unzip(in[0],m_uiEUnzip[0] , EM2_ASYNC_COMM_K);
        ...
        for(unsigned int blk =0; blk < numBlocks; blk++)
        {
            ...
            em2rhs(unzipOut,(const DendroScalar**)unzipIn, offset, ptmin, ptmax, lsz, bflag);
        ...
        this->zip(m_uiEUnzip[1], out[0], EM2_ASYNC_COMM_K);
    ...
    int EM2Ctx::rhs_blk(const DendroScalar* in, DendroScalar* out, unsigned int dof, unsigned int local_blk_id, DendroScalar  blk_time)  
    {
        ...
        em2rhs(unzipOut,(const DendroScalar**)unzipIn, 0, ptmin, ptmax, lsz, bflag);
    ...
    int EM2Ctx::write_vtu()
    {

        if(!m_uiMesh->isActive())
            return 0;
        
        DendroScalar * consVar[EM2_CONSTRAINT_NUM_VARS];
        DendroScalar * evolVar[EM2_NUM_VARS];
        DendroScalar * primVar[EM2_NUM_VARS];
        ...
        const unsigned int numConstVars = EM2_NUM_CONST_VARS_VTU_OUTPUT;
        const unsigned int numEvolVars  = EM2_NUM_EVOL_VARS_VTU_OUTPUT;
        const unsigned int numPrimVars  = EM2_NUM_EVOL_VARS_VTU_OUTPUT;
        ...
        {
            pDataNames.push_back(std::string(EM2_CONSTRAINT_VAR_NAMES[EM2_VTU_OUTPUT_CONST_INDICES[i]]));
            pData[i]=consVar[EM2_VTU_OUTPUT_CONST_INDICES[i]];
        }

        for(unsigned int i=0; i<numEvolVars; i++)
        {
            pDataNames.push_back(std::string(EM2_VAR_NAMES[EM2_VTU_OUTPUT_EVOL_INDICES[i]]));
            pData[numConstVars + i] = evolVar[EM2_VTU_OUTPUT_EVOL_INDICES[i]];
        }

        for(unsigned int i=0; i<numPrimVars; i++)
        {
            pDataNames.push_back(std::string("diff_") + std::string(EM2_VAR_NAMES[EM2_VTU_OUTPUT_EVOL_INDICES[i]]));
            pData[numConstVars + numEvolVars + i] = primVar[EM2_VTU_OUTPUT_EVOL_INDICES[i]];
        }
        ...
        char fPrefix[256];
        sprintf(fPrefix,"%s_%d",EM2_VTU_FILE_PREFIX.c_str(),m_uiTinfo._m_uiStep);
    ...
    int EM2Ctx::write_checkpt()
    {   
        if(m_uiMesh->isActive())
        {
            unsigned int cpIndex;
            (m_uiTinfo._m_uiStep %(2*EM2_CHECKPT_FREQ)==0) ? cpIndex=0 : cpIndex=1; // to support alternate file writing.
            ...
            sprintf(fName,"%s_octree_%d_%d.oct",EM2_CHKPT_FILE_PREFIX.c_str(),cpIndex,rank);
            io::checkpoint::writeOctToFile(fName,pNodes,m_uiMesh->getNumLocalMeshElements());

            unsigned int numVars   = EM2_NUM_VARS;
            const char ** varNames = EM2_VAR_NAMES;

            const unsigned int dof = m_uiEVar.GetDof();
            DendroScalar* eVar[dof];
            m_uiEVar.Get2DVec(eVar);

            sprintf(fName,"%s_%d_%d.var",EM2_CHKPT_FILE_PREFIX.c_str(),cpIndex,rank);
            io::checkpoint::writeVecToFile(fName,m_uiMesh,(const double **)eVar,em2::EM2_NUM_VARS);


            if(!rank)
            {
                sprintf(fName,"%s_step_%d.cp",EM2_CHKPT_FILE_PREFIX.c_str(),cpIndex);
                ...
                checkPoint["DENDRO_TS_WAVELET_TOLERANCE"]  = EM2_WAVELET_TOL;
                checkPoint["DENDRO_TS_LOAD_IMB_TOLERANCE"] = EM2_LOAD_IMB_TOL;
        ...
        for(unsigned int cpIndex=0; cpIndex<2; cpIndex++) {
            ...
            if(!rank)
            {
                ...
                if(restoreStatus==0)
                {
                    ...
                    EM2_WAVELET_TOL=checkPoint["DENDRO_TS_WAVELET_TOLERANCE"];
                    EM2_LOAD_IMB_TOL=checkPoint["DENDRO_TS_LOAD_IMB_TOLERANCE"];
            ...
            if(!rank)
            {
                sprintf(fName,"%s_step_%d.cp", EM2_CHKPT_FILE_PREFIX.c_str(), restoreFileIndex);
                std::ifstream infile(fName);
                if(!infile) {
                    std::cout<<fName<<" file open failed "<<std::endl;
                    restoreStatus=1;
            ...
            if(isActive) {
                ...
                sprintf(fName, "%s_octree_%d_%d.oct", EM2_CHKPT_FILE_PREFIX.c_str(),restoreFileIndex,activeRank);
            ...
            if(isActive) {
                ...
                sprintf(fName,"%s_%d_%d.var",EM2_CHKPT_FILE_PREFIX.c_str(),restoreFileIndex,activeRank);
                restoreStatus=io::checkpoint::readVecFromFile(fName,newMesh,inVec,EM2_NUM_VARS);
    ...
    int EM2Ctx::pre_timestep(DVec sIn)
    {
        return 0;
    }

    int EM2Ctx::pre_stage(DVec  sIn)
    {
        return 0;
    }

    int EM2Ctx::post_stage(DVec sIn)
    { 
        return 0;
    }

    int EM2Ctx::post_timestep(DVec sIn)
    {
        return 0;
    }

    bool EM2Ctx::is_remesh()
    {
        ...
        if(EM2_ENABLE_BLOCK_ADAPTIVITY)
            return isRefine;
        
        MPI_Comm comm = m_uiMesh->getMPIGlobalCommunicator();
        this->unzip(m_uiEVar,m_uiEUnzip[0],EM2_ASYNC_COMM_K);

        DendroScalar** unzipVar;
        m_uiEUnzip[0].Get2DArray(unzipVar, false);

        unsigned int refineVarIds[EM2_NUM_REFINE_VARS];
        for(unsigned int vIndex=0; vIndex<EM2_NUM_REFINE_VARS; vIndex++)
            refineVarIds[vIndex]=EM2_REFINE_VARIABLE_INDICES[vIndex];

        double wTol=EM2_WAVELET_TOL;
        ...
        isRefine=m_uiMesh->isReMeshUnzip((const double **)unzipVar,refineVarIds,EM2_NUM_REFINE_VARS,waveletTolFunc,EM2_DENDRO_AMR_FAC); 
    ...
    DVec EM2Ctx::get_evolution_vars()
    {
        return m_uiEVar;
    }
    
    DVec EM2Ctx::get_constraint_vars()
    {
        return m_uiCVar;
    }

    DVec EM2Ctx::get_primitive_vars()
    {
        return m_uiPVar;
    }

    int EM2Ctx::compute_constraints()
    {   
        ...
        DendroScalar * eunzipIn[em2::EM2_NUM_VARS];
        DendroScalar * cunzipIn[em2::EM2_CONSTRAINT_NUM_VARS];
        DendroScalar * cunzipOut[em2::EM2_CONSTRAINT_NUM_VARS]; 
        ...
        DendroScalar l2_norm[EM2_CONSTRAINT_NUM_VARS];
        DendroScalar l2_rs  [EM2_CONSTRAINT_NUM_VARS];
        DendroScalar vmin   [EM2_CONSTRAINT_NUM_VARS];
        DendroScalar vmax   [EM2_CONSTRAINT_NUM_VARS];

        for(unsigned int v=0; v < EM2_CONSTRAINT_NUM_VARS; v++)
        {
        ...
        if(!m_uiMesh->getMPIRank()) {
            ...
            for(unsigned int v=0; v <EM2_CONSTRAINT_NUM_VARS; v++)
                std::cout << "\t ||"<<EM2_CONSTRAINT_VAR_NAMES[v]<<"|| (min, max, l2,l2rs) : ("<<vmin[v]<<", "<<vmax[v]<<", "<<l2_norm[v]<<", "<<l2_rs[v]<<" ) "<<std::endl;
        ...
        if(!m_uiMesh->getMPIRank())
        {
            ...
            std::ofstream fileGW;
            char fName[256];
            sprintf(fName,"%s_Constraints.dat",EM2_PROFILE_FILE_PREFIX.c_str());
            ...
            if(m_uiTinfo._m_uiStep==0)
            {
                ...
                for(unsigned int v=0; v < EM2_CONSTRAINT_NUM_VARS; v++)
                    fileGW<<EM2_CONSTRAINT_VAR_NAMES[v]<<"_l2\t"<<EM2_CONSTRAINT_VAR_NAMES[v]<<"_l2rs\t";
    ...
    int EM2Ctx::compute_primitives()
    {
                ...
                for(unsigned int j=0;j<(eleOrder+1);j++ )
                    for(unsigned int i=0;i<(eleOrder+1);i++)
                    {
                        ...
                        if(nodeLookUp_CG>=nodeLocalBegin && nodeLookUp_CG<nodeLocalEnd)
                        {
                            ...
                            em2::analyticalSol((double)x,(double)y,(double)z,m_uiTinfo._m_uiT,var);
        ...
        double l2_norm[em2::EM2_NUM_VARS];
        for(unsigned int v=0; v < em2::EM2_NUM_VARS; v++)
        ...
        if(!m_uiMesh->getMPIRank())
        {
            ...
            std::ofstream fileGW;
            char fName[256];
            sprintf(fName,"%s_Primitives.dat",em2::EM2_PROFILE_FILE_PREFIX.c_str());
            fileGW.open (fName,std::ofstream::app);
            // writes the header
            if(m_uiTinfo._m_uiStep==0)
            {  
                ...
                for(unsigned int v=0; v < EM2_NUM_VARS; v++)
                    fileGW<<"diff_"<<EM2_VAR_NAMES[v]<<"_l2\t";
    ...
    int EM2Ctx::terminal_output()
    {
        if(m_uiMesh->isActive())
        {
            ...
            if((m_uiMesh->isActive()) && (currentStep%em2::EM2_TIME_STEP_OUTPUT_FREQ)==0)
            {
                for(unsigned int v=0; v < m_uiEVar.GetDof(); v++)
                {
                    ...
                    if(!(m_uiMesh->getMPIRank()))
                    std::cout<<"[EM2Ctx]:  "<<EM2_VAR_NAMES[v]<<" (min,max) : \t ( "<<min<<", "<<max<<" ) "<<std::endl;
                }
                ...
                DendroScalar l2_norm[EM2_NUM_VARS];
                DendroScalar l2_rs  [EM2_NUM_VARS];
                DendroScalar vmin   [EM2_NUM_VARS];
                DendroScalar vmax   [EM2_NUM_VARS];
                ...
                if(!m_uiMesh->getMPIRank()) {
                    ...
                    for(unsigned int v=0; v <EM2_NUM_VARS; v++)
                        std::cout << "\t ||diff_["<<EM2_VAR_NAMES[v]<<"]|| (min, max, l2,l2rs) : ("<<vmin[v]<<", "<<vmax[v]<<", "<<l2_norm[v]<<", "<<l2_rs[v]<<" ) "<<std::endl;
    ...
    unsigned int EM2Ctx::getBlkTimestepFac(unsigned int blev, unsigned int lmin, unsigned int lmax)
    {
