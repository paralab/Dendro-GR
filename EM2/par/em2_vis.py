...
def saveImg(params, step, imgIndex,cam_coords):
    ...
    maxDepth = params["EM2_MAXDEPTH"]
    #CHI_FLOOR = params["CHI_FLOOR"]

    _evolveVars = ["U_CHI", "U_PHI", "chi_analytical", "diff"]
    _constVars = []

    file_prefix = params["EM2_VTU_FILE_PREFIX"]
    img_prefix = params["EM2_IMG_FILE_PREFIX"]
    ...
    numEVolVars = params["EM2_NUM_EVOL_VARS_VTU_OUTPUT"]
    numConstVars = 0;
    numTotal = numEVolVars+numConstVars

    varNames = []

    for index in params["EM2_VTU_OUTPUT_EVOL_INDICES"]:
        varNames.append(_evolveVars[index])

    #for index in params["EM2_VTU_OUTPUT_CONST_INDICES"]:
    ...
    for varName in varNames:
        ...
        bh_range_max = params["EM2_GRID_MAX_X"]
        bh_range_min = params["EM2_GRID_MIN_X"]
        ...
        if(params["ANNOTATE_TIME"] == 1):
            ...
            annotateTimeFilter1.Format = 'Time: %f s' % (
                params["EM2_CFL_FACTOR"]*(float(bh_range_max-bh_range_min)/(1 << maxDepth))*float(step))
...
def main():
    ...
    zfac=0.2*((params["EM2_TIMESTEP_END"]-params["EM2_TIMESTEP_BEGIN"])/(float)(params["EM2_IO_OUTPUT_FREQ"]))
    zfac=(0.8*s_z + 2*s_z)/zfac
    
    yfac=0.2*((params["EM2_TIMESTEP_END"]-params["EM2_TIMESTEP_BEGIN"])/(float)(params["EM2_IO_OUTPUT_FREQ"]))
    yfac=(2*s_y)/yfac
    #print(zfac)

    params["EM2_TIMESTEP_BEGIN"] = int(sys.argv[2])
    params["EM2_TIMESTEP_END"] = int(sys.argv[3])

    n_procs = servermanager.vtkProcessModule.GetProcessModule().GetNumberOfLocalPartitions()
    print ("number of ranks: %d "%n_procs)
    imgCount = params["EM2_TIMESTEP_BEGIN"]/params["EM2_IO_OUTPUT_FREQ"]
    #writeZSlice(params)
    for step in range(params["EM2_TIMESTEP_BEGIN"], params["EM2_TIMESTEP_END"], params["EM2_IO_OUTPUT_FREQ"]):
